	...
	protected void addTags(HolderLookup.Provider provider) {
		(new BlockItemTagsProvider() {
			...
			protected TagAppender<Block, Block> tag(TagKey<Block> tagKey, TagKey<Item> tagKey2) {
				return VanillaBlockTagsProvider.this.tag(tagKey);
			}
		}).run();
		...
		this.tag(BlockTags.REPLACEABLE)
			.addAll(provider.lookupOrThrow(Registries.BLOCK).listElements().map(Holder.Reference::value).filter(block -> block.defaultBlockState().canBeReplaced()));
