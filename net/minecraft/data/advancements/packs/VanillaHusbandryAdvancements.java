...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
	...
	public void generate(HolderLookup.Provider provider, Consumer<AdvancementHolder> consumer) {
		...
		HolderLookup<FrogVariant> holderLookup = provider.lookupOrThrow(Registries.FROG_VARIANT);
		HolderLookup<CatVariant> holderLookup2 = provider.lookupOrThrow(Registries.CAT_VARIANT);
		HolderLookup<WolfVariant> holderLookup3 = provider.lookupOrThrow(Registries.WOLF_VARIANT);
		...
		addCatVariants(Advancement.Builder.advancement(), holderLookup2)
		...
		addTamedWolfVariants(Advancement.Builder.advancement(), holderLookup3)
			.parent(advancementHolder4)
			.display(
				Items.BONE,
		...
		AdvancementHolder advancementHolder11 = addLeashedFrogVariants(holderGetter, holderGetter2, holderLookup, Advancement.Builder.advancement())
						...
						.withComponents(
							DataComponentMatchers.Builder.components()
								.partial(
									...
									EnchantmentsPredicate.enchantments(List.of(new EnchantmentPredicate(registryLookup.getOrThrow(Enchantments.SILK_TOUCH), MinMaxBounds.Ints.atLeast(1))))
								)
								.build()
						),
		...
		Advancement.Builder.advancement()
			...
			.addCriterion(
				...
				PlayerInteractTrigger.TriggerInstance.equipmentSheared(
					ItemPredicate.Builder.item().of(holderGetter2, Items.WOLF_ARMOR),
									...
									EntityEquipmentPredicate.Builder.equipment()
										.body(
											ItemPredicate.Builder.item()
												...
												.withComponents(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.DAMAGE, 0)).build())
										)
								)
						)
					)
	...
	private static Advancement.Builder addLeashedFrogVariants(
		HolderGetter<EntityType<?>> holderGetter, HolderGetter<Item> holderGetter2, HolderLookup<FrogVariant> holderLookup, Advancement.Builder builder
	) {
		sortedVariants(holderLookup)
			.forEach(
						...
						Optional.of(
							EntityPredicate.wrap(
								EntityPredicate.Builder.entity()
									...
									.components(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.FROG_VARIANT, reference)).build())
							)
						)
					)
				)
			...
			.forEach(
				reference -> builder.addCriterion(
					...
					TameAnimalTrigger.TriggerInstance.tamedAnimal(
						EntityPredicate.Builder.entity()
							.components(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.CAT_VARIANT, reference)).build())
					)
				)
			);
		return builder;
		...
		sortedVariants(holderLookup)
			.forEach(
				reference -> builder.addCriterion(
					...
					TameAnimalTrigger.TriggerInstance.tamedAnimal(
						EntityPredicate.Builder.entity()
							.components(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.WOLF_VARIANT, reference)).build())
					)
				)
			);
		return builder;
