...
public abstract class StoredUserList<K, V extends StoredUserEntry<K>> {
	...
	public boolean add(V storedUserEntry) {
		...
		V storedUserEntry2 = (V)this.map.get(string);
	...
	public boolean remove(K object) {
		V storedUserEntry = (V)this.map.remove(this.getKeyForUser(object));
	...
	public boolean remove(StoredUserEntry<K> storedUserEntry) {
		return this.remove((K)Objects.requireNonNull(storedUserEntry.getUser()));
	}
