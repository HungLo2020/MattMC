...
public class MinecraftPlayerListServiceImpl implements MinecraftPlayerListService {
	...
	public Optional<NameAndId> fetchUserById(UUID uUID) {
		return Optional.ofNullable(this.server.services().sessionService().fetchProfile(uUID, true)).map(profileResult -> new NameAndId(profileResult.playerProfile()));
	}
	...
	public Optional<ServerPlayer> getPlayer(Optional<UUID> optional, Optional<String> optional2) {
		if (optional.isPresent()) {
			return Optional.ofNullable(this.server.getPlayerList().getPlayer((UUID)optional.get()));
		} else {
			return optional2.isPresent() ? Optional.ofNullable(this.server.getPlayerList().getPlayerByName((String)optional2.get())) : Optional.empty();
		}
	}
