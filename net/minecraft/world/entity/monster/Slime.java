...
public class Slime extends Mob implements Enemy {
	...
	public void setSize(int i, boolean bl) {
		...
		this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.2F + 0.1F * j);
	...
	public void tick() {
		...
		if (this.onGround() && !this.wasOnGround) {
			...
			for (int i = 0; i < f * 16.0F; i++) {
				...
				this.level().addParticle(this.getParticleType(), this.getX() + k, this.getY(), this.getZ() + l, 0.0, 0.0, 0.0);
			}
		...
		if (!this.level().isClientSide() && i > 1 && this.isDeadOrDying()) {
			...
			for (int l = 0; l < k; l++) {
				...
				this.convertTo(this.getType(), new ConversionParams(ConversionType.SPLIT_ON_DEATH, false, false, playerTeam), EntitySpawnReason.TRIGGERED, slime -> {
					...
					slime.snapTo(this.getX() + h, this.getY() + 0.5, this.getZ() + m, this.random.nextFloat() * 360.0F, 0.0F);
				});
			}
		}
	...
	protected Vec3 getPassengerAttachmentPoint(Entity entity, EntityDimensions entityDimensions, float f) {
		return new Vec3(0.0, entityDimensions.height() - 0.015625 * this.getSize() * f, 0.0);
	}
