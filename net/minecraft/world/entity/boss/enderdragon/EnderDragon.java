...
public class EnderDragon extends Mob implements Enemy {
		...
		if (this.isDeadOrDying()) {
			...
			if (this.isNoAi()) {
				...
				for (int aa = 0; aa < 3; aa++) {
					...
					float ab = this.getYRot() * (float) (Math.PI / 180.0) + this.rotWrap(sample2.yRot() - sample.yRot()) * (float) (Math.PI / 180.0);
					float ac = Mth.sin(ab);
					float mx = Mth.cos(ab);
					float n = 1.5F;
					float o = (aa + 1) * 2.0F;
					this.tickPart(enderDragonPart, -(v * 1.5F + ac * o) * s, sample2.y() - sample.y() - (o + 1.5F) * t + 1.5, (w * 1.5F + mx * o) * s);
				}
	...
	protected void tickDeath() {
		...
		if (this.dragonDeathTime == 200 && this.level() instanceof ServerLevel serverLevel2) {
			if (serverLevel2.getGameRules().getBoolean(GameRules.RULE_DOMOBLOOT)) {
				ExperienceOrb.award(serverLevel2, this.position(), Mth.floor(i * 0.2F));
			}
	...
	protected void readAdditionalSaveData(ValueInput valueInput) {
		...
		valueInput.getInt("DragonPhase").ifPresent(integer -> this.phaseManager.setPhase(EnderDragonPhase.getById(integer)));
	...
	public void onCrystalDestroyed(ServerLevel serverLevel, EndCrystal endCrystal, BlockPos blockPos, DamageSource damageSource) {
		...
		if (damageSource.getEntity() instanceof Player player) {
			...
			player2 = serverLevel.getNearestPlayer(CRYSTAL_DESTROY_TARGETING, blockPos.getX(), blockPos.getY(), blockPos.getZ());
		}
