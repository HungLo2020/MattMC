...
public class Axolotl extends Animal implements Bucketable {
	...
	public <T> T get(DataComponentType<? extends T> dataComponentType) {
		return dataComponentType == DataComponents.AXOLOTL_VARIANT
	...
	protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T object) {
		if (dataComponentType == DataComponents.AXOLOTL_VARIANT) {
			this.setVariant(castComponentValue(DataComponents.AXOLOTL_VARIANT, object));
			return true;
		} else {
	...
	public boolean hurtServer(ServerLevel serverLevel, DamageSource damageSource, float f) {
		...
		if (!this.isNoAi()
			...
			&& (this.level().random.nextInt(3) < f || g / this.getMaxHealth() < 0.5F)
			&& f < g
			&& this.isInWater()
	...
	public void loadFromBucketTag(CompoundTag compoundTag) {
		...
		compoundTag.getLong("HuntingCooldown")
			.ifPresentOrElse(
				long_ -> this.getBrain().setMemoryWithExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN, true, compoundTag.getLongOr("HuntingCooldown", 0L)),
