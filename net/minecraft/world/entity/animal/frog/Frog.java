...
public class Frog extends Animal {
	...
	public <T> T get(DataComponentType<? extends T> dataComponentType) {
		return dataComponentType == DataComponents.FROG_VARIANT
	...
	protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T object) {
		if (dataComponentType == DataComponents.FROG_VARIANT) {
			this.setVariant(castComponentValue(DataComponents.FROG_VARIANT, object));
			return true;
		} else {
	...
	public static boolean canEat(LivingEntity livingEntity) {
		return livingEntity instanceof Slime slime && slime.getSize() != 1 ? false : livingEntity.getType().is(EntityTypeTags.FROG_FOOD);
	}
