...
public class MobSensor<T extends LivingEntity> extends Sensor<T> {
	...
	public void checkForMobsNearby(T livingEntity) {
		...
		if (!optional.isEmpty()) {
			boolean bl = ((List<LivingEntity>)optional.get()).stream().anyMatch(livingEntity2 -> this.mobTest.test(livingEntity, livingEntity2));
			if (bl) {
	...
	public void mobDetected(T livingEntity) {
		livingEntity.getBrain().setMemoryWithExpiry(this.toSet, true, this.memoryTimeToLive);
	}
