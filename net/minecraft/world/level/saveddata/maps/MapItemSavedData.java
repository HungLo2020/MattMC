...
public class MapItemSavedData extends SavedData {
	...
	private MapItemSavedData(
		ResourceKey<Level> resourceKey, int i, int j, byte b, ByteBuffer byteBuffer, boolean bl, boolean bl2, boolean bl3, List<MapBanner> list, List<MapFrame> list2
	) {
	...
	public static MapItemSavedData createFresh(double d, double e, byte b, boolean bl, boolean bl2, ResourceKey<Level> resourceKey) {
		int i = 128 * (1 << b);
		int j = Mth.floor((d + 64.0) / i);
		int k = Mth.floor((e + 64.0) / i);
		int l = j * i + i / 2 - 64;
		int m = k * i + i / 2 - 64;
	...
	public void tickCarriedBy(Player player, ItemStack itemStack) {
		...
		for (int i = 0; i < this.carriedBy.size(); i++) {
			...
			String string = player2.getPlainTextName();
	...
	private void addDecoration(
		...
		int i = 1 << this.scale;
		float g = (float)(d - this.centerX) / i;
		float h = (float)(e - this.centerZ) / i;
	...
	private static byte clampMapCoordinate(float f) {
		...
		if (f <= -63.0F) {
			...
			return f >= 63.0F ? 127 : (byte)(f * 2.0F + 0.5);
		}
	}
