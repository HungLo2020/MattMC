...
public class ConduitBlockEntity extends BlockEntity {
	...
	public static void clientTick(Level level, BlockPos blockPos, BlockState blockState, ConduitBlockEntity conduitBlockEntity) {
		...
		animationTick(level, blockPos, list, livingEntity, conduitBlockEntity.tickCount);
	...
	public static void serverTick(Level level, BlockPos blockPos, BlockState blockState, ConduitBlockEntity conduitBlockEntity) {
		...
		if (conduitBlockEntity.isActive()) {
			...
			if (l > conduitBlockEntity.nextAmbientSoundActivation) {
				conduitBlockEntity.nextAmbientSoundActivation = l + 60L + level.getRandom().nextInt(40);
	...
	private static void applyEffects(Level level, BlockPos blockPos, List<BlockPos> list) {
		...
		if (!list2.isEmpty()) {
			for (Player player : list2) {
				if (blockPos.closerThan(player.blockPosition(), j) && player.isInWaterOrRain()) {
	...
	private static EntityReference<LivingEntity> updateDestroyTarget(
		@Nullable EntityReference<LivingEntity> entityReference, ServerLevel serverLevel, BlockPos blockPos, boolean bl
	) {
		if (!bl) {
			return null;
		} else if (entityReference == null) {
			return selectNewTarget(serverLevel, blockPos);
		} else {
			LivingEntity livingEntity = EntityReference.getLivingEntity(entityReference, serverLevel);
			return livingEntity != null && livingEntity.isAlive() && blockPos.closerThan(livingEntity.blockPosition(), 8.0) ? entityReference : null;
		}
	}
	...
	private static EntityReference<LivingEntity> selectNewTarget(ServerLevel serverLevel, BlockPos blockPos) {
		List<LivingEntity> list = serverLevel.getEntitiesOfClass(
			LivingEntity.class, getDestroyRangeAABB(blockPos), livingEntity -> livingEntity instanceof Enemy && livingEntity.isInWaterOrRain()
		);
		return list.isEmpty() ? null : EntityReference.of(Util.getRandom(list, serverLevel.random));
	}
	...
	private static void animationTick(Level level, BlockPos blockPos, List<BlockPos> list, @Nullable Entity entity, int i) {
		...
		double d = Mth.sin((i + 35) * 0.1F) / 2.0F + 0.5F;
		d = (d * d + d) * 0.3F;
		Vec3 vec3 = new Vec3(blockPos.getX() + 0.5, blockPos.getY() + 1.5 + d, blockPos.getZ() + 0.5);

		for (BlockPos blockPos2 : list) {
			if (randomSource.nextInt(50) == 0) {
				BlockPos blockPos3 = blockPos2.subtract(blockPos);
				float f = -0.5F + randomSource.nextFloat() + blockPos3.getX();
				float g = -2.0F + randomSource.nextFloat() + blockPos3.getY();
				float h = -0.5F + randomSource.nextFloat() + blockPos3.getZ();
