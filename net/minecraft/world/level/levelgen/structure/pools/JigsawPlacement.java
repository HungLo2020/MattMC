...
public class JigsawPlacement {
	...
	public static Optional<Structure.GenerationStub> addPieces(
		...
		if (structurePoolElement == EmptyPoolElement.INSTANCE) {
			...
			int l = optional2.isEmpty()
				? blockPos3.getY()
				: blockPos.getY() + chunkGenerator.getFirstFreeHeight(j, k, (Heightmap.Types)optional2.get(), levelHeightAccessor, generationContext.randomState());
				...
				return Optional.of(
					new Structure.GenerationStub(
						...
						structurePiecesBuilder -> {
							...
							if (i > 0) {
								AABB aABB = new AABB(
									j - maxDistance.horizontal(),
									Math.max(n - maxDistance.vertical(), levelHeightAccessor.getMinY() + dimensionPadding.bottom()),
									k - maxDistance.horizontal(),
									j + maxDistance.horizontal() + 1,
									Math.min(n + maxDistance.vertical() + 1, levelHeightAccessor.getMaxY() + 1 - dimensionPadding.top()),
									k + maxDistance.horizontal() + 1
								);
