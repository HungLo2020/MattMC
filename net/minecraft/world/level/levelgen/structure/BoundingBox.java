...
public class BoundingBox {
	...
	public static final StreamCodec<ByteBuf, BoundingBox> STREAM_CODEC = StreamCodec.composite(
		...
		(blockPos, blockPos2) -> new BoundingBox(blockPos.getX(), blockPos.getY(), blockPos.getZ(), blockPos2.getX(), blockPos2.getY(), blockPos2.getZ())
	);
	private int minX;
	private int minY;
	private int minZ;
	...
	public static BoundingBox encapsulating(BoundingBox boundingBox, BoundingBox boundingBox2) {
		return new BoundingBox(
			Math.min(boundingBox.minX, boundingBox2.minX),
			Math.min(boundingBox.minY, boundingBox2.minY),
			Math.min(boundingBox.minZ, boundingBox2.minZ),
			Math.max(boundingBox.maxX, boundingBox2.maxX),
			Math.max(boundingBox.maxY, boundingBox2.maxY),
			Math.max(boundingBox.maxZ, boundingBox2.maxZ)
		);
	}
