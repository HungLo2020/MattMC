...
public class NoiseRouterData {
	...
	private static void registerTerrainNoises(
		...
		DensityFunction densityFunction4 = registerAndWrap(bootstrapContext, resourceKey4, offsetToDepth(densityFunction2));
	...
	protected static NoiseRouter overworld(
		...
		DensityFunction densityFunction9 = getFunction(holderGetter, bl ? OFFSET_LARGE : (bl2 ? OFFSET_AMPLIFIED : OFFSET));
		DensityFunction densityFunction10 = getFunction(holderGetter, bl ? FACTOR_LARGE : (bl2 ? FACTOR_AMPLIFIED : FACTOR));
		DensityFunction densityFunction11 = getFunction(holderGetter, bl ? DEPTH_LARGE : (bl2 ? DEPTH_AMPLIFIED : DEPTH));
		DensityFunction densityFunction12 = preliminarySurfaceLevel(densityFunction9, densityFunction10, bl2);
		DensityFunction densityFunction13 = getFunction(holderGetter, bl ? SLOPED_CHEESE_LARGE : (bl2 ? SLOPED_CHEESE_AMPLIFIED : SLOPED_CHEESE));
		DensityFunction densityFunction14 = DensityFunctions.min(
			densityFunction13, DensityFunctions.mul(DensityFunctions.constant(5.0), getFunction(holderGetter, ENTRANCES))
		);
		DensityFunction densityFunction15 = DensityFunctions.rangeChoice(
			densityFunction13, -1000000.0, 1.5625, densityFunction14, underground(holderGetter, holderGetter2, densityFunction13)
		);
		DensityFunction densityFunction16 = DensityFunctions.min(postProcess(slideOverworld(bl2, densityFunction15)), getFunction(holderGetter, NOODLE));
		DensityFunction densityFunction17 = getFunction(holderGetter, Y);
		...
		DensityFunction densityFunction18 = yLimitedInterpolatable(
			densityFunction17, DensityFunctions.noise(holderGetter2.getOrThrow(Noises.ORE_VEININESS), 1.5, 1.5), i, j, 0
		);
		float f = 4.0F;
		DensityFunction densityFunction19 = yLimitedInterpolatable(
				densityFunction17, DensityFunctions.noise(holderGetter2.getOrThrow(Noises.ORE_VEIN_A), 4.0, 4.0), i, j, 0
			)
			.abs();
		DensityFunction densityFunction20 = yLimitedInterpolatable(
				densityFunction17, DensityFunctions.noise(holderGetter2.getOrThrow(Noises.ORE_VEIN_B), 4.0, 4.0), i, j, 0
			)
			.abs();
	...
	private static DensityFunction preliminarySurfaceLevel(DensityFunction densityFunction, DensityFunction densityFunction2, boolean bl) {
		...
		DensityFunction densityFunction5 = remap(
			DensityFunctions.add(
				DensityFunctions.mul(DensityFunctions.constant(0.2734375), densityFunction3.invert()),
				DensityFunctions.mul(DensityFunctions.constant(-1.0), densityFunction4)
			),
			1.5,
			-1.5,
			-64.0,
		...
		DensityFunction densityFunction6 = DensityFunctions.add(
			slideOverworld(
				bl, DensityFunctions.add(noiseGradientDensity(densityFunction3, offsetToDepth(densityFunction4)), DensityFunctions.constant(-0.703125)).clamp(-64.0, 64.0)
			),
