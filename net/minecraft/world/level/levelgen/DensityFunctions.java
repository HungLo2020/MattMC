...
public final class DensityFunctions {
	...
	public static DensityFunction findTopSurface(DensityFunction densityFunction, DensityFunction densityFunction2, int i, int j) {
		return new DensityFunctions.FindTopSurface(densityFunction, densityFunction2, i, j);
	}
	...
	record FindTopSurface(DensityFunction density, DensityFunction upperBound, int lowerBound, int cellHeight) implements DensityFunction {
		...
		@Override
		public double compute(DensityFunction.FunctionContext functionContext) {
			int i = Mth.floor(this.upperBound.compute(functionContext) / this.cellHeight) * this.cellHeight;
			if (i <= this.lowerBound) {
				return this.lowerBound;
			} else {
				for (int j = i; j >= this.lowerBound; j -= this.cellHeight) {
					if (this.density.compute(new DensityFunction.SinglePointContext(functionContext.blockX(), j, functionContext.blockZ())) > 0.0) {
						return j;
					}
				}
	...
	protected record Mapped(DensityFunctions.Mapped.Type type, DensityFunction input, double minValue, double maxValue)
		...
		public static DensityFunctions.Mapped create(DensityFunctions.Mapped.Type type, DensityFunction densityFunction) {
			...
			double f = transform(type, d);
			double g = transform(type, e);
			if (type == DensityFunctions.Mapped.Type.INVERT) {
				return d < 0.0 && e > 0.0
					? new DensityFunctions.Mapped(type, densityFunction, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY)
					: new DensityFunctions.Mapped(type, densityFunction, g, f);
			} else {
				return type != DensityFunctions.Mapped.Type.ABS && type != DensityFunctions.Mapped.Type.SQUARE
					? new DensityFunctions.Mapped(type, densityFunction, f, g)
					: new DensityFunctions.Mapped(type, densityFunction, Math.max(0.0, d), Math.max(f, g));
			}
		}
	...
	interface TwoArgumentSimpleFunction extends DensityFunction {
		...
		static DensityFunctions.TwoArgumentSimpleFunction create(
			...
			if (type == DensityFunctions.TwoArgumentSimpleFunction.Type.MIN || type == DensityFunctions.TwoArgumentSimpleFunction.Type.MAX) {
				...
				boolean bl2 = e >= f;
				if (bl || bl2) {
					LOGGER.warn("Creating a {} function between two non-overlapping inputs: {} and {}", type, densityFunction, densityFunction2);
				}
			}
