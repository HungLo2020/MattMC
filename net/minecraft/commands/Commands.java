...
public class Commands {
	...
	private static final ClientboundCommandsPacket.NodeInspector<CommandSourceStack> COMMAND_NODE_INSPECTOR = new ClientboundCommandsPacket.NodeInspector<CommandSourceStack>() {
		...
		public ResourceLocation suggestionId(ArgumentCommandNode<CommandSourceStack, ?> argumentCommandNode) {
			SuggestionProvider<CommandSourceStack> suggestionProvider = argumentCommandNode.getCustomSuggestions();
		...
		public boolean isRestricted(CommandNode<CommandSourceStack> commandNode) {
			return commandNode.getRequirement() instanceof PermissionCheck<?> permissionCheck && permissionCheck.requiredLevel() > 0;
		}
	};
	...
	public void performPrefixedCommand(CommandSourceStack commandSourceStack, String string) {
		string = trimOptionalPrefix(string);
	...
	private static <S> void fillUsableCommands(CommandNode<S> commandNode, CommandNode<S> commandNode2, S object, Map<CommandNode<S>, CommandNode<S>> map) {
		for (CommandNode<S> commandNode3 : commandNode.getChildren()) {
			if (commandNode3.canUse(object)) {
				ArgumentBuilder<S, ?> argumentBuilder = commandNode3.createBuilder();
				if (argumentBuilder.getRedirect() != null) {
					argumentBuilder.redirect((CommandNode<S>)map.get(argumentBuilder.getRedirect()));
				}
				...
				if (!commandNode3.getChildren().isEmpty()) {
					fillUsableCommands(commandNode3, commandNode4, object, map);
				}
			}
		}
	}
