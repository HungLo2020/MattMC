...
public class SimpleSoundInstance extends AbstractSoundInstance {
	public SimpleSoundInstance(SoundEvent soundEvent, SoundSource soundSource, float f, float g, RandomSource randomSource, BlockPos blockPos) {
		this(soundEvent, soundSource, f, g, randomSource, blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5);
	}
	...
	public static SimpleSoundInstance forUI(Holder<SoundEvent> holder, float f) {
		return forUI((SoundEvent)holder.value(), f);
	}

	public static SimpleSoundInstance forUI(SoundEvent soundEvent, float f, float g) {
		return new SimpleSoundInstance(
			soundEvent.location(), SoundSource.UI, g, f, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0, 0.0, 0.0, true
		);
	}

	public static SimpleSoundInstance forMusic(SoundEvent soundEvent, float f) {
		return new SimpleSoundInstance(
			soundEvent.location(), SoundSource.MUSIC, f, 1.0F, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0, 0.0, 0.0, true
		);
	}
