...
public abstract class Particle {
	...
	protected void setSize(float f, float g) {
		if (f != this.bbWidth || g != this.bbHeight) {
			...
			double d = (aABB.minX + aABB.maxX - f) / 2.0;
			double e = (aABB.minZ + aABB.maxZ - f) / 2.0;
	...
	public void setPos(double d, double e, double f) {
		...
		this.setBoundingBox(new AABB(d - g, e, f - g, d + g, e + h, f + g));
	}
	...
	public record LifetimeAlpha(float startAlpha, float endAlpha, float startAtNormalizedAge, float endAtNormalizedAge) {
		...
		public float currentAlphaForAge(int i, int j, float f) {
			if (Mth.equal(this.startAlpha, this.endAlpha)) {
				...
				float g = Mth.inverseLerp((i + f) / j, this.startAtNormalizedAge, this.endAtNormalizedAge);
