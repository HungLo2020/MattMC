...
public class WardenModel extends EntityModel<WardenRenderState> {
	...
	public static LayerDefinition createTendrilsLayer() {
		return createBodyLayer().apply(meshDefinition -> {
			meshDefinition.getRoot().retainExactParts(Set.of("left_tendril", "right_tendril"));
	...
	public static LayerDefinition createHeartLayer() {
		return createBodyLayer().apply(meshDefinition -> {
			meshDefinition.getRoot().retainExactParts(Set.of("body"));
			return meshDefinition;
		});
	}
	...
	public static LayerDefinition createBioluminescentLayer() {
		return createBodyLayer().apply(meshDefinition -> {
			meshDefinition.getRoot().retainExactParts(Set.of("head", "left_arm", "right_arm", "left_leg", "right_leg"));
			return meshDefinition;
		});
	}
	...
	public static LayerDefinition createPulsatingSpotsLayer() {
		return createBodyLayer().apply(meshDefinition -> {
			meshDefinition.getRoot().retainExactParts(Set.of("body", "head", "left_arm", "right_arm", "left_leg", "right_leg"));
			return meshDefinition;
		});
	}
	...
	private void animateTendrils(WardenRenderState wardenRenderState, float f) {
		float g = wardenRenderState.tendrilAnimation * (float)(Math.cos(f * 2.25) * Math.PI * 0.1F);
