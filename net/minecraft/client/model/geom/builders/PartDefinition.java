...
public class PartDefinition {
	...
	public PartDefinition clearChild(String string) {
		PartDefinition partDefinition = (PartDefinition)this.children.get(string);
	...
	public void retainPartsAndChildren(Set<String> set) {
		for (Entry<String, PartDefinition> entry : this.children.entrySet()) {
			...
			if (!set.contains(entry.getKey())) {
				this.addOrReplaceChild((String)entry.getKey(), CubeListBuilder.create(), partDefinition.partPose).retainPartsAndChildren(set);
			}
		}
	}
	...
	public void retainExactParts(Set<String> set) {
		for (Entry<String, PartDefinition> entry : this.children.entrySet()) {
			...
			if (set.contains(entry.getKey())) {
				...
				this.addOrReplaceChild((String)entry.getKey(), CubeListBuilder.create(), partDefinition.partPose).retainExactParts(set);
			}
		}
	}
