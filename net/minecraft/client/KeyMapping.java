...
public class KeyMapping implements Comparable<KeyMapping> {
	...
	public int compareTo(KeyMapping keyMapping) {
		return this.category == keyMapping.category
			? I18n.get(this.name).compareTo(I18n.get(keyMapping.name))
			: Integer.compare(KeyMapping.Category.SORT_ORDER.indexOf(this.category), KeyMapping.Category.SORT_ORDER.indexOf(keyMapping.category));
	}
	...
	public record Category(ResourceLocation id) {
		...
		public static KeyMapping.Category register(ResourceLocation resourceLocation) {
			KeyMapping.Category category = new KeyMapping.Category(resourceLocation);
			if (SORT_ORDER.contains(category)) {
				throw new IllegalArgumentException(String.format(Locale.ROOT, "Category '%s' is already registered.", resourceLocation));
			} else {
