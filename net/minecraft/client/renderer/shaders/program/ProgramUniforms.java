...
import net.minecraft.client.renderer.shaders.uniform.*;
...
/**
 * Manages uniform binding for shader programs.
 * 
 * Based on IRIS's ProgramUniforms class
 * Reference: frnsrc/Iris-1.21.9/.../gl/program/ProgramUniforms.java
 */
public class ProgramUniforms {
	private static final Logger LOGGER = LoggerFactory.getLogger(ProgramUniforms.class);
	...
	public static class Builder implements DynamicLocationalUniformHolder {
		...
		public OptionalInt location(String name, UniformType type) {
			...
			if ((!locations.containsKey(id) && !uniformNames.containsKey(name))) {
				...
				LOGGER.warn("[" + this.name + "] Duplicate uniform: " + type.toString().toLowerCase() + " " + name);
		...
		public ProgramUniforms buildUniforms() {
			...
			for (int index = 0; index < activeUniforms; index++) {
				...
				if (provided != null && provided != expected) {
					...
					LOGGER.error("[" + this.name + "] Wrong uniform type for " + name + ": providing " + provided + " but the program expects " + expectedName + ". Disabling that uniform.");
