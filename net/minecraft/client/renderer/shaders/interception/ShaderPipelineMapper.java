 ...
 * Based on IRIS IrisPipelines.java exactly.
 */
public class ShaderPipelineMapper {
    private static final Logger LOGGER = LoggerFactory.getLogger("ShaderPipelineMapper");
    private static final Map<RenderPipeline, Function<ShaderRenderingPipeline, ShaderKey>> coreShaderMap = new Object2ObjectArrayMap<>();
    private static final Map<RenderPipeline, Function<ShaderRenderingPipeline, ShaderKey>> coreShaderMapShadow = new Object2ObjectArrayMap<>();
    private static final Function<ShaderRenderingPipeline, ShaderKey> FAKE_FUNCTION = p -> null;

    static {
        // Main pass mappings (84 mappings)
        ...
        // Shadow pass mappings (48 mappings)
    ...
    private static ShaderKey getText(ShaderRenderingPipeline pipeline) {
    ...
    private static ShaderKey getTextIntensity(ShaderRenderingPipeline pipeline) {
        if (ShaderOverrides.isBlockEntities(pipeline)) {
    ...
    private static void assignToMain(RenderPipeline pipeline, Function<ShaderRenderingPipeline, ShaderKey> o) {
        if (coreShaderMap.containsKey(pipeline)) {
            Function<ShaderRenderingPipeline, ShaderKey> current = coreShaderMap.get(pipeline);
            ShaderKey currentKey = current.apply(null);
            ShaderKey newKey = o.apply(null);
            if (currentKey != newKey) {
                LOGGER.warn("Pair already assigned: " + pipeline + " to " + currentKey + " -> " + newKey);
            }
        }
    ...
    private static void assignToShadow(RenderPipeline pipeline, Function<ShaderRenderingPipeline, ShaderKey> o) {
        if (coreShaderMapShadow.containsKey(pipeline)) {
            LOGGER.warn("Pair already assigned: " + pipeline);
        }

        coreShaderMapShadow.put(pipeline, o);
    }

    private static ShaderKey getCutout(ShaderRenderingPipeline pipeline) {
        // Hand renderer check would go here (future implementation)
        if (ShaderOverrides.isBlockEntities(pipeline)) {
    ...
    private static ShaderKey getSolid(ShaderRenderingPipeline pipeline) {
        // Hand renderer check would go here (future implementation)
        if (ShaderOverrides.isBlockEntities(pipeline)) {
    ...
    private static ShaderKey getTranslucent(ShaderRenderingPipeline pipeline) {
        // Hand renderer check would go here (future implementation)
        if (ShaderOverrides.isBlockEntities(pipeline)) {
            return ShaderKey.BLOCK_ENTITY;
        } else {
    ...
    @Nullable
    public static ShaderKey getPipeline(ShaderRenderingPipeline pipeline, RenderPipeline shader) {
