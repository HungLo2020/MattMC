...
/**
 * Interface combining LocationalUniformHolder and DynamicUniformHolder.
 * Provides location-aware dynamic uniform registration.
 * 
 * Based on IRIS's DynamicLocationalUniformHolder interface
 * Reference: frnsrc/Iris-1.21.9/.../gl/uniform/DynamicLocationalUniformHolder.java
 */
public interface DynamicLocationalUniformHolder extends LocationalUniformHolder, DynamicUniformHolder {
	...
	default DynamicUniformHolder uniformMatrix3(String name, Supplier<Matrix3fc> value, ValueUpdateNotifier notifier) {
		location(name, UniformType.MAT3).ifPresent(id -> addDynamicUniform(new Matrix3Uniform(id, value, notifier), notifier));
