...
/**
 * Iris-exclusive uniforms that don't exist in OptiFine or ShadersMod.
 * 
 * Based on IRIS's IrisExclusiveUniforms.java (simplified for MattMC)
 * Reference: frnsrc/Iris-1.21.9/.../uniforms/IrisExclusiveUniforms.java
 * 
 * Step 27 of NEW-SHADER-PLAN.md
 * 
 * Uniforms provided (~20 uniforms):
 * - Player stats: health, hunger, armor, air
 * - World info: bedrockLevel, heightLimit, cloudHeight, seaLevel
 * - Effects: thunderStrength, heavyFog, endFlashIntensity
 * - Camera: firstPersonCamera, isSpectator, eyePosition, playerLookVector
 * - Selection: currentSelectedBlockId, currentSelectedBlockPos
 * - Lightning: lightningBoltPosition
 */
public class IrisExclusiveUniforms {
	...
	public static void addIrisExclusiveUniforms(UniformHolder uniforms, FrameUpdateNotifier updateNotifier) {
		...
		uniforms.uniform1f(PER_TICK, "currentPlayerHealth", IrisExclusiveUniforms::getCurrentHealth);
		uniforms.uniform1b(PER_TICK, "heavyFog", IrisExclusiveUniforms::isHeavyFog);
		uniforms.uniform1f(PER_TICK, "maxPlayerHealth", IrisExclusiveUniforms::getMaxHealth);
		uniforms.uniform1f(PER_TICK, "currentPlayerHunger", IrisExclusiveUniforms::getCurrentHunger);
		uniforms.uniform1f(PER_TICK, "maxPlayerHunger", () -> 20);
		uniforms.uniform1f(PER_TICK, "endFlashIntensity", endFlashStorage::getCurrentEndFlash);
		uniforms.uniform1f(PER_TICK, "previousEndFlashIntensity", endFlashStorage::getLastEndFlash);
		uniforms.uniform1f(PER_TICK, "currentPlayerArmor", IrisExclusiveUniforms::getCurrentArmor);
		uniforms.uniform1f(PER_TICK, "maxPlayerArmor", () -> 50);
		uniforms.uniform1f(PER_TICK, "currentPlayerAir", IrisExclusiveUniforms::getCurrentAir);
		uniforms.uniform1f(PER_TICK, "maxPlayerAir", IrisExclusiveUniforms::getMaxAir);
		uniforms.uniform1b(PER_FRAME, "firstPersonCamera", IrisExclusiveUniforms::isFirstPersonCamera);
		uniforms.uniform1b(PER_TICK, "isSpectator", IrisExclusiveUniforms::isSpectator);
		...
		uniforms.uniform1f(PER_TICK, "cloudTime", CapturedRenderingState.INSTANCE::getCloudTime);
		uniforms.uniform3d(PER_FRAME, "relativeEyePosition", () -> CameraUniforms.getUnshiftedCameraPosition().sub(getEyePosition(), new Vector3d()));
		uniforms.uniform3d(PER_FRAME, "playerLookVector", () -> {
			if (Minecraft.getInstance().getCameraEntity() instanceof LivingEntity livingEntity) {
				Vec3 lookVec = livingEntity.getViewVector(CapturedRenderingState.INSTANCE.getTickDelta());
				return new Vector3d(lookVec.x, lookVec.y, lookVec.z);
			} else {
				return ZERO;
			}
		});
		uniforms.uniform3d(PER_FRAME, "playerBodyVector", () -> {
			Vec3 forward = Minecraft.getInstance().getCameraEntity().getForward();
			return new Vector3d(forward.x, forward.y, forward.z);
		});
		Vector4f zero = new Vector4f(0, 0, 0, 0);
		uniforms.uniform4f(PER_TICK, "lightningBoltPosition", () -> {
			if (Minecraft.getInstance().level != null) {
				return StreamSupport.stream(Minecraft.getInstance().level.entitiesForRendering().spliterator(), false)
					...
					.map(bolt -> {
						...
						Vec3 vec3 = bolt.getPosition(Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaTicks());
	...
	private static int getCurrentSelectedBlockId() {
		...
		if (Minecraft.getInstance().level != null && hitResult != null && hitResult.getType() == HitResult.Type.BLOCK) {
			...
			if (!blockState.isAir() && Minecraft.getInstance().level.getWorldBorder().isWithinBounds(blockPos4)) {
				// Note: IRIS uses WorldRenderingSettings.INSTANCE.getBlockStateIds().getInt(blockState)
				// For MattMC, return a simplified ID (block registry ID)
				return 1; // Placeholder - would need proper block ID mapping
			}
		}
