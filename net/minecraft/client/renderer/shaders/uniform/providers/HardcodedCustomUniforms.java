...
import net.minecraft.client.renderer.shaders.uniform.transforms.SmoothedFloat;
...
/**
 * Hardcoded custom uniforms for various popular shaderpacks (BSL, Complementary, Sildur's, SEUS, etc.)
 * 
 * COPIED VERBATIM from IRIS's HardcodedCustomUniforms.java
 * Reference: frnsrc/Iris-1.21.9/.../uniforms/HardcodedCustomUniforms.java
 * 
...
public class HardcodedCustomUniforms {
	...
	private static float getBurnFactor() {
		return Minecraft.getInstance().player != null && Minecraft.getInstance().player.isOnFire() ? 1.0f : 0f;
	}

	private static float getSneakFactor() {
		return Minecraft.getInstance().player != null && Minecraft.getInstance().player.isCrouching() ? 1.0f : 0f;
	}

	private static float getHurtFactor() {
		Player player = Minecraft.getInstance().player;
		return player != null && (player.hurtTime > 0 || player.deathTime > 0) ? 0.4f : 0f;
	}

	private static float getEyeInCave() {
		if (client.getCameraEntity() != null && client.getCameraEntity().getEyeY() < 5.0) {
	...
	private static float getRawPrecipitation() {
		...
		if (Minecraft.getInstance().getCameraEntity() == null || Minecraft.getInstance().level == null) {
			return 0;
		}
