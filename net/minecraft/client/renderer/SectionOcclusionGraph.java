			...
			for (Direction direction : DIRECTIONS) {
				...
				if (renderSection2 != null && (!bl || !node.hasDirection(direction.getOpposite()))) {
					...
					if (bl && bl2) {
						...
						boolean bl4 = direction.getAxis() == Axis.X ? blockPos.getX() > j : blockPos.getX() < j;
						boolean bl5 = direction.getAxis() == Axis.Y ? blockPos.getY() > k : blockPos.getY() < k;
						boolean bl6 = direction.getAxis() == Axis.Z ? blockPos.getZ() > ix : blockPos.getZ() < ix;
