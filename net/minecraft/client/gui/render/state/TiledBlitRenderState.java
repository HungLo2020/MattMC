	...
	public void buildVertices(VertexConsumer vertexConsumer) {
		...
		for (int k = 0; k < i; k += this.tileWidth()) {
			...
			for (int n = 0; n < j; n += this.tileHeight()) {
				...
				int s = this.y0() + n;
				int t = this.y0() + n + p;
				vertexConsumer.addVertexWith2DPose(this.pose(), q, s).setUv(this.u0(), this.v0()).setColor(this.color());
				vertexConsumer.addVertexWith2DPose(this.pose(), q, t).setUv(this.u0(), g).setColor(this.color());
				vertexConsumer.addVertexWith2DPose(this.pose(), r, t).setUv(f, g).setColor(this.color());
				vertexConsumer.addVertexWith2DPose(this.pose(), r, s).setUv(f, this.v0()).setColor(this.color());
			}
		}
	}
