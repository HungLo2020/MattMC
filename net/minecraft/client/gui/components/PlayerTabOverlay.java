...
public class PlayerTabOverlay {
	...
	protected void renderPingIcon(GuiGraphics guiGraphics, int i, int j, int k, PlayerInfo playerInfo) {
		...
		guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocation, j + i - 11, k, 10, 8);
	}
	...
	private void renderTablistHearts(int i, int j, int k, UUID uUID, GuiGraphics guiGraphics, int l) {
		...
		if (m > 0) {
			int o = Mth.floor(Math.min((float)(k - j - 4) / n, 9.0F));
			if (o <= 3) {
				float f = Mth.clamp(l / 20.0F, 0.0F, 1.0F);
				int p = (int)((1.0F - f) * 255.0F) << 16 | (int)(f * 255.0F) << 8;
				float g = l / 2.0F;
				Component component = Component.translatable("multiplayer.player.list.hp", new Object[]{g});
				...
				guiGraphics.drawString(this.minecraft.font, component2, (k + j - this.minecraft.font.width(component2)) / 2, i, ARGB.opaque(p));
			} else {
				ResourceLocation resourceLocation = bl ? HEART_CONTAINER_BLINKING_SPRITE : HEART_CONTAINER_SPRITE;

				for (int p = m; p < n; p++) {
					guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocation, j + p * o, i, 9, 9);
				}
