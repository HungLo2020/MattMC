...
public class BossHealthOverlay {
	...
	private void drawBar(
		...
		guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocations[bossEvent.getColor().ordinal()], 182, 5, 0, 0, i, j, k, 5);
		if (bossEvent.getOverlay() != BossBarOverlay.PROGRESS) {
			guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocations2[bossEvent.getOverlay().ordinal() - 1], 182, 5, 0, 0, i, j, k, 5);
		}
	}

	public void update(ClientboundBossEventPacket clientboundBossEventPacket) {
		clientboundBossEventPacket.dispatch(new Handler() {
			public void add(UUID uUID, Component component, float f, BossBarColor bossBarColor, BossBarOverlay bossBarOverlay, boolean bl, boolean bl2, boolean bl3) {
			...
			public void updateStyle(UUID uUID, BossBarColor bossBarColor, BossBarOverlay bossBarOverlay) {
