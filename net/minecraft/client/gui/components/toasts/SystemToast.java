...
public class SystemToast implements Toast {
	...
	public void update(ToastManager toastManager, long l) {
		...
		double d = this.id.displayTime * toastManager.getNotificationDisplayTimeMultiplier();
		long m = l - this.lastChanged;
		this.wantedVisibility = !this.forceHide && m < d ? Toast.Visibility.SHOW : Toast.Visibility.HIDE;
	}
	...
	public void render(GuiGraphics guiGraphics, Font font, long l) {
		...
		if (this.messageLines.isEmpty()) {
			...
			for (int i = 0; i < this.messageLines.size(); i++) {
				guiGraphics.drawString(font, (FormattedCharSequence)this.messageLines.get(i), 18, 18 + i * 12, -1, false);
			}
		}
	}
	...
	public static void onFileDropFailure(Minecraft minecraft, int i) {
		add(
			...
			Component.translatable("gui.fileDropFailure.detail", new Object[]{i})
		);
	}
	...
	public static void onChunkLoadFailure(Minecraft minecraft, ChunkPos chunkPos) {
		addOrUpdate(
			...
			Component.translatable("chunk.toast.loadFailure", new Object[]{Component.translationArg(chunkPos)}).withStyle(ChatFormatting.RED),
	...
	public static void onChunkSaveFailure(Minecraft minecraft, ChunkPos chunkPos) {
		addOrUpdate(
			...
			Component.translatable("chunk.toast.saveFailure", new Object[]{Component.translationArg(chunkPos)}).withStyle(ChatFormatting.RED),
			Component.translatable("chunk.toast.checkLog")
		);
	}
