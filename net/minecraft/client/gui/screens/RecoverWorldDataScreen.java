...
public class RecoverWorldDataScreen extends Screen {
	...
	private final LevelStorageAccess storageAccess;

	public RecoverWorldDataScreen(Minecraft minecraft, BooleanConsumer booleanConsumer, LevelStorageAccess levelStorageAccess) {
		super(TITLE);
		this.callback = booleanConsumer;
		this.message = Component.translatable(
			"recover_world.message", new Object[]{Component.literal(levelStorageAccess.getLevelId()).withStyle(ChatFormatting.GRAY)}
		);
	...
	private Component buildInfo(LevelStorageAccess levelStorageAccess, boolean bl, @Nullable Exception exception) {
		if (bl && exception instanceof FileNotFoundException) {
			...
			mutableComponent.append(Component.translatable("recover_world.state_entry", new Object[]{mutableComponent2.withStyle(ChatFormatting.GRAY)}));
			if (exception == null) {
	...
	@Nullable
	private Exception collectIssue(LevelStorageAccess levelStorageAccess, boolean bl) {
		try {
			if (!bl) {
