...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
	...
	public void send(Packet<?> packet, @Nullable ChannelFutureListener channelFutureListener, boolean bl) {
		if (this.isConnected()) {
			...
			this.pendingActions.add((Consumer<Connection>)(connection -> connection.sendPacket(packet, channelFutureListener, bl)));
		}
	}
	...
	private static ChannelOutboundHandler createFrameEncoder(boolean bl) {
		return (ChannelOutboundHandler)(bl ? new LocalFrameEncoder() : new Varint21LengthFieldPrepender());
	}
