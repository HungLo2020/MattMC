...
public class ItemStackComponentizationFix extends DataFix {
	...
	private static Dynamic<?> fixDisplay(ItemStackComponentizationFix.ItemStackData itemStackData, Dynamic<?> dynamic, int i) {
		dynamic.get("Name")
			...
			.ifPresent(dynamicx -> itemStackData.setComponent("minecraft:custom_name", dynamicx));
		OptionalDynamic<?> optionalDynamic = dynamic.get("Lore");
		if (optionalDynamic.result().isPresent()) {
			itemStackData.setComponent("minecraft:lore", dynamic.createList(dynamic.get("Lore").asStream().filter(LegacyComponentDataFixUtils::isStrictlyValidJson)));
		}
		...
		if (optional2.isPresent()) {
			itemStackData.setComponent("minecraft:item_name", LegacyComponentDataFixUtils.createTranslatableComponent(dynamic.getOps(), (String)optional2.get()));
		}
	...
	private static <T> Dynamic<T> fixBlockEntityTag(ItemStackComponentizationFix.ItemStackData itemStackData, Dynamic<T> dynamic, String string) {
		...
		return switch (string) {
			...
			case "minecraft:banner" -> {
				...
				if (optional2.isPresent()) {
					itemStackData.setComponent("minecraft:base_color", dynamic.createString(ExtraDataFixUtils.dyeColorIdToName(((Number)optional2.get()).intValue())));
				}
	...
	private static void fixEnchantments(ItemStackComponentizationFix.ItemStackData itemStackData, Dynamic<?> dynamic, String string, String string2, boolean bl) {
		...
		List<Pair<String, Integer>> list = optionalDynamic.asList(Function.identity())
			.stream()
			.flatMap(dynamicx -> parseEnchantment(dynamicx).stream())
			.filter(pairx -> (Integer)pairx.getSecond() > 0)
			.toList();
		if (!list.isEmpty() || bl) {
			Dynamic<?> dynamic2 = dynamic.emptyMap();
				...
				.set(
					...
					dynamic.createList(
						((Dynamic)optional.get())
							.asStream()
							.map((Dynamic<?> dynamicx) -> DataFixUtils.orElse(dynamicx.asString().map(stringx -> fixBlockStatePredicate(dynamicx, stringx)).result(), dynamicx))
					)
				);
			if (bl) {
				dynamic2 = dynamic2.set("show_in_tooltip", dynamic.createBoolean(false));
