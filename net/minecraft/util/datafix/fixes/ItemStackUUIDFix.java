...
public class ItemStackUUIDFix extends AbstractUUIDFix {
	...
	private Dynamic<?> updateAttributeModifiers(Dynamic<?> dynamic) {
		return dynamic.update(
			"AttributeModifiers",
			dynamic2 -> dynamic.createList(dynamic2.asStream().map((Dynamic<?> dynamicxx) -> (Dynamic<?>)replaceUUIDLeastMost(dynamicxx, "UUID", "UUID").orElse(dynamicxx)))
		);
	}

	private Dynamic<?> updateSkullOwner(Dynamic<?> dynamic) {
		return dynamic.update("SkullOwner", (Dynamic<?> dynamicx) -> (Dynamic<?>)replaceUUIDString(dynamicx, "Id", "Id").orElse(dynamicx));
	}
}
