...
public class Mth {
	...
	public static double absMax(double d, double e) {
		return Math.max(Math.abs(d), Math.abs(e));
	}
	...
	public static IntStream outFromOrigin(int i, int j, int k, int l) {
		if (j > k) {
			...
			throw new IllegalArgumentException(String.format(Locale.ROOT, "step size expected to be >= 1, was %d", l));
		} else {
			int m = clamp(i, j, k);
			return IntStream.iterate(m, lx -> {
				int mx = Math.abs(m - lx);
				return m - mx >= j || m + mx <= k;
			}, mx -> {
				boolean bl = mx <= m;
				int n = Math.abs(m - mx);
				boolean bl2 = m + n + l <= k;
