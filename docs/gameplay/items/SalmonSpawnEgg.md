# Salmon Spawn Egg

## Obtaining

The Salmon Spawn Egg is available from the Creative Menu.

## Usage

Use the Salmon Spawn Egg on a block to spawn a Salmon.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:salmon_spawn_egg`.
