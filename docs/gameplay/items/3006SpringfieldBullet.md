# .30-06 Springfield Bullet

## Obtaining

.30-06 Springfield Bullet can be obtained from the Creative Menu. It is registered as `minecraft:30_06`.

## Usage

.30-06 Springfield Bullet is ammunition for TaCZ firearms. It is consumed when compatible guns fire in survival unless the player has infinite materials.

## Properties

| Property | Value |
| --- | --- |
| Stack size | 36 |
| Used by | [.30-06 Lonetrail Hand Cannon](3006LonetrailHandCannon.md), [M700 Sniper Rifle](M700SniperRifle.md) |

## Behavior

Ammo is counted from the player inventory when reloading compatible firearms. The gun HUD shows both magazine ammo and reserve ammo.

## Notes

* This item is part of the integrated TaCZ firearms system.
