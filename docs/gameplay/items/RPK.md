...
Lightweight, but power-up.

## Obtaining

RPK can be obtained from the Creative Menu. It is registered as `minecraft:rpk`.

## Usage

RPK is a TaCZ firearm. It uses [7.62x39mm Bullet](762x39mmBullet.md) and fires with the TaCZ gun controls. Reload it with the reload key and aim down sights with the aim key.
