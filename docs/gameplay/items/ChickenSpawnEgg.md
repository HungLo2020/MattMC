# Chicken Spawn Egg

## Obtaining

The Chicken Spawn Egg is available from the Creative Menu.

## Usage

Use the Chicken Spawn Egg on a block to spawn a Chicken.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:chicken_spawn_egg`.
