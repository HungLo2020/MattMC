# Hoglin Spawn Egg

## Obtaining

The Hoglin Spawn Egg is available from the Creative Menu.

## Usage

Use the Hoglin Spawn Egg on a block to spawn a Hoglin.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:hoglin_spawn_egg`.
