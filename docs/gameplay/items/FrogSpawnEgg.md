# Frog Spawn Egg

## Obtaining

The Frog Spawn Egg is available from the Creative Menu.

## Usage

Use the Frog Spawn Egg on a block to spawn a Frog.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:frog_spawn_egg`.
