# Cat Spawn Egg

## Obtaining

The Cat Spawn Egg is available from the Creative Menu.

## Usage

Use the Cat Spawn Egg on a block to spawn a Cat.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:cat_spawn_egg`.
