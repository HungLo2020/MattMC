# Cow Spawn Egg

## Obtaining

The Cow Spawn Egg is available from the Creative Menu.

## Usage

Use the Cow Spawn Egg on a block to spawn a Cow.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:cow_spawn_egg`.
