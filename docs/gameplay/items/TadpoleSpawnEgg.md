# Tadpole Spawn Egg

## Obtaining

The Tadpole Spawn Egg is available from the Creative Menu.

## Usage

Use the Tadpole Spawn Egg on a block to spawn a Tadpole.

## Behavior

Spawn eggs are creative utility items. They do not have durability and are primarily used for testing, mapmaking, creative building, and quickly placing mobs into the world.

## Notes

* This item is registered as `minecraft:tadpole_spawn_egg`.
