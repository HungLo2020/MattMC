...
public class BlockFaceCollectorTest {
    ...
    /**
     * Test that face brightness values match Minecraft's standard directional shading.
     * This ensures consistent lighting across all block faces.
     */
    @Test
    public void testFaceBrightnessValues() {
        LevelChunk chunk = new LevelChunk(0, 0);
        chunk.setBlock(8, 64, 8, Blocks.STONE);
        ...
        // Verify standard Minecraft directional shading values
        // Top: 1.0 (full brightness)
        assertFalse(collector.getTopFaces().isEmpty(), "Top face should be visible");
        if (!collector.getTopFaces().isEmpty()) {
            BlockFaceCollector.FaceData topFace = collector.getTopFaces().get(0);
            assertEquals(1.0f, topFace.colorBrightness, 0.001f, 
                "Top face should have 1.0 brightness");
        }
        
        // Bottom: 0.5 (darkest)
        assertFalse(collector.getBottomFaces().isEmpty(), "Bottom face should be visible");
        if (!collector.getBottomFaces().isEmpty()) {
            BlockFaceCollector.FaceData bottomFace = collector.getBottomFaces().get(0);
            assertEquals(0.5f, bottomFace.colorBrightness, 0.001f, 
                "Bottom face should have 0.5 brightness");
        }
        
        // North: 0.8 (medium brightness)
        assertFalse(collector.getNorthFaces().isEmpty(), "North face should be visible");
        if (!collector.getNorthFaces().isEmpty()) {
            BlockFaceCollector.FaceData northFace = collector.getNorthFaces().get(0);
            assertEquals(0.8f, northFace.colorBrightness, 0.001f, 
                "North face should have 0.8 brightness");
        }
        
        // South: 0.8 (medium brightness)
        assertFalse(collector.getSouthFaces().isEmpty(), "South face should be visible");
        if (!collector.getSouthFaces().isEmpty()) {
            BlockFaceCollector.FaceData southFace = collector.getSouthFaces().get(0);
            assertEquals(0.8f, southFace.colorBrightness, 0.001f, 
                "South face should have 0.8 brightness");
        }
        
        // West: 0.6 (darker)
        assertFalse(collector.getWestFaces().isEmpty(), "West face should be visible");
        if (!collector.getWestFaces().isEmpty()) {
            BlockFaceCollector.FaceData westFace = collector.getWestFaces().get(0);
            assertEquals(0.6f, westFace.colorBrightness, 0.001f, 
                "West face should have 0.6 brightness");
        }
        
        // East: 0.8 (medium brightness) - THIS IS THE FIX
        assertFalse(collector.getEastFaces().isEmpty(), "East face should be visible");
        if (!collector.getEastFaces().isEmpty()) {
            BlockFaceCollector.FaceData eastFace = collector.getEastFaces().get(0);
            assertEquals(0.8f, eastFace.colorBrightness, 0.001f, 
                "East face should have 0.8 brightness (same as North/South)");
        }
    }
}
