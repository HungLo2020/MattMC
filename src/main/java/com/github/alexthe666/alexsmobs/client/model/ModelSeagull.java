...
import com.github.alexthe666.alexsmobs.client.render.SeagullRenderState;
...
public class ModelSeagull extends AdvancedEntityModel<SeagullRenderState> {
	...
	@Override
	public void setupAnim(SeagullRenderState state){
		...
		float flapAmount = state.flapAmount * flyProgress * 0.2F;
		...
		if(flyProgress > 0) {
			...
			this.flap(tail, walkSpeed, walkDegree * 0.5F, false, 1F, 0F, state.walkAnimationPos, state.walkAnimationSpeed);
		}
		...
		head.rotateAngleY += Math.toRadians(state.flightLookYaw) * flyProgress * 0.2F;
		head.rotateAngleY += state.yRot / 57.295776F * groundProgress * 0.2F;
		head.rotateAngleX += state.xRot / 57.295776F * groundProgress * 0.2F;
