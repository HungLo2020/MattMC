...
import com.github.alexthe666.alexsmobs.client.render.CosmawRenderState;
...
public class ModelCosmaw extends AdvancedEntityModel<CosmawRenderState> {
    ...
    @Override
    public void setupAnim(CosmawRenderState state) {
        this.resetToDefaultPose();
        float walkSpeed = 0.7F;
        float walkDegree = 0.4F;
        float idleSpeed = 0.1F;
        float idleDegree = 0.2F;
        float clutchProgress = state.clutchProgress;
        float biteProgress = state.biteProgress;
        float openProgress = Math.max(Math.max(state.openProgress, clutchProgress), biteProgress);
        float cosmawPitch = (float) (Math.toRadians(state.cosmawPitch) * (5F - clutchProgress) * 0.2F);
        float cosmawPitchPos = (float) ((state.cosmawPitch / 90F) * (5F - clutchProgress) * 0.2F);
