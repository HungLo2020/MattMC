...
import com.github.alexthe666.alexsmobs.client.render.ToucanRenderState;
...
public class ModelToucan extends AdvancedEntityModel<ToucanRenderState> {
    ...
    public void setupAnim(ToucanRenderState state) {
        ...
        float runProgress = Math.max(0, (state.walkAnimationSpeed * 5F) - flyProgress);
        float biteProgress = state.peckProgress;
        ...
        if(flyProgress > 0) {
            this.flap(right_wing, flapSpeed, flapDegree * 5, true, 0F, 0F, state.ageInTicks, 1);
            this.flap(left_wing, flapSpeed, flapDegree * 5, false, 0F, 0F, state.ageInTicks, 1);
            this.swing(right_wingtip, flapSpeed, flapDegree * 0.5F, false, 0F, 0F, state.ageInTicks, 1);
            this.swing(left_wingtip, flapSpeed, flapDegree * 0.5F, true, 0F, 0F, state.ageInTicks, 1);
            this.bob(body, flapSpeed * 0.5F, flapDegree * 12, true, state.ageInTicks, 1);
            this.walk(head, flapSpeed, flapDegree * 0.2F, true, 2F, -0.1F, state.ageInTicks, 1);
        }else {
            this.walk(head, idleSpeed * 0.7F, idleDegree, false, 1F, 0.05F, state.ageInTicks, 1);
            this.walk(tail, idleSpeed * 0.7F, idleDegree, false, -1F, 0.05F, state.ageInTicks, 1);
            this.bob(body, walkSpeed * 1F, walkDegree * 1.3F, true, state.walkAnimationPos, state.walkAnimationSpeed);
            this.walk(right_leg, walkSpeed, walkDegree * 1.85F, false, 0F, 0.2F, state.walkAnimationPos, state.walkAnimationSpeed);
            this.walk(left_leg, walkSpeed, walkDegree * 1.85F, true, 0F, 0.2F, state.walkAnimationPos, state.walkAnimationSpeed);
            this.walk(head, walkSpeed, walkDegree * 0.4F, false, 2F, -0.01F, state.walkAnimationPos, state.walkAnimationSpeed);
            this.swing(tail, walkSpeed, walkDegree * 0.5F, false, 1F, 0F, state.walkAnimationPos, state.walkAnimationSpeed);
        }
        this.faceTarget(state.yRot, state.xRot, 1, head);
    }
