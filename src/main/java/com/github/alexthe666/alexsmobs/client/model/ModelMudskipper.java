...
import com.github.alexthe666.alexsmobs.client.render.state.MudskipperRenderState;
...
public class ModelMudskipper extends AdvancedEntityModel<MudskipperRenderState> {
    ...
    @Override
    public void setupAnim(MudskipperRenderState renderState) {
        this.resetToDefaultPose();
        float blinkAmount = Math.max(0, ((float)Math.sin(renderState.ageInTicks * 0.1F) - 0.5F) * 2F);
        float displayProgress = renderState.displayProgress;
        float swimProgress = renderState.swimProgress;
        float sitProgress = renderState.sitProgress;
        float mudProgress = renderState.mudProgress;
        ...
        progressPositionPrev(head, mudProgress, 0, -2F, 0F, 1f);
