...
public class ModelMimicube extends AdvancedEntityModel<MimicubeRenderState> {
	...
	public void setupAnim(MimicubeRenderState renderState) {
		...
		float squishFactor = renderState.squishFactor;
		
		this.bob(innerbody, idleDegree, idleSpeed, false, ageInTicks, 1.0F);
		this.flap(innerbody, idleSpeed * 1.3F, idleDegree * 0.05F, false, 2F, 0F, ageInTicks, 1);
		
		float lvt_7_1_ = 1.0F / (squishFactor + 1.0F);
		float squishScale = 1.0F / lvt_7_1_;
		this.innerbody.rotationPointY += squishFactor * -5F;
