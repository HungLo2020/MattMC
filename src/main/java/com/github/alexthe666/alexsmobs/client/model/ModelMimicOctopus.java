...
import com.github.alexthe666.alexsmobs.client.render.MimicOctopusRenderState;
...
public class ModelMimicOctopus extends AdvancedEntityModel<MimicOctopusRenderState> {
    ...
    @Override
    public void setupAnim(MimicOctopusRenderState state, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.resetToDefaultPose();
        float transProgress = state.transProgress;
        float groundProgress = state.groundProgress;
        ...
        if(state.prevMimicState != null) {
            ...
            animateForMimicGround(state.prevMimicState, state, limbSwing, limbSwingAmount, ageInTicks,  progress * groundProgressNorm);
            if(sitProgress == 0){
                animateForMimicWater(state.prevMimicState, state, limbSwing, limbSwingAmount, ageInTicks, progress * (1 - groundProgressNorm));
            }
        }
        animateForMimicGround(state.mimicState, state, limbSwing, limbSwingAmount, ageInTicks, notSitProgress * transProgress * groundProgressNorm);
        animateForMimicWater(state.mimicState, state, limbSwing, limbSwingAmount, ageInTicks, notSitProgress * transProgress * (1 - groundProgressNorm));
    ...
    public void animateForMimicWater(EntityMimicOctopus.MimicState state, MimicOctopusRenderState renderState, float limbSwing, float limbSwingAmount, float ageInTicks, float swimProgress) {
    ...
    public void animateForMimicGround(EntityMimicOctopus.MimicState state, MimicOctopusRenderState renderState, float limbSwing, float limbSwingAmount, float ageInTicks, float groundProgress){
        ...
        if(state == EntityMimicOctopus.MimicState.CREEPER){
            ...
            if(renderState.hasGuardianLaser){
                progressRotationPrev(body, groundProgress, 0, Maths.rad(180), 0, 5F);
            }
        }
