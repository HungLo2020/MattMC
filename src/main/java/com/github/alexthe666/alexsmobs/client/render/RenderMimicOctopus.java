...
public class RenderMimicOctopus extends MobRenderer<EntityMimicOctopus, MimicOctopusRenderState, ModelMimicOctopus> {
    ...
    @Override
    public void extractRenderState(EntityMimicOctopus entity, MimicOctopusRenderState state, float partialTick) {
        super.extractRenderState(entity, state, partialTick);
        state.transProgress = entity.prevTransProgress + (entity.transProgress - entity.prevTransProgress) * partialTick;
        state.prevTransProgress = entity.prevTransProgress;
        state.colorShiftProgress = entity.prevColorShiftProgress + (entity.colorShiftProgress - entity.prevColorShiftProgress) * partialTick;
        state.prevColorShiftProgress = entity.prevColorShiftProgress;
        state.groundProgress = entity.prevGroundProgress + (entity.groundProgress - entity.prevGroundProgress) * partialTick;
        state.prevGroundProgress = entity.prevGroundProgress;
        state.sitProgress = entity.prevSitProgress + (entity.sitProgress - entity.prevSitProgress) * partialTick;
        state.prevSitProgress = entity.prevSitProgress;
        state.mimicState = entity.getMimicState();
        state.prevMimicState = entity.getPrevMimicState();
        state.mimickedBlock = entity.getMimickedBlock();
        state.prevMimickedBlock = entity.getPrevMimickedBlock();
        state.hasGuardianLaser = entity.hasGuardianLaser();
        state.guardianLaserId = entity.getGuardianLaserId();
    ...
    public void render(MimicOctopusRenderState state, PoseStack matrixStackIn,
            MultiBufferSource bufferIn, int packedLightIn) {
        EntityMimicOctopus entityIn = (EntityMimicOctopus) this.entityRenderDispatcher.level.getEntity(state.guardianLaserId);
        LivingEntity livingentity = entityIn != null ? entityIn.getGuardianLaser() : null;
        if (livingentity != null && entityIn != null) {
            float f = entityIn.getLaserAttackAnimationScale(0);
            float f1 = (float) entityIn.level().getGameTime();
            float f2 = f1 * 0.5F % 1.0F;
            float f3 = entityIn.getEyeHeight();
            matrixStackIn.pushPose();
            matrixStackIn.translate(0.0D, f3, 0.0D);
            Vec3 vector3d = this.getPosition(livingentity, (double) livingentity.getBbHeight() * 0.5D, 0);
            Vec3 vector3d1 = this.getPosition(entityIn, f3, 0);
    ...
    protected void scale(MimicOctopusRenderState state, PoseStack matrixStackIn) {
        matrixStackIn.translate(0, -0.02F, 0);
        matrixStackIn.scale(0.9F * state.scale, 0.9F * state.scale, 0.9F * state.scale);
    }
    ...
    public ResourceLocation getTextureLocation(MimicOctopusRenderState state) {
        return TEXTURE;
    }
    ...
    static class OverlayLayer extends RenderLayer<MimicOctopusRenderState, ModelMimicOctopus> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource buffer, int packedLightIn,
                MimicOctopusRenderState state, float limbSwing, float limbSwingAmount) {
            float transProgress = state.transProgress;
            float colorProgress = state.colorShiftProgress * 0.2F;
            float r = 1F;
            float g = 1F;
            float b = 1F;
            float a = 1F;
            ...
            float finB = 1.0F;
            float finA = 1.0F;
            if (state.prevMimicState == EntityMimicOctopus.MimicState.OVERLAY) {
                if (state.prevMimickedBlock != null) {
                    int j = OctopusColorRegistry.getBlockColor(state.prevMimickedBlock);
            ...
            if ((state.mimicState == EntityMimicOctopus.MimicState.OVERLAY)) {
                if (state.mimickedBlock != null) {
                    int i = OctopusColorRegistry.getBlockColor(state.mimickedBlock);
            ...
            if (a == 1.0F) {
                a *= 0.9F + 0.1F * (float) Math.sin(state.ageInTicks * 0.1F);
            }
            if (state.prevMimicState != null) {
                float alphaPrev = 1 - transProgress * 0.2F;
                VertexConsumer prev = buffer
                        .getBuffer(AMRenderTypes.entityTranslucent(getFor(state.prevMimicState)));
                if (state.prevMimicState == state.mimicState) {
                    alphaPrev *= a;
                }
            ...
            VertexConsumer current = buffer
                    .getBuffer(AMRenderTypes.entityTranslucent(getFor(state.mimicState)));
