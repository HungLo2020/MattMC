...
public class EntityAnaconda extends Animal implements ISemiAquatic {
    ...
    public static boolean canAnacondaSpawn(EntityType type, LevelAccessor worldIn, EntitySpawnReason reason, BlockPos pos, RandomSource randomIn) {
        // Using swamp/jungle blocks as spawn blocks instead of custom tag
        final boolean spawnBlock = worldIn.getBlockState(pos.below()).is(BlockTags.ANIMALS_SPAWNABLE_ON);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.25D, itemStack -> itemStack.is(ItemTags.FISHES), false));
        ...
        // Target small animals and fish - simplified predicate
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal(this, LivingEntity.class, 200, false, false, 
            (entity, level) -> entity instanceof Animal && entity.getBbWidth() < 1.5F));
...
//                float headRot = Mth.wrapDegrees(this.getYRot());
        ...
        if (this.getSheddingTime() > 0) {
            ...
            if (this.getSheddingTime() == 0) {
                // Shed snake skin - using vanilla item as placeholder
                this.spawnItemAtOffset(new ItemStack(Items.PAPER), 1 + random.nextFloat(), 0.2F);
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob mob) {
        // TODO: EntityType.ANACONDA needs to be registered in vanilla EntityType
        Entity entity = this.getType().create(serverWorld, EntitySpawnReason.BREEDING);
    ...
    public void awardKillScore(Entity entity, DamageSource src) {
        if(entity instanceof LivingEntity living){
            // In 1.21, addAdditionalSaveData uses ValueOutput and readAdditionalSaveData uses ValueInput
            // We can't directly manipulate loot tables the same way anymore
        ...
        public void tick() {
            ...
            if (target != null && target.isAlive()) {
                if (jumpAttemptCooldown == 0 && snake.distanceTo(target) < 1 + target.getBbWidth() && !snake.isStrangling()) {
                    ...
                    // TODO: Add ANACONDA_ATTACK sound event
                    snake.playSound(SoundEvents.GENERIC_HURT, snake.getSoundVolume(), snake.getVoicePitch());
