...
public class EntityAlligatorSnappingTurtle extends Animal implements ISemiAquatic, Shearable {
    ...
    public static final TargetingConditions.Selector TARGET_PRED = (animal, level) -> {
    ...
    public EntityAlligatorSnappingTurtle(EntityType type, Level worldIn) {
        super(type, worldIn);
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.ALLIGATOR_SNAPPING_TURTLE_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return SoundEvents.ALLIGATOR_SNAPPING_TURTLE_HURT;
    }
    ...
    public static boolean canTurtleSpawn(EntityType type, LevelAccessor worldIn, EntitySpawnReason reason, BlockPos pos, RandomSource randomIn) {
        // Using sand as spawn block instead of custom tag
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createAnimalAttributes().add(Attributes.MAX_HEALTH, 18.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.7D).add(Attributes.ARMOR, 8D).add(Attributes.FOLLOW_RANGE, 16.0D).add(Attributes.ATTACK_DAMAGE, 4.0D).add(Attributes.MOVEMENT_SPEED, 0.2F).add(Attributes.STEP_HEIGHT, 1.0D);
    }
    ...
    public boolean isFood(ItemStack stack) {
        // Using fish items instead of custom tag
    ...
    protected void updateAir(int air) {
        // Alligator Snapping Turtle can breathe underwater
    }

    public void addAdditionalSaveData(ValueOutput valueOutput) {
        super.addAdditionalSaveData(valueOutput);
        valueOutput.putBoolean("Waiting", this.isWaiting());
        valueOutput.putInt("MossLevel", this.getMoss());
        valueOutput.putFloat("TurtleScale", this.getTurtleScale());
        valueOutput.putInt("MossTime", this.mossTime);
        valueOutput.putInt("WaitTime", this.waitTime);
        valueOutput.putInt("WaitTime2", this.timeUntilWait);
    }

    public void readAdditionalSaveData(ValueInput valueInput) {
        super.readAdditionalSaveData(valueInput);
        this.setWaiting(valueInput.getBooleanOr("Waiting", false));
        this.setMoss(valueInput.getIntOr("MossLevel", 0));
        this.setTurtleScale(valueInput.getFloatOr("TurtleScale", 1.0F));
        this.mossTime = valueInput.getIntOr("MossTime", 0);
        this.waitTime = valueInput.getIntOr("WaitTime", 0);
        this.timeUntilWait = valueInput.getIntOr("WaitTime2", 0);
    }
    ...
    public void shear(ServerLevel level, SoundSource category, ItemStack stack) {
        ...
        if (!level.isClientSide()) {
            // Simplified: always drop seagrass (no spiked scute)
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableEntity) {
        return EntityType.ALLIGATOR_SNAPPING_TURTLE.create(serverLevel, EntitySpawnReason.BREEDING);
    }
}
