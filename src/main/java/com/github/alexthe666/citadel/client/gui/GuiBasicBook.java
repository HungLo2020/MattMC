...
public abstract class GuiBasicBook extends Screen {
    ...
    public static void drawTabulaModelOnScreen(GuiGraphics guiGraphics, TabulaModel model, ResourceLocation tex, int posX, int posY, float scale, boolean follow, double xRot, double yRot, double zRot, float mouseX, float mouseY) {
        ...
        // Citadel: 1.21 API - EntityRenderDispatcher.overrideCameraOrientation() removed
        // Citadel: 1.21 API - EntityRenderDispatcher.setRenderShadow() removed
        // Citadel: 1.21 API - RenderSystem.runAsFancy() removed
        // Citadel: 1.21 API - Lighting.setupFor3DItems() removed
        // TODO: Adapt entity rendering for 1.21's new rendering pipeline
        /*
    ...
    public void drawEntityOnScreen(GuiGraphics guiGraphics, MultiBufferSource bufferSource, int posX, int posY, float zOff, float scale, boolean follow, double xRot, double yRot, double zRot, float mouseX, float mouseY, Entity entity) {
        ...
        // Citadel: 1.21 API - GuiGraphics.pose() returns Matrix3x2fStack (2D only), not PoseStack
        // Citadel: 1.21 API - No pushPose/popPose/mulPose methods on Matrix3x2fStack
        // Citadel: 1.21 API - EntityRenderDispatcher methods (overrideCameraOrientation, setRenderShadow, render) changed/removed
        // Citadel: 1.21 API - RenderSystem.setShaderLights() now requires GpuBufferSlice not Vector3f
        // Citadel: 1.21 API - RenderSystem.runAsFancy() removed
        // Citadel: 1.21 API - GuiGraphics.flush() removed
        // Citadel: 1.21 API - Lighting.setupFor3DItems() removed
        // TODO: This entire method needs to be rewritten for 1.21's GUI rendering system
        // Entity rendering in GUIs is handled completely differently in 1.21
        // For now, commenting out all incompatible code
    ...
    /**
     * Override removed - method signature changed in 1.21
     * renderBlurredBackground() still exists but may have different parameters
     */
    // Citadel: 1.21 API - Method signature for renderBlurredBackground changed
    // Commenting out override to prevent compilation error
    /*
    @Override
    ...
    */
    ...
    private Recipe getRecipeByName(ResourceLocation registryName) {
        try {
            // Citadel: 1.21 API - Level.getRecipeManager() removed
            // Citadel: 1.21 API - RecipeManager.byKey() now requires ResourceKey<Recipe<?>> not ResourceLocation
            // TODO: Adapt recipe lookup for 1.21's new recipe system
            // Recipe manager access and lookup completely redesigned
            /*
            ...
            */
            // Return null for now - recipe rendering will be disabled
            return null;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void renderOtherWidgets(GuiGraphics guiGraphics, int x, int y, BookPage page) {
        ...
        for (ImageData imageData : images) {
            if (imageData.page() == this.currentPageCounter) {
                ...
                // Citadel: 1.21 API - GuiGraphics.pose() returns Matrix3x2fStack, no pushPose/popPose
                // Citadel: 1.21 API - translate() and scale() signatures changed for 2D rendering
                // Citadel: 1.21 API - guiGraphics.blit() signature changed completely
                // TODO: Adapt image rendering for 1.21's GuiGraphics API
                /*
                guiGraphics.pose().pushPose();
