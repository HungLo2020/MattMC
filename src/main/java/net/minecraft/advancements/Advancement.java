...
public record Advancement(
	...
	private static Component decorateName(DisplayInfo displayInfo) {
		...
		Component component3 = component.copy().withStyle(style -> style.withHoverEvent(new HoverEvent.ShowText(component2)));
	...
	public void validate(ProblemReporter problemReporter, HolderGetter.Provider provider) {
		this.criteria.forEach((string, criterion) -> {
			CriterionValidator criterionValidator = new CriterionValidator(problemReporter.forChild(new ProblemReporter.RootFieldPathElement(string)), provider);
	...
	public static class Builder {
		...
		public Advancement.Builder display(
			...
			return this.display(
				new DisplayInfo(
					itemStack, component, component2, Optional.ofNullable(resourceLocation).map(ClientAsset.ResourceTexture::new), advancementType, bl, bl2, bl3
				)
			);
		}
		...
		public Advancement.Builder display(
			...
			return this.display(
				new DisplayInfo(
					new ItemStack(itemLike.asItem()),
					component,
					component2,
					Optional.ofNullable(resourceLocation).map(ClientAsset.ResourceTexture::new),
					advancementType,
					bl,
					bl2,
					bl3
