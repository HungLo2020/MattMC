...
public class ParticleResources implements PreparableReloadListener {
	...
	private final Map<ResourceLocation, ParticleResources.MutableSpriteSet> spriteSets = Maps.<ResourceLocation, ParticleResources.MutableSpriteSet>newHashMap();
	...
	public CompletableFuture<Void> reload(SharedState sharedState, Executor executor, PreparationBarrier preparationBarrier, Executor executor2) {
		...
		return CompletableFuture.allOf(completableFuture, completableFuture2).thenCompose(preparationBarrier::wait).thenAcceptAsync(void_ -> {
			...
			TextureAtlasSprite textureAtlasSprite = preparations.missing();
					completableFuture.join().forEach(arg -> {
				Optional<List<ResourceLocation>> optional = arg.sprites();
				if (!optional.isEmpty()) {
					List<TextureAtlasSprite> list = new ArrayList();

					for (ResourceLocation resourceLocation : (List<ResourceLocation>)optional.get()) {
						TextureAtlasSprite textureAtlasSprite2 = preparations.getSprite(resourceLocation);
