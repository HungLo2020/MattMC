...
public class LocalPlayer extends AbstractClientPlayer implements net.irisshaders.iris.mixinterface.LocalPlayerInterface {
	...
	public void tick() {
		...
		if (this.hasClientLoaded()) {
			...
			if (this.isPassenger()) {
				this.connection.send(new Rot(this.getYRot(), this.getXRot(), this.onGround(), this.horizontalCollision));
	...
	private void sendPosition() {
		...
		if (this.isControlledCamera()) {
			...
			if (bl && bl2) {
				this.connection.send(new PosRot(this.position(), this.getYRot(), this.getXRot(), this.onGround(), this.horizontalCollision));
			} else if (bl) {
				this.connection.send(new Pos(this.position(), this.onGround(), this.horizontalCollision));
			} else if (bl2) {
				this.connection.send(new Rot(this.getYRot(), this.getXRot(), this.onGround(), this.horizontalCollision));
			} else if (this.lastOnGround != this.onGround() || this.lastHorizontalCollision != this.horizontalCollision) {
				this.connection.send(new StatusOnly(this.onGround(), this.horizontalCollision));
			}
	...
	public boolean drop(boolean bl) {
		net.minecraft.network.protocol.game.ServerboundPlayerActionPacket.Action action = bl
	...
	private void moveTowardsClosestSpace(double d, double e) {
		...
		if (this.suffocatesAt(blockPos)) {
			...
			for (Direction direction2 : directions) {
				...
				double j = direction2.getAxisDirection() == AxisDirection.POSITIVE ? 1.0 - i : i;
							...
							if (aABB2.intersects(vec310, vec311) || aABB2.intersects(vec312, vec313)) {
								...
								for (int u = 1; u < p; u++) {
									...
									if (!(voxelShape = blockState3.getCollisionShape(this.level(), blockPos3, collisionContext)).isEmpty()) {
										t = (float)voxelShape.max(Axis.Y) + blockPos3.getY();
										if (t - this.getY() > p) {
											return;
										}
									}
		...
		while (true) {
			...
			if (this.level().getBlockState(toPos).getBlock() instanceof net.minecraft.world.level.block.ElevatorBlock) {
				if (this.isValidElevatorPos(toPos)) {
					// Call the elevator block to handle teleportation
					...
					elevatorBlock.tryTeleportUp(this.level(), fromPos, this);
					break;
				}
			}
		}
