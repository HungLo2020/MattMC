...
public class TpsDebugChart extends AbstractDebugChart {
	...
	protected int getSampleHeight(double d) {
		return (int)Math.round(toMilliseconds(d) * 60.0 / ((Float)this.msptSupplier.get()).floatValue());
	}

	@Override
	protected int getSampleColor(long l) {
		float f = (Float)this.msptSupplier.get();
		return this.getSampleColor(toMilliseconds(l), f, -16711936, f * 1.125, -256, f * 1.25, -65536);
	}
