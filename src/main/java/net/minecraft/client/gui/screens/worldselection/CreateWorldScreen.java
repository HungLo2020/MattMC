...
public class CreateWorldScreen extends Screen {
	...
	private static void openCreateWorldScreen(
		Minecraft minecraft,
		Runnable runnable,
		Function<DataLoadContext, WorldGenSettings> function,
		...
		PackRepository packRepository = new PackRepository(new RepositorySource[]{new ServerPacksSource(minecraft.directoryValidator())});
		...
		minecraft.setScreen(
			new CreateWorldScreen(
				minecraft, runnable, (WorldCreationContext)completableFuture.join(), Optional.of(resourceKey), OptionalLong.empty(), createWorldCallback
			)
		);
	}

	public static CreateWorldScreen createFromExisting(
		Minecraft minecraft, Runnable runnable, LevelSettings levelSettings, WorldCreationContext worldCreationContext, @Nullable Path path
	) {
	...
	private void onCreate() {
		...
		LayeredRegistryAccess<RegistryLayer> layeredRegistryAccess = worldCreationContext.worldgenRegistries()
			.replaceFrom(RegistryLayer.DIMENSIONS, new Frozen[]{complete.dimensionsRegistryAccess()});
		...
		LevelSettings levelSettings = this.createLevelSettings(complete.specialWorldProperty() == SpecialWorldProperty.DEBUG);
	...
	private boolean createNewWorld(LayeredRegistryAccess<RegistryLayer> layeredRegistryAccess, WorldData worldData) {
		...
		Optional<LevelStorageAccess> optional = createNewWorldDirectory(this.minecraft, string, this.tempDataPackDir);
		if (optional.isEmpty()) {
			...
			this.minecraft
				...
				.createLevelFromExistingSettings((LevelStorageAccess)optional.get(), worldCreationContext.dataPackResources(), layeredRegistryAccess, worldData);
			return true;
		}
	}
	...
	private static InitConfig createDefaultLoadConfig(PackRepository packRepository, WorldDataConfiguration worldDataConfiguration) {
		PackConfig packConfig = new PackConfig(packRepository, worldDataConfiguration, false, true);
		return new InitConfig(packConfig, CommandSelection.INTEGRATED, 2);
	}
	...
	private static Optional<LevelStorageAccess> createNewWorldDirectory(Minecraft minecraft, String string, @Nullable Path path) {
		try {
			LevelStorageAccess levelStorageAccess = minecraft.getLevelSource().createAccess(string);
			if (path == null) {
						...
						.setTooltip(
							Tooltip.create(
								Component.translatable(
									"selectWorld.targetFolder", new Object[]{Component.literal(worldCreationUiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)}
								)
							)
						)
				);
	...
	class WorldTab extends GridLayoutTab {
		...
		private static MutableComponent createTypeButtonNarration(CycleButton<WorldCreationUiState.WorldTypeEntry> cycleButton) {
			return cycleButton.getValue().isAmplified()
				? CommonComponents.joinForNarration(new Component[]{cycleButton.createDefaultNarrationMessage(), AMPLIFIED_HELP_TEXT})
