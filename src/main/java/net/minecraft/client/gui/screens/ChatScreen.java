...
public class ChatScreen extends Screen {
	...
	public void render(GuiGraphics guiGraphics, int i, int j, float f) {
		...
		if (guiMessageTag != null && guiMessageTag.text() != null) {
			guiGraphics.setTooltipForNextFrame(this.font, this.font.split(guiMessageTag.text(), 210), i, j);
		} else {
	...
	protected void updateNarrationState(NarrationElementOutput narrationElementOutput) {
		...
		if (!string.isEmpty()) {
			narrationElementOutput.nest().add(NarratedElementType.TITLE, Component.translatable("chat_screen.message", new Object[]{string}));
		}
	}
	...
	public interface ChatConstructor<T extends ChatScreen> {
		T create(String string, boolean bl);
	}
