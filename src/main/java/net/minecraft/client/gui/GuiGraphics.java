...
public class GuiGraphics {
	...
	public void fill(RenderPipeline renderPipeline, TextureSetup textureSetup, int i, int j, int k, int l) {
		if ((net.vulkanic.bridge.RustGalVulkanWholeFrameMode.enabled()
			...
			// explicit end-portal path) so Rust can own image admission and binding.
			throw new IllegalStateException("Java textured GUI fill is unavailable while Rust owns Vulkan presentation");
		}
	...
	public void textHighlight(int i, int j, int k, int l) {
	...
	public void blitSprite(RenderPipeline renderPipeline, ResourceLocation resourceLocation, int i, int j, int k, int l, int m, int n, int o, int p) {
		this.blitSprite(renderPipeline, resourceLocation, i, j, k, l, m, n, o, p, -1);
	}
	...
	public void blitSprite(RenderPipeline renderPipeline, ResourceLocation resourceLocation, int i, int j, int k, int l, int m, int n, int o, int p, int q) {
		...
		if (guiSpriteScaling instanceof GuiSpriteScaling.Stretch) {
			...
			this.blitSprite(renderPipeline, resourceLocation, m - k, n - l, i, j, q);
			this.disableScissor();
		}
	}
	...
	public void blit(RenderPipeline renderPipeline, ResourceLocation resourceLocation, int i, int j, float f, float g, int k, int l, int m, int n, int o) {
		this.blit(renderPipeline, resourceLocation, i, j, f, g, k, l, k, l, m, n, o);
	}
	...
	public void blit(RenderPipeline renderPipeline, ResourceLocation resourceLocation, int i, int j, float f, float g, int k, int l, int m, int n) {
		this.blit(renderPipeline, resourceLocation, i, j, f, g, k, l, k, l, m, n);
	}
	...
	public void blit(
		RenderPipeline renderPipeline, ResourceLocation resourceLocation, int i, int j, float f, float g, int k, int l, int m, int n, int o, int p, int q
	) {
		this.innerBlit(renderPipeline, resourceLocation, i, i + k, j, j + l, (f + 0.0F) / o, (f + m) / o, (g + 0.0F) / p, (g + n) / p, q);
	}
	...
	public void setTooltipForNextFrame(Font font, ItemStack itemStack, int i, int j) {
		this.setTooltipForNextFrame(
			font, Screen.getTooltipFromItem(this.minecraft, itemStack), itemStack.getTooltipImage(), i, j, (ResourceLocation)itemStack.get(DataComponents.TOOLTIP_STYLE)
		);
	}
	...
	public void setTooltipForNextFrame(
		Font font, List<Component> list, Optional<TooltipComponent> optional, int i, int j, @Nullable ResourceLocation resourceLocation
	) {
		...
		this.setTooltipForNextFrameInternal(font, list2, i, j, DefaultTooltipPositioner.INSTANCE, resourceLocation, false);
	}
	...
	public void setTooltipForNextFrame(Font font, Component component, int i, int j, @Nullable ResourceLocation resourceLocation) {
		this.setTooltipForNextFrame(font, List.of(component.getVisualOrderText()), i, j, resourceLocation);
	}
	...
	private void setTooltipForNextFrameInternal(
		...
		if (!list.isEmpty()) {
			if (this.deferredTooltip == null || bl) {
				this.deferredTooltip = () -> this.renderTooltip(font, list, i, j, clientTooltipPositioner, resourceLocation);
			}
		}
	}
	...
	public void renderTooltip(
		...
		int q = vector2ic.y();
		this.pose.pushMatrix();
		TooltipRenderUtil.renderTooltipBackground(this, p, q, k, l, resourceLocation);
		int r = q;
	...
	public void renderDeferredElements() {
		if (!this.deferredOutlines.isEmpty()) {
			...
			for (GuiGraphics.OutlineBox outlineBox : this.deferredOutlines) {
				outlineBox.render(this);
			}
	...
	public void submitSkinRenderState(PlayerModel playerModel, ResourceLocation resourceLocation, float f, float g, float h, float i, int j, int k, int l, int m) {
		this.guiRenderState.submitPicturesInPictureState(new GuiSkinRenderState(playerModel, resourceLocation, g, h, i, j, k, l, m, f, this.scissorStack.peek()));
	}
	...
	public void submitBookModelRenderState(BookModel bookModel, ResourceLocation resourceLocation, float f, float g, float h, int i, int j, int k, int l) {
		this.guiRenderState.submitPicturesInPictureState(new GuiBookModelRenderState(bookModel, resourceLocation, g, h, i, j, k, l, f, this.scissorStack.peek()));
	}
