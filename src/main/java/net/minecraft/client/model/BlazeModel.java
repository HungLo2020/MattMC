...
public class BlazeModel extends EntityModel<LivingEntityRenderState> {
	...
	public static LayerDefinition createBodyLayer() {
		...
		for (int i = 0; i < 4; i++) {
			...
			float h = -2.0F + Mth.cos(i * 2 * 0.25F);
		...
		for (int i = 8; i < 12; i++) {
			...
			float h = 11.0F + Mth.cos(i * 1.5F * 0.5F);
	...
	public void setupAnim(LivingEntityRenderState livingEntityRenderState) {
		...
		for (int i = 0; i < 4; i++) {
			this.upperBodyParts[i].y = -2.0F + Mth.cos((i * 2 + livingEntityRenderState.ageInTicks) * 0.25F);
		...
		for (int i = 8; i < 12; i++) {
			this.upperBodyParts[i].y = 11.0F + Mth.cos((i * 1.5F + livingEntityRenderState.ageInTicks) * 0.5F);
