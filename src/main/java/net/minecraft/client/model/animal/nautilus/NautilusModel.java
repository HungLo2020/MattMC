...
public class NautilusModel extends EntityModel<NautilusRenderState> {
	...
	public static MeshDefinition createBodyMesh() {
		...
		PartDefinition nautilus = root.addOrReplaceChild("root", CubeListBuilder.create(), PartPose.offset(0.0F, 29.0F, -6.0F));
	...
	public static LayerDefinition createBabyBodyLayer() {
		MeshDefinition meshDefinition = new MeshDefinition();
		PartDefinition root = meshDefinition.getRoot();
		PartDefinition nautilus = root.addOrReplaceChild("root", CubeListBuilder.create(), PartPose.offset(-0.5F, 28.0F, -0.5F));
		nautilus.addOrReplaceChild(
			"shell",
			CubeListBuilder.create()
				.texOffs(0, 0)
	...
	public void setupAnim(NautilusRenderState state) {
		...
		this.body.yRot = Mth.clamp(state.yRot, -10.0F, 10.0F) * (float) (Math.PI / 180.0);
		this.body.xRot = Mth.clamp(state.xRot, -10.0F, 10.0F) * (float) (Math.PI / 180.0);
