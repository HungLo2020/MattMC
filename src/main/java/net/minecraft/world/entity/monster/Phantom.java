...
public class Phantom extends Mob implements Enemy {
	...
	public void tick() {
		...
		if (this.level().isClientSide()) {
			...
			float g = Mth.cos((this.getUniqueFlapTickOffset() + this.tickCount + 1) * 7.448451F * (float) (Math.PI / 180.0) + (float) Math.PI);
			if (f > 0.0F && g <= 0.0F) {
				this.level()
					.playLocalSound(
						this.getX(),
			...
			this.level().addParticle(ParticleTypes.MYCELIUM, this.getX() + i, this.getY() + k, this.getZ() + j, 0.0, 0.0, 0.0);
			this.level().addParticle(ParticleTypes.MYCELIUM, this.getX() - i, this.getY() + k, this.getZ() - j, 0.0, 0.0, 0.0);
		}
	}
	...
	protected void readAdditionalSaveData(ValueInput valueInput) {
		...
		this.anchorPoint = (BlockPos)valueInput.read("anchor_pos", BlockPos.CODEC).orElse(null);
	...
	class PhantomMoveControl extends MoveControl {
		...
		public void tick() {
			...
			if (Math.abs(g) > 1.0E-5F) {
				...
				double p = this.speed * Mth.cos(o * (float) (Math.PI / 180.0)) * Math.abs(d / i);
				double q = this.speed * Mth.sin(o * (float) (Math.PI / 180.0)) * Math.abs(f / i);
				double r = this.speed * Mth.sin(n * (float) (Math.PI / 180.0)) * Math.abs(e / i);
