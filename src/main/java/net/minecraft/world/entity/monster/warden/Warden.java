...
public class Warden extends Monster implements VibrationSystem {
	...
	private void clientDiggingParticles(AnimationState animationState) {
		if ((float)animationState.getTimeInMillis(this.tickCount) < 4500.0F) {
			...
			if (blockState.getRenderShape() != RenderShape.INVISIBLE) {
				for (int i = 0; i < 30; i++) {
					double d = this.getX() + Mth.randomBetween(randomSource, -0.7F, 0.7F);
					double e = this.getY();
					double f = this.getZ() + Mth.randomBetween(randomSource, -0.7F, 0.7F);
	...
	protected void readAdditionalSaveData(ValueInput valueInput) {
		...
		this.angerManagement = (AngerManagement)valueInput.read("anger", AngerManagement.codec(this::canTargetEntity))
