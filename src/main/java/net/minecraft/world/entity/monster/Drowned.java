...
public class Drowned extends Zombie implements RangedAttackMob {
	...
	protected boolean canReplaceCurrentItem(ItemStack itemStack, ItemStack itemStack2, EquipmentSlot equipmentSlot) {
		return itemStack2.is(Items.NAUTILUS_SHELL) ? false : super.canReplaceCurrentItem(itemStack, itemStack2, equipmentSlot);
	}
	...
	public boolean okTarget(@Nullable LivingEntity livingEntity) {
		return livingEntity != null ? !this.level().isBrightOutside() || livingEntity.isInWater() : false;
	}
	...
	public void performRangedAttack(LivingEntity livingEntity, float f) {
		...
		if (this.level() instanceof ServerLevel serverLevel) {
			Projectile.spawnProjectileUsingShoot(thrownTrident, serverLevel, itemStack2, d, e + h * 0.2F, g, 1.6F, 14 - this.level().getDifficulty().getId() * 4);
		}
	...
	static class DrownedSwimUpGoal extends Goal {
		...
		public void tick() {
			if (this.drowned.getY() < this.seaLevel - 1 && (this.drowned.getNavigation().isDone() || this.drowned.closeToNextPos())) {
				Vec3 vec3 = DefaultRandomPos.getPosTowards(this.drowned, 4, 8, new Vec3(this.drowned.getX(), this.seaLevel - 1, this.drowned.getZ()), (float) (Math.PI / 2));
				if (vec3 == null) {
					this.stuck = true;
					return;
				}
