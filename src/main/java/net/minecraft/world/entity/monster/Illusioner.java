...
public class Illusioner extends SpellcasterIllager implements RangedAttackMob {
	...
	public Vec3[] getIllusionOffsets(float f) {
		if (this.clientSideIllusionTicks <= 0) {
			...
			double d = (this.clientSideIllusionTicks - f) / 3.0F;
			d = Math.pow(d, 0.25);
	...
	public void performRangedAttack(LivingEntity livingEntity, float f) {
		...
		if (this.level() instanceof ServerLevel serverLevel) {
			Projectile.spawnProjectileUsingShoot(abstractArrow, serverLevel, itemStack2, d, e + h * 0.2F, g, 1.6F, 14 - serverLevel.getDifficulty().getId() * 4);
		}
