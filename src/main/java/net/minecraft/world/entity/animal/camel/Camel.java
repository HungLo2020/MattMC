...
public class Camel extends AbstractHorse {
	...
	public static boolean checkCamelSpawnRules(
		EntityType<Camel> entityType, LevelAccessor levelAccessor, EntitySpawnReason entitySpawnReason, BlockPos blockPos, RandomSource randomSource
	) {
		return levelAccessor.getBlockState(blockPos.below()).is(BlockTags.CAMELS_SPAWNABLE_ON) && isBrightEnoughToSpawn(levelAccessor, blockPos);
	}
	...
	protected void executeRidersJump(float f, Vec3 vec3) {
		...
		this.addDeltaMovement(
			this.getLookAngle()
				...
				.normalize()
				.scale(22.2222F * f * this.getAttributeValue(Attributes.MOVEMENT_SPEED) * this.getBlockSpeedFactor())
				.add(0.0, 1.4285F * f * d, 0.0)
		);
	...
	public boolean canMate(Animal animal) {
		return animal != this && animal instanceof Camel camel && this.canParent() && camel.canParent();
	}
	...
	protected Vec3 getPassengerAttachmentPoint(Entity entity, EntityDimensions entityDimensions, float f) {
		...
		return new Vec3(0.0, h, g * f).yRot(-this.getYRot() * (float) (Math.PI / 180.0));
	}
	...
	private double getBodyAnchorAnimationYOffset(boolean bl, float f, EntityDimensions entityDimensions, float g) {
		...
		if (bl2) {
			...
			boolean bl4 = n < l;
			float o = bl4 ? n / l : (n - l) / (k - l);
			float p = h - m * i;
			d += bl3 ? Mth.lerp(o, bl4 ? h : p, bl4 ? p : j) : Mth.lerp(o, bl4 ? j - h : j - p, bl4 ? j - p : 0.0F);
		}
	...
	public Vec3 getLeashOffset(float f) {
		...
		return new Vec3(0.0, this.getBodyAnchorAnimationYOffset(true, f, entityDimensions, g) - 0.2F * g, entityDimensions.width() * 0.56F);
	}
	...
	protected Holder<SoundEvent> getEquipSound(EquipmentSlot equipmentSlot, ItemStack itemStack, Equippable equippable) {
		return (Holder<SoundEvent>)(equipmentSlot == EquipmentSlot.SADDLE ? SoundEvents.CAMEL_SADDLE : super.getEquipSound(equipmentSlot, itemStack, equippable));
	}
