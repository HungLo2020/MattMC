...
public class LlamaSpit extends Projectile {
	...
	public LlamaSpit(Level level, Llama llama) {
		...
		this.setOwner(llama);
		this.setPos(
			llama.getX() - (llama.getBbWidth() + 1.0F) * 0.5 * Mth.sin(llama.yBodyRot * (float) (Math.PI / 180.0)),
			llama.getEyeY() - 0.1F,
			llama.getZ() + (llama.getBbWidth() + 1.0F) * 0.5 * Mth.cos(llama.yBodyRot * (float) (Math.PI / 180.0))
		);
	}
