...
public class HoneyBlock extends HalfTransparentBlock {
	...
	private boolean isSlidingDown(BlockPos blockPos, Entity entity) {
		if (entity.onGround()) {
			return false;
		} else if (entity.getY() > blockPos.getY() + 0.9375 - 1.0E-7) {
			return false;
		} else if (getOldDeltaY(entity.getDeltaMovement().y) >= -0.08) {
			return false;
		} else {
			double d = Math.abs(blockPos.getX() + 0.5 - entity.getX());
			double e = Math.abs(blockPos.getZ() + 0.5 - entity.getZ());
			double f = 0.4375 + entity.getBbWidth() / 2.0F;
