...
public class ChunkStorage implements AutoCloseable {
		...
		if (i == SharedConstants.getCurrentVersion().dataVersion().version()) {
			...
			try {
				if (i < 1493) {
					...
					if ((Boolean)compoundTag.getCompound("Level").flatMap(compoundTagx -> compoundTagx.getBoolean("hasLegacyStructureData")).orElse(false)) {
	...
	public CompletableFuture<Optional<NativeChunkSectionStorage.DecodeResult>> readChunkSections(ChunkPos chunkPos) {
		return this.worker.loadChunkSectionsAsync(chunkPos);
	}
