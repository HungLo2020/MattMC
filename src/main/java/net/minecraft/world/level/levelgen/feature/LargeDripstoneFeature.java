...
public class LargeDripstoneFeature extends Feature<LargeDripstoneConfiguration> {
	...
	private static LargeDripstoneFeature.LargeDripstone makeDripstone(
		...
		return new LargeDripstoneFeature.LargeDripstone(blockPos, bl, i, floatProvider.sample(randomSource), floatProvider2.sample(randomSource));
	}
	...
	static final class LargeDripstone {
					...
					if (!(f > this.radius)) {
						...
						if (k > 0) {
							if (randomSource.nextFloat() < 0.2) {
								k = (int)(k * Mth.randomBetween(randomSource, 0.8F, 1.0F));
							}
		...
		boolean isSuitableForWind(LargeDripstoneConfiguration largeDripstoneConfiguration) {
			return this.radius >= largeDripstoneConfiguration.minRadiusForWind && this.bluntness >= largeDripstoneConfiguration.minBluntnessForWind;
		}
	}
