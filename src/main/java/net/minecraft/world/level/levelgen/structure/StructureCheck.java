...
public class StructureCheck {
	...
	private Object2IntMap<Structure> loadStructures(CompoundTag compoundTag) {
		Optional<CompoundTag> optional = compoundTag.getCompound("structures").flatMap(compoundTagx -> compoundTagx.getCompound("starts"));
		if (optional.isEmpty()) {
			return null;
		} else {
			...
			if (compoundTag2.isEmpty()) {
				...
				compoundTag2.forEach((string, tag) -> {
					...
					if (resourceLocation != null) {
						...
						if (structure != null) {
							tag.asCompound().ifPresent(compoundTagx -> {
