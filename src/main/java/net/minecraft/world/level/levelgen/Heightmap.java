...
public class Heightmap {
	...
	public void setRawData(ChunkAccess chunkAccess, Heightmap.Types types, long[] ls) {
		...
		if (ms.length == ls.length) {
			...
			LOGGER.warn("Ignoring heightmap data for chunk {}, size does not match; expected: {}, got: {}", chunkAccess.getPos(), ms.length, ls.length);
	...
	public static enum Types implements StringRepresentable {
		...
		private Types(final int j, final String string2, final Heightmap.Usage usage, final Predicate<BlockState> predicate) {
			this.id = j;
