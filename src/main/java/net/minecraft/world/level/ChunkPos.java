...
public class ChunkPos {
	...
	public boolean contains(BlockPos blockPos) {
		return blockPos.getX() >= this.getMinBlockX()
			&& blockPos.getZ() >= this.getMinBlockZ()
			&& blockPos.getX() <= this.getMaxBlockX()
			&& blockPos.getZ() <= this.getMaxBlockZ();
	}
			...
			public boolean tryAdvance(Consumer<? super ChunkPos> consumer) {
				if (this.pos == null) {
					...
					if (ix == chunkPos2.x) {
						...
						this.pos = new ChunkPos(chunkPos.x, jx + l);
					} else {
