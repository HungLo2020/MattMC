...
public class MobEffectInstance implements Comparable<MobEffectInstance> {
	...
	public MobEffectInstance withScaledDuration(float f) {
		...
		mobEffectInstance.duration = mobEffectInstance.mapDuration(i -> Math.max(Mth.floor(i * f), 1));
	...
	static class BlendState {
		...
		public void setImmediate(MobEffectInstance mobEffectInstance) {
			this.factor = hasEffect(mobEffectInstance) ? 1.0F : 0.0F;
		...
		public void tick(MobEffectInstance mobEffectInstance) {
			this.factorPreviousFrame = this.factor;
			boolean bl = hasEffect(mobEffectInstance);
			float f = bl ? 1.0F : 0.0F;
			if (this.factor != f) {
				MobEffect mobEffect = mobEffectInstance.getEffect().value();
				int i = bl ? mobEffect.getBlendInDurationTicks() : mobEffect.getBlendOutDurationTicks();
				if (i == 0) {
					this.factor = f;
				} else {
					float g = 1.0F / i;
