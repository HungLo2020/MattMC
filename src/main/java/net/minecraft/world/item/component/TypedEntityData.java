	...
	public static <T> Codec<TypedEntityData<T>> codec(Codec<T> codec) {
		return new Codec<TypedEntityData<T>>() {
			...
			public <V> DataResult<Pair<TypedEntityData<T>, V>> decode(DynamicOps<V> dynamicOps, V object) {
				DataResult<T> dataResult = dynamicOps.get(object, "id")
					.flatMap(objectx -> codec.parse(dynamicOps, (V)objectx).mapError(string -> "Failed to parse 'id': " + string));
				DataResult<Pair<CompoundTag, V>> dataResult2 = CustomData.COMPOUND_TAG_CODEC.decode(dynamicOps, dynamicOps.remove(object, "id"));
				return dataResult.apply2stable((objectx, pair) -> new Pair<>(new TypedEntityData<>(objectx, (CompoundTag)pair.getFirst()), pair.getSecond()), dataResult2);
			}
