...
public class Scoreboard {
		...
		return this.playerScores
			...
			.flatMap(
				entry -> {
					...
					return ((PlayerScores)entry.getValue())
						.listRawScores()
						.entrySet()
						.stream()
						.map(entryx -> new Scoreboard.PackedScore(string, ((Objective)entryx.getKey()).getName(), (Score)entryx.getValue()));
				}
			)
			.toList();
	}

	protected void loadPlayerScore(Scoreboard.PackedScore packedScore) {
	...
	protected void loadObjective(Objective.Packed packed) {
		this.addObjective(
			packed.name(), packed.criteria(), packed.displayName(), packed.renderType(), packed.displayAutoUpdate(), (NumberFormat)packed.numberFormat().orElse(null)
		);
	}
