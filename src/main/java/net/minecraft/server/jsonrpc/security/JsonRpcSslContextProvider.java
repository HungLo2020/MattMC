...
public class JsonRpcSslContextProvider {
	...
	public static SslContext createFrom(String string, String string2) throws Exception {
		if (string.isEmpty()) {
			...
			if (file.exists() && file.isFile()) {
				String string3 = getKeystorePassword(string2);
				return loadKeystoreFromPath(file, string3);
			} else {
	...
	private static String getKeystorePassword(String string) {
		String string2 = (String)System.getenv().get("MINECRAFT_MANAGEMENT_TLS_KEYSTORE_PASSWORD");
		if (string2 != null) {
			return string2;
		} else {
			String string3 = System.getProperty("management.tls.keystore.password", null);
