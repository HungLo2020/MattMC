...
public class BlockPos extends Vec3i {
	...
	public Vec3 clampLocationWithin(Vec3 vec3) {
		return new Vec3(
			Mth.clamp(vec3.x, (double)(this.getX() + 1.0E-5F), this.getX() + 1.0 - 1.0E-5F),
			Mth.clamp(vec3.y, (double)(this.getY() + 1.0E-5F), this.getY() + 1.0 - 1.0E-5F),
			Mth.clamp(vec3.z, (double)(this.getZ() + 1.0E-5F), this.getZ() + 1.0 - 1.0E-5F)
		);
	}
	...
	public static int breadthFirstTraversal(
		...
		while (!queue.isEmpty()) {
			...
			if (longSet.add(m)) {
				BlockPos.TraversalNodeStatus traversalNodeStatus = (BlockPos.TraversalNodeStatus)function.apply(blockPos2);
