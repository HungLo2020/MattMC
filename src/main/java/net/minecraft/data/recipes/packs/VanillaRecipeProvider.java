	...
	public static Stream<VanillaRecipeProvider.TrimTemplate> smithingTrims() {
		return Stream.of(
			...
			.map(pair -> {
				...
				ResourceKey<TrimPattern> resourceKey = (ResourceKey<TrimPattern>)pair.getSecond();
				ResourceKey<Recipe<?>> resourceKey2 = ResourceKey.create(Registries.RECIPE, ResourceLocation.withDefaultNamespace(getItemName(item) + "_smithing_trim"));
				return new VanillaRecipeProvider.TrimTemplate(item, resourceKey, resourceKey2);
			});
	}
