	...
	protected void addTags(HolderLookup.Provider provider) {
		(new BlockItemTagsProvider() {
			...
			protected TagAppender<Block, Block> tag(TagKey<Block> tagKey, TagKey<Item> tagKey2) {
				return new VanillaItemTagsProvider.BlockToItemConverter(VanillaItemTagsProvider.this.tag(tagKey2));
			}
		}).run();
		...
		this.tag(ItemTags.TRIM_MATERIALS)
			.addAll(
				provider.lookupOrThrow(Registries.ITEM)
					.listElements()
					.filter(reference -> ((Item)reference.value()).components().has(DataComponents.PROVIDES_TRIM_MATERIAL))
