...
public class ActiveProfiler implements ProfileCollector {
	...
	public void pop() {
		if (!this.started) {
			...
			if (n > WARNING_TIME_NANOS && !this.suppressWarnings.getAsBoolean()) {
				LOGGER.warn(
					"Something's taking too long! '{}' took aprox {} ms", LogUtils.defer(() -> ProfileResults.demanglePath(this.path)), LogUtils.defer(() -> n / 1000000.0)
				);
			}
	...
	public void incrementCounter(Supplier<String> supplier, int i) {
		this.getCurrentEntry().counters.addTo((String)supplier.get(), i);
	}
