...
public interface StringRepresentable {
	...
	static <E extends Enum<E> & StringRepresentable> StringRepresentable.EnumCodec<E> fromEnumWithMapping(
		...
		Function<String, E> function2 = createNameLookup(enums, enum_ -> (String)function.apply(((StringRepresentable)enum_).getSerializedName()));
	...
	static <T extends StringRepresentable> Function<String, T> createNameLookup(T[] stringRepresentables) {
	...
	static <T> Function<String, T> createNameLookup(T[] objects, Function<T, String> function) {
		if (objects.length > 16) {
			Map<String, T> map = (Map<String, T>)Arrays.stream(objects).collect(Collectors.toMap(function, object -> object));
			return string -> string == null ? null : map.get(string);
		} else {
			return string -> {
				for (T object : objects) {
					if (((String)function.apply(object)).equals(string)) {
						return object;
					}
				}
