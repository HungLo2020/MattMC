...
public interface CubicSpline<C, I extends BoundedFloatFunction<C>> extends BoundedFloatFunction<C> {
	...
	static <C, I extends BoundedFloatFunction<C>> Codec<CubicSpline<C, I>> codec(Codec<I> codec) {
		...
		Codec<CubicSpline.Multipoint<C, I>> codec3 = RecordCodecBuilder.create(
			instance -> instance.group(
				...
				.apply(instance, (boundedFloatFunction, list) -> {
					...
					return CubicSpline.Multipoint.create((I)boundedFloatFunction, fs, builder.build(), gs);
				})
		);
	...
	static <C, I extends BoundedFloatFunction<C>> CubicSpline<C, I> constant(float f) {
		return new CubicSpline.Constant<>(f);
	}

	static <C, I extends BoundedFloatFunction<C>> CubicSpline.Builder<C, I> builder(I boundedFloatFunction) {
		return new CubicSpline.Builder<>(boundedFloatFunction);
	}

	static <C, I extends BoundedFloatFunction<C>> CubicSpline.Builder<C, I> builder(I boundedFloatFunction, BoundedFloatFunction<Float> boundedFloatFunction2) {
		return new CubicSpline.Builder<>(boundedFloatFunction, boundedFloatFunction2);
	}
	...
	public record Multipoint<C, I extends BoundedFloatFunction<C>>(
		...
		static <C, I extends BoundedFloatFunction<C>> CubicSpline.Multipoint<C, I> create(
			I boundedFloatFunction, float[] fs, List<CubicSpline<C, I>> list, float[] gs
		) {
			...
			return new CubicSpline.Multipoint<>(boundedFloatFunction, fs, list, gs, f, g);
		}
		...
		public float apply(C object) {
			...
			if (i < 0) {
				...
				BoundedFloatFunction<C> boundedFloatFunction2 = (BoundedFloatFunction<C>)this.values.get(i + 1);
		...
		private String toString(float[] fs) {
			return "["
				+ (String)IntStream.range(0, fs.length).mapToDouble(i -> fs[i]).mapToObj(d -> String.format(Locale.ROOT, "%.3f", d)).collect(Collectors.joining(", "))
				+ "]";
		}
