...
import net.voxelmap.ColorManager;
import net.voxelmap.VoxelConstants;
import net.voxelmap.WaypointManager;
import net.voxelmap.gui.overridden.GuiScreenMinimap;
import net.voxelmap.gui.overridden.IPopupGuiScreen;
import net.voxelmap.gui.overridden.Popup;
import net.voxelmap.gui.overridden.PopupGuiButton;
...
public class GuiAddWaypoint extends GuiScreenMinimap implements IPopupGuiScreen {
    ...
    public boolean mouseClicked(MouseButtonEvent mouseButtonEvent, boolean doubleClick) {
        ...
        if (this.choosingIcon && button == 0) {
            ...
            if (pickedIcon != chooser.getMissingImage()) {
                this.waypoint.imageSuffix = ((String) pickedIcon.getIconName()).replace("mattmc:images/waypoints/waypoint", "").replace(".png", "");
        ...
        if (this.choosingColor || this.choosingIcon) {
            ...
            if (this.choosingIcon) {
                ...
                if (pickedIcon != chooser.getMissingImage()) {
                    ...
                    this.tooltip = Component.translatable(((String) (pickedIcon.getIconName())).replace("mattmc:images/waypoints/", "voxelmap.minimap.waypoints.").replace(".png", ""));
                }
            }
        }
    ...
    private int pickColor(int width, int height, int mouseX, int mouseY) {
        ...
        if (mouseX >= anchorX && mouseX <= anchorX + width && mouseY >= anchorY && mouseY <= anchorY + height){
            ...
            // Check if colorPicker is loaded
            if (this.colorManager.getColorPicker() != null) {
