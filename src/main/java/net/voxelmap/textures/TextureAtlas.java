...
public class TextureAtlas extends AbstractTexture {
    ...
    public void stitch() {
        ...
        texture = net.vulkanic.VulkanicAPI.getDevice().createTexture("voxelmap-atlas", GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_COPY_SRC | GpuTexture.USAGE_TEXTURE_BINDING, TextureFormat.RGBA8, this.stitcher.getCurrentImageWidth(), this.stitcher.getCurrentImageHeight(), 1, 1);
        ...
        for (Sprite icon : this.stitcher.getStitchSlots()) {
            ...
            try {
                if (icon.getTextureData() != null) {
                    net.vulkanic.VulkanicAPI.getDevice().createCommandEncoder().writeToTexture(texture, icon.getTextureData(), 0, 0, icon.getOriginX(), icon.getOriginY(), icon.getIconWidth(), icon.getIconHeight(), 0, 0);
                }
