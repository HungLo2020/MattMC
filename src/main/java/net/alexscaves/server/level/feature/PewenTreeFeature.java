package net.alexscaves.server.level.feature;


import net.alexscaves.server.block.PewenBranchBlock;
...
public class PewenTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 0; i <= height; i++) {
            ...
            level.setBlock(trunkPos, i == height ? Blocks.PEWEN_WOOD.defaultBlockState() : Blocks.PEWEN_LOG.defaultBlockState(), 3);
            if (i == height) {
                level.setBlock(trunkPos.above(), Blocks.PEWEN_PINES.defaultBlockState(), 3);
            }
            if (i > j) {
                ...
                if (canReplace(level.getBlockState(offset))) {
                    level.setBlock(offset, Blocks.PEWEN_BRANCH.defaultBlockState().setValue(PewenBranchBlock.ROTATION, branchRot).setValue(PewenBranchBlock.PINES, true), 3);
                }
            }
        }
        return true;
    ...
    private void buildPenumbra(WorldGenLevel level, BlockPos trunkPos, int currentIndex, int maxIndex, RandomSource randomSource) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 1; j <= actualLength; j++) {
                ...
                if (canReplace(level.getBlockState(offset))) {
                    level.setBlock(offset, Blocks.PEWEN_BRANCH.defaultBlockState().setValue(PewenBranchBlock.ROTATION, i).setValue(PewenBranchBlock.PINES, j == actualLength), 3);
                }
            }
        }
    }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir() || state.canBeReplaced() || state.is(Blocks.PEWEN_BRANCH)) && !state.is(BlockTags.FEATURES_CANNOT_REPLACE) && state.getFluidState().isEmpty();
    }
}
