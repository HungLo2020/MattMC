...
public class AncientTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (Direction direction : Direction.values()) {
            if (direction.getAxis() != Direction.Axis.Y) {
                ...
                drawLeafOrb(level, canopyPos.relative(direction, branchOut).above(branchUp + 2), randomsource, Blocks.ANCIENT_LEAVES.defaultBlockState(), 2 + randomsource.nextInt(2), 4 + randomsource.nextInt(2), 2 + randomsource.nextInt(2));
            }
        }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir() || state.canBeReplaced() || state.is(Blocks.ANCIENT_LEAVES)) && !state.is(BlockTags.FEATURES_CANNOT_REPLACE) && state.getFluidState().isEmpty();
    }
