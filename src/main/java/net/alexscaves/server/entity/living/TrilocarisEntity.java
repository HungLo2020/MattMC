...
import net.alexscaves.server.entity.ai.SemiAquaticPathNavigator;
...
public class TrilocarisEntity extends WaterAnimal implements Bucketable {
    ...
    public static boolean checkTrilocarisSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (crawling || !this.isInWater()) {
                timeSwimming = 0;
                if(delta > 0.01F){
                    if(tickCount - lastStepSoundTimestamp > 10){
                        lastStepSoundTimestamp = tickCount;
                        this.playSound(SoundEvents.TRILOCARIS_STEP, 0.2F, 1.0F);
                    }
    ...
    public ItemStack getBucketItemStack() {
        ItemStack stack = new ItemStack(Items.TRILOCARIS_BUCKET);
    ...
    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return SoundEvents.TRILOCARIS_HURT;
    }

    protected SoundEvent getDeathSound() {
        return SoundEvents.TRILOCARIS_DEATH;
    }
    ...
    private class WanderGoal extends Goal {
        ...
        public BlockPos findWaterBlock() {
            ...
            for (int i = 0; i < 15; i++) {
                ...
                if (TrilocarisEntity.this.level().getFluidState(blockPos).is(FluidTags.WATER) && blockPos.getY() > level().getMinY() + 1) {
                    result = blockPos;
                }
            }
            return result;
        ...
        protected Vec3 getPosition() {
            ...
            if (TrilocarisEntity.this.isInWater()) {
                if (water == null) {
                    return null;
                }
                if (this.isCrawling) {
                    while (TrilocarisEntity.this.level().getFluidState(water.below()).is(FluidTags.WATER) && water.getY() > level().getMinY() + 1) {
                        water = water.below();
                    }
                    water = water.above();
                }
