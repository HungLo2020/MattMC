...
public class GrottoceratopsEntity extends DinosaurEntity implements IAnimatedEntity {
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if (!this.isBaby()) {
            this.playSound(SoundEvents.GROTTOCERATOPS_STEP, 0.7F, 0.85F);
        }
    }
    ...
    public void tick() {
        ...
        if (this.getAnimation() == ANIMATION_CHEW && this.getAnimationTick() == 2 || this.getAnimation() == ANIMATION_CHEW_FROM_GROUND && this.getAnimationTick() == 10) {
            this.playSound(SoundEvents.GROTTOCERATOPS_GRAZE);
        }
    ...
    public void actuallyPlayAmbientSound() {
        ...
        if (this.getAnimation() == ANIMATION_SPEAK_2) {
            soundevent = SoundEvents.GROTTOCERATOPS_CALL;
            volume += 1.0F;
        }
        if (soundevent != null) {
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.GROTTOCERATOPS_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return SoundEvents.GROTTOCERATOPS_HURT;
    }

    protected SoundEvent getDeathSound() {
        return SoundEvents.GROTTOCERATOPS_DEATH;
    }
