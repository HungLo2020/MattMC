package net.alexscaves.server.entity.living;

import net.alexscaves.server.entity.ai.*;
import net.alexscaves.server.entity.ai.AnimalBreedEggsGoal;
import net.alexscaves.server.entity.ai.AnimalLayEggGoal;
import net.alexscaves.server.entity.ai.AtlatitanMeleeGoal;
import net.alexscaves.server.entity.ai.AtlatitanNibbleTreesGoal;
import net.alexscaves.server.entity.util.KeybindUsingMount;
import net.alexscaves.server.misc.ACMath;
...
public class AtlatitanEntity extends SauropodBaseEntity implements KeybindUsingMount {
    ...
    protected void onStep() {
        if(!this.isBaby()){
            if (screenShakeAmount <= 1.0F) {
                this.playSound(SoundEvents.ATLATITAN_STEP, 2, 1);
            }
            if (screenShakeAmount <= 1.0F) {
    ...
    public void tick() {
        ...
        if (level().isClientSide()) {
            ...
            if(this.getRideableFor() > 0 && level().random.nextInt(2) == 0 && !this.isDancing()){
                ...
                // Use vanilla particle instead of HAPPINESS
                this.level().addParticle(ParticleTypes.HEART, particlePos.x, particlePos.y, particlePos.z, ((double) this.random.nextFloat() - 0.5D) * 0.1D, ((double) this.random.nextFloat() - 0.5D) * 0.1D, ((double) this.random.nextFloat() - 0.5D) * 0.1D);
            }
            // Client-side controls commented out - requires client hooks
            ...
            if(this.getAnimation() == ANIMATION_STOMP && this.getAnimationTick() == 30){
                this.playSound(SoundEvents.ATLATITAN_STOMP, 3.0F, 1.0F);
    ...
    public BlockPos getStandAtTreePos(BlockPos target) {
        ...
        // PEWEN_BRANCH replaced with OAK_LEAVES
        if (state.is(Blocks.OAK_LEAVES)) {
            // No rotation property for vanilla leaves
            dir = Direction.fromYRot(f);
        }
    ...
    public BlockState createEggBlockState() {
        // Egg block not implemented yet - return null
        return null;
    }
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.ATLATITAN_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return SoundEvents.ATLATITAN_HURT;
    }

    protected SoundEvent getDeathSound() {
        return SoundEvents.ATLATITAN_DEATH;
    }
