package net.alexscaves.client.model;

import net.alexscaves.client.render.ColorUtil;
import net.alexscaves.client.render.entity.VallumraptorRenderState;
...
public class VallumraptorModel extends AdvancedEntityModel<VallumraptorRenderState> {
    ...
    @Override
    public void setupAnim(VallumraptorRenderState renderState) {
        this.resetToDefaultPose();
        // Get animation data from render state
        Animation animation = renderState.animation;
        ...
        float sprintProgress = renderState.runProgress;
        float walkProgress = sprintProgress - 1F;
        float jumpProgress = renderState.leapProgress;
        ...
        float relaxedAmount = renderState.relaxedProgress;
        float sitAmount = relaxedAmount; // Simplified - would need getSitProgress in render state
        float puzzleRot = renderState.puzzledHeadYRot;
        ...
        if (animation != null && animation != IAnimatedEntity.NO_ANIMATION) {
            setupAnimForAnimation(renderState, animation, animationTick, limbSwing, limbSwingAmount, ageInTicks);
        }
        if (animation != VallumraptorEntity.ANIMATION_CALL_2) {
    ...
    private void setupAnimForAnimation(VallumraptorRenderState renderState, Animation animation, int animationTick, float limbSwing, float limbSwingAmount, float ageInTicks) {
