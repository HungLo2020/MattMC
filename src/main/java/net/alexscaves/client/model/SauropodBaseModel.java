...
import net.alexscaves.server.entity.living.SauropodBaseEntity;
...
public abstract class SauropodBaseModel<T extends LivingEntityRenderState> extends AdvancedEntityModel<T> {
    ...
    protected void setupAnimSauropod(AtlatitanRenderState renderState) {
        this.resetToDefaultPose();
    ...
    private void positionNeckAndTailFromRenderState(AtlatitanRenderState renderState) {
        if (!straighten && !renderState.isFakeEntity) {
            ...
            float pitchAmount = renderState.animation == SauropodBaseEntity.ANIMATION_SPEW_FLAMES ? 0.0F : Mth.clamp(renderState.headPitch, -30, 30) / 57.295776F;
