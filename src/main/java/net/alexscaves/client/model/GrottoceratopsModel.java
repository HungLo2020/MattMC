...
import net.alexscaves.client.render.entity.GrottoceratopsRenderState;
...
public class GrottoceratopsModel extends AdvancedEntityModel<GrottoceratopsRenderState> {
    ...
    private void setupAnimForAnimation(GrottoceratopsRenderState renderState, Animation animation, float limbSwing, float limbSwingAmount, float ageInTicks) {
        ...
        if (chewing) {
            float animationIntensity = ACMath.cullAnimationTick(renderState.animationTick, 3, animation, 0, animation == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND ? 15 : 0);
    ...
    @Override
    public void setupAnim(GrottoceratopsRenderState renderState) {
        ...
        float walkDegree = 1F;
        float buryEggsAmount = renderState.buryEggsProgress;
        float stillAmount = 1 - limbSwingAmount;
        float danceAmount = renderState.danceProgress;
        float danceSpeed = 0.5F;
        boolean showGrass = renderState.animation == GrottoceratopsEntity.ANIMATION_CHEW_FROM_GROUND && renderState.animationTick > 15 || renderState.animation == GrottoceratopsEntity.ANIMATION_CHEW;
        if (renderState.animation != IAnimatedEntity.NO_ANIMATION) {
            setupAnimForAnimation(renderState, renderState.animation, limbSwing, limbSwingAmount, ageInTicks);
        }
        float tailSwingYaw = Mth.wrapDegrees(renderState.tailSwingRot) / 57.295776F;
        ...
        // Note: legSolver articulation removed as it requires entity reference
        ...
        if (renderState.animation != GrottoceratopsEntity.ANIMATION_MELEE_TAIL_1 && renderState.animation != GrottoceratopsEntity.ANIMATION_MELEE_TAIL_2) {
