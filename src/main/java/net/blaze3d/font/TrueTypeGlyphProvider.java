...
public class TrueTypeGlyphProvider implements GlyphProvider {
	...
	public TrueTypeGlyphProvider(ByteBuffer byteBuffer, FT_Face fT_Face, float f, float g, float h, float i, String string) {
		...
		try (MemoryStack memoryStack = MemoryStack.stackPush()) {
			...
			while (true) {
				...
				m = (int)FreeType.FT_Get_Next_Char(fT_Face, m, intBuffer);
			}
		}
	}
	...
	@Nullable
	@Override
	public UnbakedGlyph getGlyph(int i) {
	...
	private UnbakedGlyph getOrLoadGlyphInfo(int i, TrueTypeGlyphProvider.GlyphEntry glyphEntry) {
		...
		if (unbakedGlyph == null) {
			...
			synchronized (fT_Face) {
				...
				if (unbakedGlyph == null) {
					unbakedGlyph = this.loadGlyph(i, fT_Face, glyphEntry.index);
	...
	private UnbakedGlyph loadGlyph(int i, FT_Face fT_Face, int j) {
		...
		if (fT_GlyphSlot == null) {
			...
			return (UnbakedGlyph)(n > 0 && o > 0 ? new TrueTypeGlyphProvider.Glyph(l, m, n, o, f, j) : new EmptyGlyph(f / this.oversample));
		}
	}
				...
				public void upload(int i, int j, GpuTexture gpuTexture) {
					...
					try (NativeImage nativeImage = new NativeImage(NativeImage.Format.LUMINANCE, Glyph.this.width, Glyph.this.height, false)) {
						if (nativeImage.copyFromFont(fT_Face, Glyph.this.index)) {
							net.vulkanic.VulkanicAPI.getDevice().createCommandEncoder().writeToTexture(gpuTexture, nativeImage, 0, 0, i, j, Glyph.this.width, Glyph.this.height, 0, 0);
						}
					}
				}
