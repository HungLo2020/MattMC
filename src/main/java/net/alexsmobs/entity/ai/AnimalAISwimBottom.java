...
public class AnimalAISwimBottom extends RandomStrollGoal {
    ...
    protected Vec3 getPosition() {
        ...
        for(int var2 = 0; vec != null && !this.mob.level().getBlockState(AMBlockPos.fromVec3(vec)).isPathfindable(PathComputationType.WATER) && var2++ < 10; vec = DefaultRandomPos.getPos(this.mob, 10, 7)) {
        }
        int yDrop = 1 + this.mob.getRandom().nextInt(3);
        if(vec != null){
            BlockPos pos = AMBlockPos.fromVec3(vec);
            while(this.mob.level().getFluidState(pos).is(FluidTags.WATER) && this.mob.level().getBlockState(pos).isPathfindable(PathComputationType.WATER) && pos.getY() > 1){
                pos = pos.below();
            }
            pos = pos.above();
            int yUp = 0;
