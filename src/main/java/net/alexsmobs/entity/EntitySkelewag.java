...
import net.alexsmobs.entity.ai.AnimalAIRandomSwimming;
import net.alexsmobs.entity.ai.AquaticMoveController;
import net.alexsmobs.entity.ai.EntityAINearestTarget3D;
import net.alexsmobs.entity.ai.SemiAquaticPathNavigator;
...
public class EntitySkelewag extends Monster implements IAnimatedEntity {
    ...
    public EntitySkelewag(EntityType<? extends EntitySkelewag> monster, Level level) {
        super(monster, level);
        this.xpReward = 10;
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, EntitySpawnReason spawnReasonIn) {
        return true; // Default spawn rules
    }
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.SKELEWAG_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return SoundEvents.SKELEWAG_HURT;
    }
    ...
    public void tick(){
        ...
        if(!isInWater()){
            if (this.onGround() && random.nextFloat() < 0.2F) {
                ...
                this.playSound(SoundEvents.SKELEWAG_HURT, this.getSoundVolume(), this.getVoicePitch());
            }
        }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, EntitySpawnReason reason, @Nullable SpawnGroupData spawnDataIn) {
        ...
        if(reason == EntitySpawnReason.STRUCTURE){
            // Home position restriction (structure spawns) - no API available, just skip
        }
