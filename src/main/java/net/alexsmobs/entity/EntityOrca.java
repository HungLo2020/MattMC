...
import net.alexsmobs.entity.ai.OrcaAIJump;
import net.alexsmobs.entity.ai.OrcaAIMeleeJump;
import net.alexsmobs.entity.ai.SwimmerJumpPathNavigator;
...
public class EntityOrca extends TamableAnimal implements IAnimatedEntity {
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, EntitySpawnReason spawnReasonIn) {
        return true; // Simplified spawn check for vanilla integration
    }

    public static AttributeSupplier.Builder createAttributes() {
        return TamableAnimal.createMobAttributes().add(Attributes.MAX_HEALTH, 60.0D).add(Attributes.FOLLOW_RANGE, 64.0D).add(Attributes.ARMOR, 0.0D).add(Attributes.ATTACK_DAMAGE, 10.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.7F).add(Attributes.MOVEMENT_SPEED, 1.35F);
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob p_241840_2_) {
        return EntityType.ORCA.create(serverWorld, EntitySpawnReason.BREEDING);
    }
