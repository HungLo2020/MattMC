...
public class EntityBunfungus extends PathfinderMob implements IAnimatedEntity {
    ...
    // Tag keys for bunfungus behavior
    private static final TagKey<EntityType<?>> BUNFUNGUS_IGNORES = TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.withDefaultNamespace("bunfungus_ignores"));
    private static final TagKey<EntityType<?>> BUNFUNGUS_IGNORE_AOE_ATTACKS = TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.withDefaultNamespace("bunfungus_ignore_aoe_attacks"));
    private static final TagKey<Item> BUNFUNGUS_FOODSTUFFS = TagKey.create(Registries.ITEM, ResourceLocation.withDefaultNamespace("bunfungus_foodstuffs"));
                ...
                if (this.getAnimationTick() == 5) {
                    if (dist < 3.5D && this.getAnimation() == ANIMATION_BELLY) {
                        for (final LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(2.0D))) {
                            if ((entity == target || entity instanceof Monster) && !entity.getType().is(BUNFUNGUS_IGNORE_AOE_ATTACKS)) {
                                flag = true;
                                launch(entity);
        ...
        if (this.level().isClientSide()) {
            if (isRabbitForm()){
                for (int i = 0; i < 3; i++) {
                    ...
                    this.level().addParticle(ParticleTypes.MYCELIUM, this.getRandomX(scale), this.getY(this.random.nextDouble() * scale), this.getRandomZ(scale), d0, d1, d2);
                }
            }
            ...
            if (isSleeping() && random.nextFloat() < 0.3F) {
                ...
                ParticleOptions data = random.nextFloat() < 0.3F ? ParticleTypes.MYCELIUM : ParticleTypes.BUBBLE_POP;
    ...
    public boolean isCarrot(ItemStack stack) {
        return stack.is(BUNFUNGUS_FOODSTUFFS);
    }
