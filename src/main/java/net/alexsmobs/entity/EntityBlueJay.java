...
public class EntityBlueJay extends Animal implements ITargetsDroppedItems{
    ...
    public static boolean checkBlueJaySpawnRules(EntityType type, LevelAccessor worldIn, EntitySpawnReason reason, BlockPos pos, RandomSource randomIn) {
        return Animal.isBrightEnoughToSpawn(worldIn, pos);
    }
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, EntitySpawnReason spawnReasonIn) {
        return true; // Simplified spawn check - was using AMEntityRegistry.rollSpawn
    }
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.BLUE_JAY_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return SoundEvents.BLUE_JAY_HURT;
    }
    ...
    public boolean canTargetItem(ItemStack stack) {
        return stack.has(net.minecraft.core.component.DataComponents.FOOD) || stack.is(ItemTags.PARROT_FOOD);
    }
    ...
    public void onGetItem(ItemEntity e) {
        ...
        if(itemThrower != null && e.getItem().is(ItemTags.PARROT_FOOD)){
            this.setLastFeederUUID(itemThrower.getUUID());
            this.setFeedTime(1200);
            this.stopRiding();
        }
        if(e.getOwner() != null && e.getItem().is(ItemTags.PARROT_FOOD)){
            this.setSingTime(40);
        }
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!type.consumesAction()) {
            if(itemstack.is(ItemTags.PARROT_FOOD) && this.getFeedTime() <= 0){
                this.heal(3);
            ...
            }else if(itemstack.is(ItemTags.PARROT_FOOD) && this.getSingTime() <= 0){
                this.heal(3);
                this.setSingTime(40);
                this.usePlayerItem(player, hand, itemstack);
                return InteractionResult.SUCCESS;
    ...
    public void handleEntityEvent(byte id) {
        if (id == 67 || id == 68) {
            // Client-side visual effects removed (was AlexsMobs.PROXY.onEntityStatus)
        } else {
        ...
        AIScatter() {
            ...
            this.targetEntitySelector = new Predicate<Entity>() {
                ...
                public boolean apply(@javax.annotation.Nullable Entity e) {
                    return e.isAlive() && e instanceof Player && !((Player) e).isCreative();
                }
            };
        }
