...
// Removed: import net.alexsmobs.client.particle.AMParticleRegistry;
// Removed: import net.alexsmobs.config.AMConfig;
import net.alexsmobs.entity.ai.*;
import net.alexsmobs.entity.ai.*;
import net.alexsmobs.entity.util.Maths;
// Removed: import net.alexsmobs.item.AMItemRegistry;
import net.alexsmobs.misc.AMBlockPos;
// Removed: import net.alexsmobs.misc.AMSoundRegistry;
...
// Removed: import net.neoforged.api.distmarker.Dist;
// Removed: import net.neoforged.api.distmarker.OnlyIn;
...
public class EntityCachalotWhale extends Animal {
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.CACHALOT_WHALE_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return SoundEvents.CACHALOT_WHALE_HURT;
    }
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput valueOutput) {
        ...
        valueOutput.putBoolean("Albino", this.isAlbino());
        valueOutput.putBoolean("Beached", this.isBeached());
        valueOutput.putBoolean("BeachedDespawnFlag", this.isDespawnBeach());
        valueOutput.putBoolean("GivenReward", this.hasRewardedPlayer);
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput valueInput) {
        ...
        this.setAlbino(valueInput.getBooleanOr("Albino", false));
        this.setBeached(valueInput.getBooleanOr("Beached", false));
        this.setDespawnBeach(valueInput.getBooleanOr("BeachedDespawnFlag", false));
        this.despawnDelay = valueInput.getIntOr("DespawnDelay", this.despawnDelay);
        this.hasRewardedPlayer = valueInput.getBooleanOr("GivenReward", false);
    ...
    public void breakBlock() {
        ...
        boolean flag = false;
        if (!this.level().isClientSide() && this.blockBreakCounter == 0) { // Removed NeoForge event check
            // STUB: Simplified - AMConfig.cachalotDestruction and AMTagRegistry not available
            final TagKey<Block> breakables = this.isCharging() && this.getTarget() != null ? BlockTags.MINEABLE_WITH_PICKAXE : BlockTags.ICE;
    ...
    private void spawnSpoutParticles() {
        if (this.isAlive()) {
            ...
            for (int j = 0; j < 5 + random.nextInt(4); ++j) {
                ...
                // STUB: Particle effect commented out - AMParticleRegistry.WHALE_SPLASH not available in vanilla
                // this.level().addParticle(ParticleTypes.SPLASH, true, false, this.headPart.getX() + extraX, this.headPart.getY() + this.headPart.getBbHeight(), this.headPart.getZ() + extraZ, motX * 0.1F + this.getDeltaMovement().x, 2F, motZ * 0.1F + this.getDeltaMovement().z);
            }
        }
    }
    ...
    protected float getSoundVolume() {
        return this.isSilent() ? 0 : 1.0F; // STUB: AMConfig.cachalotVolume not available, using 1.0
    }
        ...
        if (rewardPlayer != null && !hasRewardedPlayer && this.isInWater()) {
            ...
            if (this.distanceTo(rewardPlayer) < 10F) {
                if (!this.level().isClientSide()) {
                    ...
                    // STUB: AMItemRegistry.AMBERGRIS not available, using PRISMARINE_CRYSTALS
                    final ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(Items.PRISMARINE_CRYSTALS, 2 + random.nextInt(2)));
        ...
        if (!this.level().isClientSide()) {
            ...
            if (target == null || !target.isAlive()) {
                ...
                if (isGrabbing() && this.getTarget().isAlive()) {
                    ...
                    // TODO: EntityGiantSquid not yet implemented
                    ...
                    if (waitForEchoFlag && !receivedEcho) {
                        ...
                        if (echoTimer % 10 == 0) {
                            if (echoTimer % 40 == 0) {
                                this.playSound(SoundEvents.CACHALOT_WHALE_CLICK, this.getSoundVolume(), this.getVoicePitch());
                            ...
                            if (this.isCharging()) {
                                if (this.distanceTo(target) < this.getBbWidth() && chargeProgress > 4) {
                                    ...
                                    if (target.getVehicle() instanceof final Boat boat) {
                                        // Drop oak planks and sticks when breaking boats
                                        if (this.level() instanceof ServerLevel serverLevel) {
                                            for (int i = 0; i < 3; ++i) {
                                                this.spawnAtLocation(serverLevel, Items.OAK_PLANKS);
                                            }
                                            for (int j = 0; j < 2; ++j) {
                                    ...
                                    if (random.nextInt(10) == 0) {
                                        ...
                                        // STUB: AMItemRegistry.CACHALOT_WHALE_TOOTH not available, using BONE
                                        ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(Items.BONE));
                                        itementity.setDefaultPickUpDelay();
                                        level().addFreshEntity(itementity);
                                    }
                                }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        EntityCachalotWhale whale = EntityType.CACHALOT_WHALE.create(serverWorld, EntitySpawnReason.BREEDING);
        if (whale != null) {
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, EntitySpawnReason spawnReasonIn) {
        return true; // STUB: AMEntityRegistry.rollSpawn not available, always return true
    }
