...
public class EntitySnowLeopard extends Animal implements IAnimatedEntity, ITargetsDroppedItems {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createAnimalAttributes().add(Attributes.MAX_HEALTH, 30D).add(Attributes.ATTACK_DAMAGE, 6.0D).add(Attributes.MOVEMENT_SPEED, 0.35F).add(Attributes.FOLLOW_RANGE, 64F).add(Attributes.STEP_HEIGHT, 1.0D);
    }

    protected SoundEvent getAmbientSound() {
        return SoundEvents.SNOW_LEOPARD_IDLE;
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return SoundEvents.SNOW_LEOPARD_HURT;
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        return EntityType.SNOW_LEOPARD.create(serverWorld, EntitySpawnReason.BREEDING);
    }
