...
import net.alexsmobs.client.render.OrcaRenderState;
import net.alexsmobs.entity.EntityOrca;
...
public class ModelOrca extends AdvancedEntityModel<OrcaRenderState> {
    ...
    private static double horizontalMag(double deltaX, double deltaZ) {
    ...
    public void setupAnim(OrcaRenderState state) {
        this.resetToDefaultPose();
        animate(state.currentAnimation, state.animationTick);
        ...
        if (horizontalMag(state.xd, state.zd) > 1.0E-7D) {
