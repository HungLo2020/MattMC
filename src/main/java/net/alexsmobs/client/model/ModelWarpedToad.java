...
import net.alexsmobs.client.render.WarpedToadRenderState;
...
public class ModelWarpedToad extends AdvancedEntityModel<WarpedToadRenderState> {
    ...
    @Override
    public void setupAnim(WarpedToadRenderState state) {
        ...
        float blinkProgress = Math.max(attackProgress, state.blinkProgress);
        float sitProgress = state.sitProgress;
        float swimProgress = state.swimProgress;
        float fallProgress = state.reboundProgress;
        float jumpProgress = Math.max(0, state.jumpProgress - fallProgress);
        float glowyBob = 0.75F + (Mth.cos(state.ageInTicks * 0.2F) + 1F) * 0.125F + attackProgress * 0.1F;
        float toungeScale = 1F + attackProgress * 0.285F * state.tongueLength;
        ...
        float walkSwingAmount = state.walkAnimationSpeed * walkAmount;
        float swimSwingAmount = state.walkAnimationSpeed * 0.2F * swimProgress;
        this.swing(right_leg, swimSpeed, swimDegree, false, -2.5F, -0.3F, state.walkAnimationPos, swimSwingAmount);
        this.swing(left_leg, swimSpeed, swimDegree, true, -2.5F, -0.3F, state.walkAnimationPos, swimSwingAmount);
        this.flap(right_foot, swimSpeed, swimDegree, false, -1, -0.3F, state.walkAnimationPos, swimSwingAmount);
        this.flap(left_foot, swimSpeed, swimDegree, true, -1, -0.3F, state.walkAnimationPos, swimSwingAmount);
        this.flap(right_arm, swimSpeed, swimDegree, false, -2.5F, -0.1F, state.walkAnimationPos, swimSwingAmount);
        this.flap(left_arm, swimSpeed, swimDegree, true, -2.5F, -0.1F, state.walkAnimationPos, swimSwingAmount);
        this.swing(right_arm, swimSpeed, swimDegree, false, -2.5F, 0.3F, state.walkAnimationPos, swimSwingAmount);
        this.swing(left_arm, swimSpeed, swimDegree, true, -2.5F, 0.3F, state.walkAnimationPos, swimSwingAmount);
        this.swing(body, swimSpeed, swimDegree * 0.1F, false, 0, 0F, state.walkAnimationPos, swimSwingAmount);
        ...
        this.flap(body, walkSpeed, walkDegree * 0.35F, false, 0F, 0F, state.walkAnimationPos, walkSwingAmount);
        this.swing(body, walkSpeed, walkDegree * 0.35F, false, 1F, 0F, state.walkAnimationPos, walkSwingAmount);
        this.walk(left_arm, walkSpeed, walkDegree, false, -2.5F, -0.3F, state.walkAnimationPos, walkSwingAmount);
        this.walk(right_arm, walkSpeed, walkDegree, true, -2.5F, 0.3F, state.walkAnimationPos, walkSwingAmount);
        this.walk(right_leg, walkSpeed, walkDegree, false, -2.5F, 0.1F, state.walkAnimationPos, walkSwingAmount);
        this.walk(left_leg, walkSpeed, walkDegree, true, -2.5F, -0.1F, state.walkAnimationPos, walkSwingAmount);
        ...
        this.left_arm.rotationPointZ += 1.5F * (float) (Math.sin((double) (state.walkAnimationPos * walkSpeed) - 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        this.left_arm.rotationPointY += 0.5F * (float) (Math.sin((double) (state.walkAnimationPos * walkSpeed) - 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        this.right_arm.rotationPointZ += 1.5F * (float) (Math.sin(-(double) (state.walkAnimationPos * walkSpeed) + 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        this.right_arm.rotationPointY += 0.5F * (float) (Math.sin(-(double) (state.walkAnimationPos * walkSpeed) + 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        float leftLegS = (float) (Math.sin((double) (state.walkAnimationPos * walkSpeed) - 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        float rightLegs = (float) (Math.sin(-(double) (state.walkAnimationPos * walkSpeed) + 2.5F) * (double) walkSwingAmount * (double) walkDegree - (double) (walkSwingAmount * walkDegree));
        ...
        if (attackProgress > 0) {
            this.tongue.rotateAngleX += state.xRot * Mth.DEG_TO_RAD;
        }
