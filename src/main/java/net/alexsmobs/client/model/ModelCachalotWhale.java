package net.alexsmobs.client.model;// Made with Blockbench 3.8.3
...
public class ModelCachalotWhale extends AdvancedEntityModel<CachalotWhaleRenderState> {
	...
	@Override
	public void setupAnim(CachalotWhaleRenderState renderState){
		this.resetToDefaultPose();
		float ageInTicks = renderState.ageInTicks;
		float chargeProgress = renderState.chargeProgress;
		float sleepProgress = renderState.sleepProgress;
		float beachedProgress = renderState.beachedProgress;
		...
		float f = Mth.clamp((float)renderState.movementOffsets[7][0] - renderYaw, -50, 50);
		this.tail1.rotateAngleY += (float) Mth.clamp((float)renderState.movementOffsets[15][0] - renderYaw, -50, 50)  * 0.017453292F;
		this.tail2.rotateAngleY += (float) Mth.clamp((float)renderState.movementOffsets[17][0] - renderYaw, -50, 50)  * 0.017453292F;
		this.body.rotateAngleX += Math.min(properPitch, sleepProgress * -9) * Mth.DEG_TO_RAD;
		this.body.rotateAngleZ += f * 0.017453292F;
		this.head.rotateAngleY += Math.sin((grabTime + ageInTicks - (int)ageInTicks) * 0.3F) * 0.1F * grabProgress;
