...
public final class TradeOfferInternals {
	...
	private static void initVillagerTrades() {
		if (!(MerchantOffers.REBALANCED_PROFESSION_TO_LEVELED_TRADE instanceof HashMap)) {
			Map<ResourceKey<VillagerProfession>, Int2ObjectMap<VillagerTrades.ItemListing[]>> map = new HashMap<>(MerchantOffers.REBALANCED_PROFESSION_TO_LEVELED_TRADE);

			for (Map.Entry<ResourceKey<VillagerProfession>, Int2ObjectMap<VillagerTrades.ItemListing[]>> trade : MerchantOffers.PROFESSION_TO_LEVELED_TRADE.entrySet()) {
