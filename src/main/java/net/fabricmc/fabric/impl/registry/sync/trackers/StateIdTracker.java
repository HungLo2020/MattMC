...
public final class StateIdTracker<T, S> implements RegistryIdRemapCallback<T>, RegistryEntryAddedCallback<T> {
	...
	public static <T, S> void register(Registry<T> registry, IdMap<S> stateList, Function<T, Collection<S>> stateGetter) {
		if (!TRACKED.add(registry.getKey().location())) {
			throw new IllegalStateException("Trying to register a tracker for registry " + registry.getKey().location() + " more than once!");
		}
