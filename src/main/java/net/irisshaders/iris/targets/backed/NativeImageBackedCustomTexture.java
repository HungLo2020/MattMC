...
public class NativeImageBackedCustomTexture extends DynamicTexture implements TextureAccess {
	public NativeImageBackedCustomTexture(CustomTextureData.PngData textureData) throws IOException {
		...
		if (net.vulkanic.VulkanicAPI.isVulkanBackendSelected()
			...
			throw new IllegalStateException("Java Iris custom textures are unavailable on selected Vulkan");
		}
	...
	public void upload() {
		if (net.vulkanic.VulkanicAPI.isVulkanBackendSelected()
			|| net.vulkanic.bridge.RustGalVulkanWholeFrameMode.enabled()) {
			throw new IllegalStateException("Java Iris custom texture uploads are unavailable on selected Vulkan");
		}
