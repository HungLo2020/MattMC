...
public class FallbackShader extends GlProgram implements IrisProgram {
	...
	public void iris$setupState() {
		...
		if (FOG_DENSITY > -1 && FOG_IS_EXP2 > -1) {
			...
			if (fogDensity >= 0.0) {
				VulkanicAPI.assignUniformFloat(CTX, FOG_DENSITY, fogDensity);
				VulkanicAPI.assignUniformInteger(CTX, FOG_IS_EXP2, 1);
			} else {
				VulkanicAPI.assignUniformFloat(CTX, FOG_DENSITY, 0.0f);
				VulkanicAPI.assignUniformInteger(CTX, FOG_IS_EXP2, 0);
			}
		}
