...
public final class NativeSectionMeshBuilder implements AutoCloseable {
    ...
    private static final MethodHandle APPEND_STATIC_MODEL_BATCH_ENCODED = NativeLibraryLoader.downcallHandle("mattmc_rust",
            ...
            FunctionDescriptor.of(ValueLayout.JAVA_INT,
                    ...
                    ValueLayout.JAVA_INT,
                    ValueLayout.ADDRESS));
    // Legacy 316-byte records are retained for benchmark/test replay only; production uses the compact all-pass snapshot.
    private static final MethodHandle APPEND_LEGACY_NATIVE_SECTION_RECORDS_ENCODED = NativeLibraryLoader.downcallHandle("mattmc_rust",
    ...
    public int appendLegacyNativeSectionRecordsEncoded(long recordAddress, int blockCount, int passId,
            NativeChunkVertexFormat format, int sectionIndex, boolean separateAo, boolean storeRawQuads) {
        return this.appendLegacyNativeSectionRecordsEncoded(recordAddress, blockCount, passId, format, sectionIndex, separateAo,
                storeRawQuads, 0L);
    }
    ...
    public int appendLegacyNativeSectionRecordsEncoded(long recordAddress, int blockCount, int passId,
        ...
        if (blockCount < 0) {
            throw new IllegalArgumentException("Invalid legacy native section block count: " + blockCount);
        }
        if (blockCount == 0) {
            return 0;
        }
        ...
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment committedCountSegment = arena.allocate(ValueLayout.JAVA_INT);
            check(invokeAppendLegacyNativeSectionRecordsEncoded(this.state.getHandle(), recordAddress, blockCount,
                    NativeChunkMeshEncoder.LEGACY_NATIVE_SECTION_BLOCK_RECORD_STRIDE, passId,
                    NativeChunkMeshEncoder.NATIVE_QUAD_STRIDE, format.stride(), format.blockIdOffset(),
                    format.normalOffset(), format.tangentOffset(), format.midUvOffset(), format.midBlockOffset(),
                    sectionIndex, separateAo ? 1 : 0, storeRawQuads ? 1 : 0, translucentAnalyzerHandle,
                    committedCountSegment),
                    "native section mesh builder legacy native section encoded append");
            return committedCountSegment.get(ValueLayout.JAVA_INT, 0);
        }
    }
    ...
    private static int invokeAppendLegacyNativeSectionRecordsEncoded(long handle, long recordAddress, int blockCount,
        ...
        try {
            return (int) APPEND_LEGACY_NATIVE_SECTION_RECORDS_ENCODED.invokeExact(handle, recordAddress, blockCount, recordStride,
