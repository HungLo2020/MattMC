...
public class GlProgram<T> extends GlObject implements ShaderBindingContext {
    ...
    public void delete() {
        VulkanicAPI.disposeProgramObject(CTX, this.handle());
    ...
    public @NotNull GlUniformBlock bindUniformBlock(String name, int bindingPoint) {
        int index = VulkanicAPI.locateUniformBlock(CTX, this.handle(), name);

        if (index < 0) {
            throw new NullPointerException("No uniform block exists with name: " + name);
        }

        VulkanicAPI.bindUniformBlock(CTX, this.handle(), index, bindingPoint);
    ...
    public static class Builder {
        ...
        public Builder(ResourceLocation name) {
            this.name = name;
            this.program = VulkanicAPI.constructProgramObject(CTX);
        }
        ...
        public <U> GlProgram<U> link(Function<ShaderBindingContext, U> factory) {
            ...
            int result = VulkanicAPI.queryProgramParameter(CTX, this.program, VulkanicAPI.GL_LINK_STATUS);
