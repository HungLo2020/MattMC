...
/**
 * Native Vulkan command encoder for Sodium chunk terrain.
 *
 * <p>This is deliberately narrower than the compatibility {@link net.blaze3d.opengl.GlCommandEncoder}.
 * It covers the terrain hot path that already uses the Vulkan renderer branch, while unrelated
 * callers continue to use the existing global command encoder until they are migrated one by one.</p>
 */
final class VulkanNativeTerrainCommandEncoder implements CommandEncoder {
    ...
    public GpuBuffer.MappedView mapBuffer(GpuBufferSlice slice, boolean read, boolean write) {
        ...
        if (mapped == null) {
            throw new IllegalStateException("Unable to map Vulkan terrain buffer");
        }
    ...
    private VulkanicTextureView createTextureView(GpuTextureView view) {
        if (!(view.texture() instanceof VulkanicTexture texture)) {
            throw new IllegalArgumentException("Terrain render-pass texture is not VulkanicTexture: " + view.texture().getClass().getName());
        }
    ...
    private void ensureNoRenderPass() {
        if (this.inRenderPass) {
            throw new IllegalStateException("Close the existing Vulkan terrain render pass before issuing another encoder command");
        }
    }

    private void unsupported(String operation) {
        throw new UnsupportedOperationException(
            "Vulkan native terrain command encoder does not support " + operation
                + "; migrate that callsite through an explicit native Vulkan encoder slice first."
        );
    }

    private final class TerrainRenderPass implements RenderPass, RenderPassResourceBinder {
        ...
        public void setPipeline(RenderPipeline renderPipeline) {
            ...
            this.pipelineDescriptor = VulkanNativeTerrainCommandEncoder.this.backend.resolvePrecompiledPipelineDescriptor(renderPipeline);
        ...
        public void drawIndexed(int baseVertex, int firstIndex, int indexCount, int instanceCount) {
            ...
            if (this.indexBuffer == null) {
                throw new IllegalStateException("Can't draw indexed terrain without an index buffer");
            }
        ...
        public <T> void drawMultipleIndexed(
            ...
            this.unsupported("drawMultipleIndexed");
        }
        ...
        public void close() {
            ...
            try {
                this.pass.close();
                VulkanNativeTerrainCommandEncoder.this.backend.submitCommandBuffer(this.ctx);
            } finally {
                VulkanNativeTerrainCommandEncoder.this.inRenderPass = false;
        ...
        private void bindPipelineAndResources() {
            ...
            VulkanNativeTerrainCommandEncoder.this.backend.bindPipelineResources(this.ctx, handle, submission.descriptor(), submission.bindings());
        }
        ...
        private RenderPipeline requirePipeline() {
            if (this.renderPipeline == null) {
                throw new IllegalStateException("Can't draw terrain without a render pipeline");
            }
        ...
        private PipelineDescriptor requirePipelineDescriptor() {
            if (this.pipelineDescriptor == null) {
                throw new IllegalStateException("No pipeline descriptor is available for terrain render pass");
            }
        ...
        private void unsupported(String operation) {
            throw new UnsupportedOperationException("Vulkan native terrain render pass does not support " + operation);
        }
