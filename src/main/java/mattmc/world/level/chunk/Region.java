 ...
 */
public class Region implements LevelAccessor {
    public static final int REGION_SIZE = 32; // 32x32 chunks
    public static final int REGION_WIDTH_BLOCKS = REGION_SIZE * LevelChunk.WIDTH; // 512 blocks
    public static final int REGION_DEPTH_BLOCKS = REGION_SIZE * LevelChunk.DEPTH; // 512 blocks
    ...
    public Region(int regionX, int regionZ) {
        ...
        this.chunks = new LevelChunk[REGION_SIZE][REGION_SIZE];
        ...
        for (int cx = 0; cx < REGION_SIZE; cx++) {
            for (int cz = 0; cz < REGION_SIZE; cz++) {
                ...
                chunks[cx][cz] = new LevelChunk(chunkX, chunkZ);
            }
        }
    }
     ...
     * @param y 0-383 (chunk-local Y, world Y = y + LevelChunk.MIN_Y)
    ...
    public Block getBlock(int x, int y, int z) {
        ...
        int chunkX = x / LevelChunk.WIDTH;
        int chunkZ = z / LevelChunk.DEPTH;
        int blockX = x % LevelChunk.WIDTH;
        int blockZ = z % LevelChunk.DEPTH;
