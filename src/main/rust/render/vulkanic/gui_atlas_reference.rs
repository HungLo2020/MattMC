...
impl GuiAtlasReference {
    pub(crate) fn validate(self) -> GalResult<()> {
        if self.asset_id == 0
            || self.atlas.texture_id == 0
            || self.atlas.generation == 0
            || self.width == 0
            || self.height == 0
            || self.atlas.width == 0
            || self.atlas.height == 0
                ...
                .is_none_or(|end| end > self.atlas.height)
        {
            return Err(GalError::ffi(
                ...
                "invalid GUI atlas reference extent or identity",
            ));
        }
        Ok(())
    }
...
impl GuiAtlasReferences {
    pub(crate) fn contains(&self, asset: u64) -> bool {
    ...
    pub(crate) fn replace(
        ...
        copied_image_ids: &BTreeSet<u64>,
        mut accepted: impl FnMut(u32) -> Option<AcceptedAtlasIncarnation>,
    ) -> GalResult<()> {
        ...
        for reference in references {
            reference.validate()?;
            if copied_image_ids.contains(&reference.asset_id)
                || next.insert(reference.asset_id, *reference).is_some()
            {
                return Err(GalError::ffi(
                    StatusCode::InvalidArgument,
                    "GUI atlas reference image identity collision",
                ));
            }
            if accepted(reference.atlas.texture_id) != Some(reference.atlas) {
                return Err(GalError::ffi(
                    StatusCode::StaleHandle,
                    "GUI atlas incarnation is not accepted",
                ));
            }
        }
        if revision == self.revision && next != self.entries {
            return Err(GalError::ffi(
                StatusCode::InvalidArgument,
                "GUI atlas reference revision reused with different data",
            ));
        }
        self.entries = next;
    ...
    pub(crate) fn resolve(
        &self,
        asset: u64,
        accepted: impl FnOnce(u32) -> Option<AcceptedAtlasIncarnation>,
    ) -> GalResult<GuiAtlasReference> {
        ...
        if accepted(reference.atlas.texture_id) != Some(reference.atlas) {
            return Err(GalError::ffi(
                StatusCode::StaleHandle,
                "GUI atlas incarnation retired before consumption",
            ));
        }
        Ok(reference)
    }
...
mod tests {
    ...
    fn exact_incarnation_and_retirement_are_checked_without_gpu_handles() {
        let r = reference();
        let mut table = GuiAtlasReferences::default();
        table
            .replace(1, &[r], &BTreeSet::new(), |_| Some(r.atlas))
            .unwrap();
        assert_eq!(table.resolve(101, |_| Some(r.atlas)).unwrap(), r);
        assert!(table.resolve(101, |_| None).is_err());
        assert!(table
            .resolve(101, |_| Some(AcceptedAtlasIncarnation {
                generation: 4,
                ..r.atlas
            }))
            .is_err());
    ...
    fn failed_transactions_preserve_old_binding_and_revision_for_retry() {
        ...
        assert!(table
            .replace(2, &[next, next], &BTreeSet::new(), |_| Some(r.atlas))
            .is_err());
        assert!(table
            .replace(2, &[next], &BTreeSet::from([102]), |_| Some(r.atlas))
            .is_err());
        assert!(table
            .replace(2, &[next], &BTreeSet::new(), |_| None)
            .is_err());
        assert!(table.contains(101));
        assert!(!table.contains(102));
        table
            .replace(2, &[next], &BTreeSet::new(), |_| Some(r.atlas))
            .unwrap();
        table
            .replace(2, &[next], &BTreeSet::new(), |_| Some(r.atlas))
            .unwrap();
        ...
        assert!(table
            ...
            .is_err());
        assert!(table
            .replace(2, &[r], &BTreeSet::new(), |_| Some(r.atlas))
            .is_err());
    }
    #[test]
    fn bounded_count_extents_and_uv_orientation() {
        ...
        assert_eq!(calls, 0);
        for bad in [
            GuiAtlasReference { x: u32::MAX, ..r },
            GuiAtlasReference { width: 0, ..r },
            GuiAtlasReference { height: 100, ..r },
            GuiAtlasReference { asset_id: 0, ..r },
        ] {
            assert!(table
                .replace(1, &[bad], &BTreeSet::new(), |_| Some(r.atlas))
                .is_err());
        }
