...
const ITEM_ORIENTATIONS: [[i64; 6]; 8] = [
    [0, 0, 16, 0, 0, 16],
    [16, 0, 0, 0, 16, 16],
    [0, 16, 16, 16, 0, 0],
    [16, 16, 0, 16, 16, 0],
...
const FROZEN_TINTED_CENTERS: [[u8; 3]; 9] = [
    [35, 105, 24],
    [9, 209, 49],
    [17, 53, 97],
    [35, 209, 97],
...
fn tinted_item_pixel(
    ...
    let (source, alpha) =
        if transparency && alpha_range.contains(&row) && alpha_range.contains(&column) {
            if column < size / 4 {
                return [0; 4];
            }
            ([255.0; 3], 26_u8)
            ...
            let index = ITEM_TINTS
                .iter()
                .position(|candidate| *candidate == tint)
                .unwrap();
...
fn vulkan_item_raster_uses_owned_atlas_at_observed_scale_and_retires_all_resources() {
    for north_face in [false, true] {
        for bottom_up in [false, true] {
            raster_and_composite(north_face, bottom_up, 0, false, 3);
        }
    }
...
fn raster_and_composite_size(
    ...
    raster_and_composite_animation(
        north_face,
        bottom_up,
        retained_slots,
        transparency,
...
fn raster_and_composite_animation(
    north_face: bool,
    bottom_up: bool,
    retained_slots: u32,
    transparency: bool,
    ...
    for (x, y) in regions {
        for row in 0..sprite_size {
            for column in 0..sprite_size {
                let value = if transparency
                    && alpha_range.contains(&row)
                    && alpha_range.contains(&column)
                {
                    ...
                    [255, 255, 0, 255]
                } else if row == sprite_size - 1 || column == sprite_size - 1 {
                    [17, 106, 49, 255]
                } else {
                    [35, 212, 98, 255]
                };
    ...
    let incarnation = world
        ...
        .unwrap();
    let references: Vec<_> = regions
        .iter()
        .enumerate()
        .map(|(index, &(x, y))| GuiAtlasReference {
            asset_id: index as u64 + 1,
            atlas: incarnation,
            x: x as u32,
            y: y as u32,
            width: sprite_size as u32,
            height: sprite_size as u32,
        })
        .collect();
    frontend
        .stage_owned_atlas_references(&mut gal, &world, 1, &references)
        .unwrap();
    let extent = Extent3d {
        width: 512,
        height: 512,
    ...
    assert!(GuiItemRasterTarget::create(
        &mut gal,
        Extent3d {
            width: 4097,
            height: 512,
    ...
    let readback = gal
        .create_buffer(BufferDesc {
            label: "item-raster.readback".into(),
            size: 512 * 512 * 4,
        ...
        .map(|index| {
            ...
            (
                ...
                GuiAffineQuadRequest {
                    ...
                    material: {
                        let lighting = GuiFlatItemLighting {
                            lightmap_generation: 1,
                            rgb: [252.0 / 255.0; 3],
                        };
                        if transparency && index == 6 {
                            GuiAffineMaterial::FlatItemCutout(lighting)
                        } else {
                            GuiAffineMaterial::FlatItem(lighting)
                        }
                    },
                    stratum: 1,
                    asset_id: index as u64 + 1,
                    x0: if north_face { 16.0 } else { 0.0 },
                    y0: 0.0,
                    x1: if north_face { 0.0 } else { 16.0 },
                    y1: 0.0,
                    x3: if north_face { 16.0 } else { 0.0 },
                    y3: 16.0,
                    z: 0.0,
                    u0: if north_face { 1.0 } else { 0.0 },
                    v0: 0.0,
                    u1: if north_face { 0.0 } else { 1.0 },
                    v1: 1.0,
                    color_argb: 0xffffffff,
                    gui_width: 16,
                    gui_height: 16,
                    projection_extent: [16.0; 2],
    ...
    let creates = gal.metrics().resource_creates;
    assert!(frontend
        .append_owned_item_raster_quads(&mut gal, &mut world, pass, target, view, &invalid)
        .is_err());
    assert_eq!(creates, gal.metrics().resource_creates);
    ...
    ops.extend(
        frontend
            .append_owned_item_raster_quads(&mut gal, &mut world, pass, target, view, &requests)
            .unwrap(),
    );
    assert!(
        frontend.raw_images.is_empty(),
        "the raster stage must sample the world-owned atlas"
    );
    ops.extend([
        CommandOp::Barrier(texture_barrier(
            color,
    ...
    let list = gal
        .create_command_list(CommandListDesc {
            label: "item-raster.commands".into(),
            operations: ops,
        })
        .unwrap();
    let token = gal
        .submit(SubmissionBatch {
            label: "item-raster.submit".into(),
    ...
    for index in 0..9 {
        ...
        if transparency {
            assert_eq!(
                pixel(x + 3 * scale as usize, 3 * scale as usize),
                [0, 0, 0, 0],
                "25/255 must be discarded before blending"
            );
            assert_eq!(
                pixel(x + 5 * scale as usize, 3 * scale as usize),
                if index == 6 {
                    [252, 252, 252, 26]
                } else {
                    [26, 26, 26, 26]
        ...
        let rows: Vec<_> = (0..cell)
            .filter(|&y| y == 0 || pixel(x + cell / 2, y) != pixel(x + cell / 2, y - 1))
            .map(|y| (y, pixel(x + cell / 2, y).to_vec()))
            .collect();
        ...
        let expected = if sprite_size == 16 {
            vec![0, scale as usize, 15 * scale as usize]
        } else if scale == 1 {
            vec![0, 15]
        } else if scale == 2 {
            vec![0, 1, 31]
            ...
            vec![0, 2, 47]
        } else if retained_slots == 9 && ![0, 5, 7].contains(&index) {
            vec![0, 2, 46]
        } else {
            vec![0, 1, 46]
        };
    ...
    let output = gal
        .create_texture(TextureDesc {
            label: "item-composite.color".into(),
    ...
    let output_view = gal
        .create_texture_view(TextureViewDesc {
            label: "item-composite.view".into(),
            texture: output,
            format: TextureFormat::Rgba8Unorm,
            base_mip: 0,
            mip_count: 1,
        ...
        .unwrap();
    let output_target = gal
        .create_render_target(RenderTargetDesc {
            label: "item-composite.target".into(),
            color_views: vec![output_view],
    ...
    let output_pass = gal
        .create_render_pass(RenderPassDesc {
            label: "item-composite.pass".into(),
    ...
    for (index, (placement, local)) in requests.iter().enumerate() {
        ...
        screen.y0 = 100.0;
        screen.y1 = 100.0;
        screen.y3 = 100.0 + cell as f32;
        screen.projection_extent = [512.0; 2];
        composite
            .append_item_raster_composite(
                color,
                if index == 0 {
                    TextureUsageState::TransferSrc
                } else {
                ...
                *placement,
                ...
                &screen,
                false,
                index as u64 * GUI_MESH_COMPOSITE_UNIFORM_STRIDE,
                &mut ops,
            )
            .unwrap();
    }
    ...
    let list = gal
        .create_command_list(CommandListDesc {
            label: "item-composite.commands".into(),
            operations: ops,
        })
        .unwrap();
    let token = gal
        .submit(SubmissionBatch {
            label: "item-composite.submit".into(),
            command_lists: vec![list],
        })
        .unwrap();
    gal.retire_through_for_test(token.submission).unwrap();
    let composite_reads = gal.completed_host_reads();
    let output_bytes = &composite_reads
        .iter()
        .rev()
        .find(|r| r.buffer == readback)
        .unwrap()
        .bytes;
    for y in 0..cell {
        for x in 0..9 * cell {
            ...
            assert_eq!(
                &output_bytes[((y + 100) * 512 + x) * 4..((y + 100) * 512 + x + 1) * 4],
                expected,
    ...
    if bottom_up && !north_face && retained_slots == 0 && scale == 2 && !interpolate {
        ...
        let groups: Vec<_> = requests
            ...
            .map(|(index, (_, local))| {
                ...
                presentation.x0 = index as f32 * 32.0;
                presentation.x1 = presentation.x0 + 32.0;
                presentation.x3 = presentation.x0;
                presentation.y0 = 100.0;
                presentation.y1 = 100.0;
                presentation.y3 = 132.0;
                presentation.gui_width = 512;
                presentation.gui_height = 512;
                presentation.projection_extent = [512.0; 2];
                let colors = if index % 2 == 0 {
                    [0x80ff0000, 0x8000ff00]
                } else {
                    [0x8000ff00, 0x80ff0000]
                };
                GuiItemRasterGroup {
                    presentation,
                    layers: colors
                        .into_iter()
                        .map(|color_argb| GuiItemRasterLayer {
                            asset_id: 1,
                            color_argb,
                            material: GuiAffineMaterial::FlatItem(GuiFlatItemLighting {
                                lightmap_generation: 1,
                                rgb: [1.0; 3],
                            }),
        ...
        for defect in 0..9 {
            ...
            match defect {
                ...
                6 => {
                    invalid[8].layers[1].material =
                        GuiAffineMaterial::FlatItem(GuiFlatItemLighting {
                            lightmap_generation: 1,
                            rgb: [f32::NAN; 3],
                        })
                }
            ...
            assert!(
                ops.is_empty(),
                "malformed final layer must not partially encode earlier items"
            );
        ...
        for repeat in 0..12 {
            ...
            let (frame_ops, stats) = frontend
                ...
                .unwrap();
            assert_eq!(
                stats.affine_quad_count, 9,
                "exactly one presentation per item, never per child layer"
            );
            assert_eq!(stats.owned_intermediate_targets.len(), 1);
            if repeat > 0 && repeat != 3 && repeat != 6 {
                assert_eq!(gal.metrics().resource_creates, creates);
            }
            let mut ops = vec![
                CommandOp::Barrier(texture_barrier(
            ...
            let list = gal
                .create_command_list(CommandListDesc {
                    label: "grouped-item.commands".into(),
                    operations: ops,
                })
                .unwrap();
            let token = gal
                .submit(SubmissionBatch {
                    label: "grouped-item.submit".into(),
                    command_lists: vec![list],
                })
                .unwrap();
            gal.retire_through_for_test(token.submission).unwrap();
            ...
            for y in 0..512 {
                for x in 0..512 {
                    let covered = if !(3..6).contains(&repeat) {
                        (100..132).contains(&y)
                    } else {
                        (108..124).contains(&y) && (8..24).contains(&(x % 32))
                    };
                    let expected = if covered && x < 9 * 32 {
                        if (6..9).contains(&repeat)
                            && !((108..124).contains(&y) && (4..20).contains(&(x % 32)))
                        {
            ...
            if let Some(previous) = stable_live {
    ...
    if bottom_up && !north_face && retained_slots == 0 {
        ...
        let lifecycle_frames = frame_scales
            .into_iter()
            .map(|value| (value, 1))
        ...
        for (frame_scale, generation) in lifecycle_frames {
            ...
            if generation == 22 {
                        ...
                        .stage_atlas_animation_assets(OwnedAtlasAnimationUpdate {
                            ...
                            sprites: regions
                                ...
                                .map(|(index, &(x, y))| {
                                    ...
                                    OwnedSpriteAnimation {
                                        sprite_id: index as u32 + 1,
                                        ...
                                        clock: SpriteAnimationClock::new(
                                            vec![
                                                SpriteAnimationFrame {
                                                    index: 0,
                                                    duration_ticks: if interpolate { 4 } else { 1 },
                                                },
                                                SpriteAnimationFrame {
                                                    index: 1,
                                                    duration_ticks: if interpolate { 4 } else { 1 },
                ...
                animation_tick += 1;
                assert!(world
                    .advance_atlas_animation(
                        &mut gal,
                        AtlasAnimationTickEvent {
                            texture_id: WORLD_MATERIAL_TEXTURE_STONE,
                            generation: 1,
                            tick: animation_tick,
                            visible: (1..=9).collect(),
                ...
                assert_eq!(
                    world
                        ...
                        .unwrap(),
                    incarnation,
                    "animation updates the same owned atlas, not a replacement binding"
                );
            }
            ...
            if generation == 6 && frontend.generation != 6 {
                previous_live = None;
            }
            let frame_cell = (16 * frame_scale) as usize;
            let screen_requests = requests
                .iter()
                .enumerate()
                .map(|(index, (_, local))| {
                    let mut screen = local.clone();
                    screen.item_raster_scale = frame_scale;
                    screen.x0 = index as f32 * frame_cell as f32;
                    screen.x1 = screen.x0 + frame_cell as f32;
                    screen.x3 = screen.x0;
                    screen.y0 = 100.0;
                    screen.y1 = 100.0;
                    screen.y3 = 100.0 + frame_cell as f32;
                    screen.gui_width = 512;
                    screen.gui_height = 512;
                    screen.projection_extent = [512.0; 2];
                    ...
                    if (14..18).contains(&generation) {
                        let [u0, v0, u1, v1] =
                            ITEM_RECTS[(index + generation as usize) % 9].map(|v| v as f32 / 16.0);
                        screen.u0 = u0;
                        screen.v0 = v0;
                        screen.u1 = u1;
                        screen.v1 = v1;
                    ...
                    if (18..22).contains(&generation) {
                        screen.item_raster_geometry.corners =
                            ITEM_ORIENTATIONS[(index + generation as usize) % 8].map(|v| v as f32);
                    }
                    screen
                })
                .collect();
            ...
            let list = gal
                .create_command_list(CommandListDesc {
                    label: "item-raster.whole-gui.commands".into(),
                    operations: ops,
                })
                .unwrap();
            let token = gal
                .submit(SubmissionBatch {
                    label: "item-raster.whole-gui.submit".into(),
                    command_lists: vec![list],
                })
                .unwrap();
            gal.retire_through_for_test(token.submission).unwrap();
                    ...
                    for x in 0..512 {
                        let expected = if (100..100 + frame_cell).contains(&y) && x < 9 * frame_cell
                        {
                            ...
                            let [source_left, source_top, source_right, source_bottom] =
                                ...
                                .map(|v| v * sprite_size / 16);
                            let source_width = source_right - source_left;
                            let source_height = source_bottom - source_top;
                            let mut column = source_left
                                + ((local_x.saturating_sub(left)) * source_width
                                    + source_width / 2)
                                    / (right - left);
                            let mut row = source_top
                                + ((local_y.saturating_sub(top)) * source_height
                                    + source_height / 2)
                                    / (bottom - top);
                            if (18..22).contains(&generation) {
                                // Exact integer inverse of the authored signed
                                // permutation, evaluated at pixel centers.
                                let [x0, y0, x1, y1, x3, y3] =
                                    ITEM_ORIENTATIONS[(x / frame_cell + generation as usize) % 8];
                                let (ux, uy, vx, vy) = (
                                    (x1 - x0) / 16,
                                    (y1 - y0) / 16,
                                    (x3 - x0) / 16,
                                ...
                                let (px, py) = (
                                    2 * local_x as i64 + 1 - 2 * x0 * frame_scale as i64,
                                    2 * local_y as i64 + 1 - 2 * y0 * frame_scale as i64,
                                );
                                let denominator = 2 * frame_cell as i64 * (ux * vy - uy * vx);
                                column = ((px * vy - py * vx) * sprite_size as i64 / denominator)
                                    as usize;
                                row = ((py * ux - px * uy) * sprite_size as i64 / denominator)
                                    as usize;
                            }
                            if generation == 22 {
                                ...
                                let value = if interpolate {
                                    [64, 128, 192, 255, 191, 127, 63, 0]
                                        [animation_tick as usize - 1]
                                } else if animation_tick % 2 == 1 {
                                    252
                                } else {
                                    0
                                };
                                [value, value, value, 255]
                            } else if !(left..right).contains(&local_x)
                                || !(top..bottom).contains(&local_y)
                            {
                                [0; 4]
                            } else if tinted {
                                tinted_item_pixel(
                                    ...
                                    row,
                                    sprite_size,
                                    transparency && x / frame_cell == 6,
                                    transparency,
                                    ITEM_TINTS[(x / frame_cell + generation as usize) % 9],
                                )
                            } else if transparency
                                && alpha_range.contains(&row)
                                && alpha_range.contains(&column)
                            {
                                if column < alpha_split {
                                    [0, 0, 0, 0]
                                } else if x / frame_cell == 6 {
                                    [26, 26, 26, 26]
                                } else {
                                    [3, 3, 3, 26]
                                }
            ...
            if let Some(previous) = previous_live {
                assert_eq!(live,previous,"repeated frames, GUI-scale changes and asset rebuilds must preserve bounded raster residency");
            }
        ...
        assert_eq!(
            frontend.item_raster_slots, slots_before,
            "a GUI asset-generation rebuild must not erase semantic model slot history"
        );
    }
    composite.destroy(&mut gal);
    for handle in [output_pass, output_target, output_view, output] {
        gal.destroy(handle).unwrap();
    }
    frontend.reset(&mut gal).unwrap();
    assert_eq!(
        frontend.item_raster_slots,
        Default::default(),
        "full context reset retires semantic history too"
    );
    world.reset(&mut gal);
