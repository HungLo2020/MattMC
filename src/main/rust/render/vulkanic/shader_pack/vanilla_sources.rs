...
pub(super) fn qualified(source: &ShaderPackSource) -> GalResult<bool> {
    match source.get(FORMAT_PATH) {
        ...
        Some(_) => Err(GalError::unsupported_feature(
            "unknown copied vanilla shader snapshot format",
        )),
    }
}
...
pub(super) fn key(namespace: &str, path: &str) -> GalResult<String> {
            ...
            .all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || b"_.-".contains(&c))
    {
        return Err(GalError::invalid_argument(
            "invalid copied shader namespace",
        ));
    }
...
mod tests {
    use super::super::{
        source::ShaderSourceFile, vanilla_imports,
        vanilla_post_effect_contract::VanillaPostEffectContract,
    };
    use super::*;
    ...
    fn qualified_stages_cross_namespaces_without_flat_or_bundled_substitution() {
        ...
        let files = vec![
            ...
            ShaderSourceFile::new(
                "assets/example/shaders/post/probe.fsh",
                "#version 330\n#moj_import <example:value.glsl>\n#moj_import \"local.glsl\"\n",
            ),
            ShaderSourceFile::new(
                "assets/example/shaders/post/local.glsl",
                "float localValue;\n",
            ),
            ShaderSourceFile::new(
            ...
            ShaderSourceFile::new(
                "program/post/probe.fsh",
                "float wrongLegacyProgramDirectory;\n",
            ),
        ];
