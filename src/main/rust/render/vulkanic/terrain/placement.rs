...
impl TerrainSectionPlacement {
    pub fn lower(self) -> GalResult<[f32; 16]> {
        if self
            .origin
            .iter()
            .any(|value| value % 16 != 0 || i64::from(*value).abs() > 30_000_000)
            || self
                .camera
                .iter()
                .any(|value| !value.is_finite() || value.abs() > 30_000_000.0)
        {
            return Err(GalError::invalid_argument(
                "invalid semantic terrain section placement",
            ));
        }
...
mod tests {
    ...
    fn fractional_camera_preserves_the_authored_precision_grid() {
        let matrix = TerrainSectionPlacement {
            origin: [112, 80, 528],
            camera: [150.5, 101.62, 530.5],
        }
        .lower()
        .unwrap();
    ...
    fn negative_coordinates_and_group_boundaries_keep_section_spacing() {
        for camera in [-101.62, 101.62, 29_999_980.62, -29_999_980.62] {
            ...
            for axis in 0..3 {
                ...
                let first = TerrainSectionPlacement {
                    origin,
                    camera: [camera; 3],
                }
                .lower()
                .unwrap();
                origin[axis] += 16;
                let second = TerrainSectionPlacement {
                    origin,
                    camera: [camera; 3],
                }
                .lower()
        ...
        let negative = TerrainSectionPlacement {
            origin: [-96; 3],
            camera: [-101.62; 3],
        }
        .lower()
    ...
    fn invalid_inputs_are_rejected_before_any_render_resources_exist() {
        for camera in [f64::NAN, f64::INFINITY, 30_000_001.0] {
            assert!(TerrainSectionPlacement {
                origin: [0; 3],
                camera: [camera; 3]
            }
            .lower()
