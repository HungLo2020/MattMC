...
pub(super) fn assign_draw_ranges(
    ops: &mut Vec<CommandOp>,
    pipelines: &BTreeMap<Handle, Handle>,
) -> GalResult<()> {
            ...
            CommandOp::BindResourceSet {
                ...
                if dynamic_offsets.len() != 2 {
                    return Err(GalError::invalid_argument(
                        "observer expects two semantic mesh offsets",
                    ));
                }
                ...
                if let Some(layout) = current_pipeline.and_then(|p| pipelines.get(&p)) {
                    ...
                    if *slot >= DRAW_LIMIT {
                        return Err(GalError::unsupported_feature(
                            "vertex observation exceeds 1024 bounded draws",
                        ));
                    }
                    let mut set = sets.get(layout).cloned().ok_or_else(|| {
                        GalError::invalid_argument(
                            "observed draw has no explicit mesh resource set",
                        )
                    })?;
                    if let CommandOp::BindResourceSet {
                        dynamic_offsets, ..
                    } = &mut set
                    {
...
pub(super) fn instrument(source: &str) -> GalResult<String> {
    ...
    if source.matches(anchor).count() != 1 || !source.starts_with("#version 450\n") {
        return Err(GalError::invalid_argument(
            "vertex observer requires exact builtin terrain source",
        ));
    }
...
"#;
    let mut source = source.replacen("#version 450\n", declarations, 1).replacen(
        anchor,
        &format!("{anchor}{observation}"),
        1,
    );
    let end = source
        .rfind("\n}")
...
impl Observation {
    ...
    pub fn create(gal: &mut VulkanicGal, created: &mut Vec<Handle>) -> GalResult<Self> {
        let mut buffer = |label: &str, memory, usages| -> GalResult<Handle> {
            let h = gal.create_buffer(BufferDesc {
                label: label.into(),
                size: POOL_BYTES,
                memory,
                usages,
    ...
    pub fn begin(&self, ops: &mut Vec<CommandOp>) {
        if !self.submitted {
            ops.push(CommandOp::HostWriteBuffer {
                buffer: self.seed,
                offset: 0,
                data: vec![0; POOL_BYTES as usize],
            });
    ...
    pub fn complete(
        ...
        let read = reads
            .iter()
            .find(|r| {
                r.buffer == self.readback
                    && r.submission.0 == submission
                    && r.offset == 0
                    && r.bytes.len() == POOL_BYTES as usize
            })
        ...
        if emit {
            ...
            for draw in read.bytes.chunks_exact(DRAW_STRIDE as usize) {
                ...
                count += n;
                truncated |= n > LIMIT;
                for row in draw[4..BYTES as usize]
                    .chunks_exact(RECORD_WORDS * 4)
                    .take(n.min(LIMIT))
                {
                    records.push(
                        row.chunks_exact(4)
...
mod tests {
    ...
    fn vertex_observation_assigns_disjoint_ranges_without_changing_draws() {
        ...
        let offsets: Vec<_> = observed
            .iter()
            .filter_map(|op| match op {
                CommandOp::BindResourceSet {
                    dynamic_offsets, ..
                } => Some(dynamic_offsets.clone()),
                _ => None,
            })
            .collect();
        assert_eq!(
            offsets,
            vec![vec![256, 512, 0], vec![256, 512, DRAW_STRIDE]]
        );
        assert!(DRAW_STRIDE >= BYTES && DRAW_STRIDE % 256 == 0);
        let draws = |ops: Vec<CommandOp>| {
            ops.into_iter()
    ...
    fn vertex_observation_keeps_original_position_expression_and_bounds_every_write() {
        ...
        assert!(
            observed.find("floatBitsToUint(clip[c])").unwrap()
