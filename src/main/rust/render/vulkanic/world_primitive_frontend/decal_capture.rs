...
fn invalid(message: &str) -> GalError { GalError::invalid_argument(message) }
...
pub(super) fn uploaded_range(ops: &[CommandOp], buffer: Handle, start: u64, size: usize) -> GalResult<&[u8]> {
    let end = start.checked_add(size as u64).ok_or_else(|| invalid("decal capture range overflow"))?;
    for op in ops.iter().rev() {
        if let CommandOp::HostWriteBuffer { buffer: written, offset, data } = op {
            if *written != buffer { continue; }
            let write_end = offset.checked_add(data.len() as u64)
                .ok_or_else(|| invalid("decal capture write overflow"))?;
            if *offset >= end || write_end <= start { continue; }
...
fn observe_inner(gal: &VulkanicGal, frontend: &WorldPrimitiveFrontend,
    frame: &WorldPrimitiveFrame, ops: &[CommandOp]) -> GalResult<(Vec<Value>,Vec<Value>)> {
    let mut semantics = Vec::new();
    for (context, instances) in [("world", &frame.mesh_instances),
        ("first-person", &frame.first_person_mesh_instances)] {
        for (index, instance) in instances.iter().enumerate() {
            let Some(decal) = instance.decal_foil else { continue; };
            if semantics.len() == MAX_RECORDS { return Err(invalid("decal semantic capture limit exceeded")); }
            let foil = instance.item_foil.ok_or_else(|| invalid("decal capture missing foil semantics"))?;
            foil.validate()?;
    ...
    for (op_index, op) in ops.iter().enumerate() {
        match op {
            CommandOp::BeginPass { .. } | CommandOp::EndPass => {
                pipeline = Handle::NULL; binding = None;
            }
            CommandOp::BindGraphicsPipeline(handle) => { pipeline = *handle; }
            ...
            CommandOp::DrawIndexed { indices, instances } => {
                let Some((program, _)) = frontend.mesh_pipeline_resources.iter().find(|(key, value)|
                    value.pipeline == pipeline && key.shader_program_identity.as_str() == WORLD_DECAL_FOIL_PROGRAM_ID)
                    else { continue; };
                if draws.len() == MAX_RECORDS || *instances as usize > MAX_RECORDS {
                    return Err(invalid("decal draw capture limit exceeded"));
                }
                let (layout,set,offsets) = binding.ok_or_else(|| invalid("decal capture missing binding"))?;
                let (key,resource) = frontend.mesh_resources.iter().find(|(_,value)| value.resource_set == set)
                    .ok_or_else(|| invalid("decal capture unowned resource set"))?;
                if resource.pipeline != pipeline || resource.pipeline_layout != layout || offsets.len() != 3 {
                    return Err(invalid("decal capture pipeline/binding mismatch"));
                }
                let asset = frontend.mesh_assets.get(&key.mesh_key)
                    .ok_or_else(|| invalid("decal capture missing source mesh"))?;
                let normals = asset.decal_normals.as_ref()
                ...
                if asset.mesh_generation != key.mesh_generation || resource.geometry_key != key.geometry_key()
                    || offsets[0] != resource.vertex_offset || normals.len() != key.decal_vertex_count
                    || asset.vertex_bytes.len() != normals.len() * WORLD_MESH_GPU_VERTEX_BYTES {
                    return Err(invalid("decal capture source geometry incarnation mismatch"));
                }
                let mut source_vertices = Vec::new();
                for (i, normal) in normals.iter().take(4).enumerate() {
                    let offset = i * WORLD_MESH_GPU_VERTEX_BYTES;
                    source_vertices.push(json!({"position":floats(&asset.vertex_bytes[offset..offset+12])?,
                        "normal_packed":normal}));
                }
                let descriptor = gal.resource_set_descriptor_for_capture(set)?;
                let texture = frontend.mesh_texture_resources.get(&key.texture_id)
                    .ok_or_else(|| invalid("decal capture missing owned texture"))?;
                let source = frontend.mesh_texture_assets.get(&key.texture_id)
                ...
                let foil_binding = descriptor.bindings.iter().find(|b| b.binding == 4)
                    .ok_or_else(|| invalid("decal capture missing foil binding"))?;
                let instance_binding = descriptor.bindings.iter().find(|b| b.binding == 1)
                ...
                if foil_binding.buffer_range.is_none_or(|range| range < size as u64) {
                    return Err(invalid("decal capture descriptor range too small"));
                }
                let payload = uploaded_range(&ops[..op_index],foil_binding.resource,offsets[2],size)?;
                let transforms = uploaded_range(&ops[..op_index],instance_binding.resource,offsets[1],
                    WORLD_MESH_BATCH_HEADER_BYTES + count * WORLD_MESH_INSTANCE_BYTES)?;
                let mut uploaded = Vec::new();
                for i in 0..count {
                    let uv_offset = u32::from_le_bytes(payload[i*48+36..i*48+40].try_into().unwrap()) as usize * 4;
                    let expected = count*48 + i*key.decal_vertex_count*8;
                    if uv_offset != expected { return Err(invalid("decal capture UV address mismatch")); }
...
mod tests {
    ...
    fn decal_capture_requires_the_actual_preceding_upload() {
        ...
        let mut ops = vec![CommandOp::HostWriteBuffer {buffer,offset:0,data:vec![1;16]}];
        assert_eq!(uploaded_range(&ops,buffer,4,8).unwrap(), &[1;8]);
        ops.push(CommandOp::HostWriteBuffer {buffer,offset:20,data:vec![2;4]});
        assert_eq!(uploaded_range(&ops,buffer,4,8).unwrap(), &[1;8]);
        ops.push(CommandOp::HostWriteBuffer {buffer,offset:6,data:vec![3;4]});
