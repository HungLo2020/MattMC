...
fn invalid(s: &str) -> GalError {
    GalError::invalid_argument(s)
}
...
fn observe_scope(
    ...
    let schema = if wolf {
        "wolf-submission-inputs-v1"
    } else {
        "equipment-submission-inputs-v1"
    };
...
fn observe_inner(
    ...
    ops: &[CommandOp],
    wolf: bool,
) -> GalResult<(Vec<Value>, Vec<Value>, Vec<Value>, Vec<Value>)> {
    ...
    for (context, instances) in [
        ...
        for (index, instance) in instances.iter().enumerate() {
            ...
            let identity = frontend
                ...
                .map(|a| a.entity_identity.as_str());
            if projection.is_none() && !(wolf && identity == Some("minecraft:wolf")) {
                continue;
            }
            if semantics.len() == MAX_RECORDS {
                return Err(invalid("equipment semantic capture limit"));
            }
    ...
    for (op_index, op) in ops.iter().enumerate() {
        match op {
            ...
            CommandOp::SetIndexBuffer {
                ...
                offset,
                index_type,
            } => index_binding = Some((*buffer, *offset, *index_type)),
            ...
            CommandOp::DrawIndexed { indices, instances } => {
                let Some((layout, set, offsets)) = binding else {
                    continue;
                };
                ...
                if draws.len() == MAX_RECORDS
                    || *instances == 0
                    || *instances as usize > MAX_RECORDS
                {
                ...
                let (program, _) = frontend
                    .mesh_pipeline_resources
                    .iter()
                    .find(|(_, v)| v.pipeline == pipeline)
                    .ok_or_else(|| invalid("equipment unowned pipeline"))?;
                let asset = frontend
                    .mesh_assets
                    .get(&key.mesh_key)
                    .ok_or_else(|| invalid("equipment missing mesh"))?;
                ...
                let mut geometry_uploads = None;
                {
                    let geometry = frontend
                        ...
                        .get(&resource.geometry_key)
                        .ok_or_else(|| invalid("equipment missing owned geometry allocation"))?;
                    let mut selected_range = None;
                    for instance in frame
                        .mesh_instances
                        .iter()
                        .chain(&frame.first_person_mesh_instances)
                    {
                        if instance.mesh_key != key.mesh_key
                            || instance.mesh_generation != key.mesh_generation
                        {
                            continue;
                        }
                        let ranges = if instance.mesh_section_index == WORLD_MESH_SECTION_ALL {
                            compatible_mesh_section_ranges(
                                instance,
                                asset,
                                key.color_format,
                            ...
                            vec![MeshSectionRange {
                                key: mesh_key_for_section(
                                    instance,
                                    section,
                    ...
                    let range = selected_range
                        .ok_or_else(|| invalid("equipment missing compatible geometry range"))?;
                    let expected_offset = resource
                        .index_offset
                        .checked_add(range.index_offset)
                    ...
                    if vertex.resource != resource.vertex_buffer
                        || vertex.kind != ResourceBindingKind::StorageBuffer
                        || vertex.dynamic_offsets != [0]
                        || vertex.buffer_range != Some(resource.vertex_range)
                        || geometry.vertex_buffer != resource.vertex_buffer
                        || geometry.vertex_offset != resource.vertex_offset
                        || geometry.vertex_range != resource.vertex_range
                        || geometry.index_buffer != resource.index_buffer
                        ...
                        || index_binding
                            != Some((resource.index_buffer, expected_offset, asset.index_type))
                    ...
                    {
                        ensure_geometry_unwritten(
                            gal,
                            &ops[..op_index],
                            resource.vertex_buffer,
                            resource.vertex_offset,
                            asset.vertex_bytes.len(),
                        )?;
                        ensure_geometry_unwritten(
                            gal,
                            &ops[..op_index],
                            resource.index_buffer,
                            resource.index_offset,
                            asset.index_bytes.len(),
                        )?;
                        let (vertices, vertex_submission) = gal.buffer_upload_for_capture(
                            resource.vertex_buffer,
                            resource.vertex_offset,
                            asset.vertex_bytes.len(),
                        )?;
                        let (indices, index_submission) = gal.buffer_upload_for_capture(
                            resource.index_buffer,
                            resource.index_offset,
                            asset.index_bytes.len(),
                        )?;
                        if vertices != asset.vertex_bytes || indices != asset.index_bytes {
                            return Err(invalid(
                                "equipment accepted geometry uploads differ from source bytes",
                            ));
                        }
                ...
                let texture = frontend
                    ...
                    .get(&key.texture_id)
                    .ok_or_else(|| invalid("equipment unowned texture"))?;
                let source = frontend
                    .mesh_texture_assets
                    .get(&key.texture_id)
                    .ok_or_else(|| invalid("equipment missing texture source"))?;
                ...
                if capture_source && !textures.contains_key(&key.texture_id) {
                    ...
                    textures.insert(
                        key.texture_id,
                        cached_texture_source(key.texture_id, source)?,
                    );
                }
                ...
                let mesh_uvs = if capture_source {
                    if wolf {
                        captured_mesh_uvs_bounded(&asset.vertex_bytes, 512)?
                    } else {
                        captured_mesh_uvs(&asset.vertex_bytes)?
                    }
                } else {
                    Vec::new()
                };
                ...
                let ib = descriptor
                    ...
                    .iter()
                    .find(|b| b.binding == 1)
                    .ok_or_else(|| invalid("equipment missing instance binding"))?;
                ...
                let payload = if key.standard_item_foil {
                    if key.decal_vertex_count != 0 {
                        return Err(invalid("equipment unexpected decal"));
                    }
                    let fb = descriptor
                        .bindings
                        .iter()
                        .find(|b| b.binding == 4)
                        .ok_or_else(|| invalid("equipment missing foil binding"))?;
                    if fb.dynamic_offsets != [0]
                        || fb.buffer_range.is_none_or(|v| v < (count * 48) as u64)
                    {
    ...
    Ok((
        ...
        model_textures.into_values().collect(),
    ))
}
...
pub(super) fn cached_texture_source(
    id: u32,
    source: &WorldMaterialTextureAsset,
) -> GalResult<Value> {
    if source.equipment_capture_png.get().is_none() {
        let encoded = encoded_texture_source(id, source.width, source.height, &source.rgba)?;
        let hex = encoded["png_hex"]
            .as_str()
            .ok_or_else(|| invalid("equipment missing encoded PNG"))?;
        let _ = source.equipment_capture_png.set(hex.to_owned());
    }
    Ok(
        json!({"texture_id":id,"width":source.width,"height":source.height,"encoding":"png-rgba8-hex",
...
fn encoded_texture_source(id: u32, width: u32, height: u32, rgba: &[u8]) -> GalResult<Value> {
    ...
    let mut bytes = Vec::new();
    {
        ...
        let mut writer = encoder
            .write_header()
            .map_err(|_| invalid("equipment texture PNG header"))?;
        writer
            .write_image_data(rgba)
            .map_err(|_| invalid("equipment texture PNG payload"))?;
    }
    if bytes.len() > 1024 * 1024 {
        return Err(invalid("equipment texture encoded capture limit"));
    }
    ...
    Ok(
        json!({"texture_id":id,"width":width,"height":height,"encoding":"png-rgba8-hex",
        "rgba_xxh32":format!("{:08x}",xxh32(rgba,0)),"png_hex":hex}),
    )
}
fn ensure_geometry_unwritten(
    gal: &VulkanicGal,
    ops: &[CommandOp],
    buffer: Handle,
    start: u64,
    ...
    let end = start
        ...
        .ok_or_else(|| invalid("equipment geometry range overflow"))?;
    let overlaps =
        |offset: u64, size: u64| size != 0 && start < offset.saturating_add(size) && offset < end;
    for op in ops {
        let writes = match op {
            ...
            CommandOp::BindResourceSet { set, .. } => gal
                .resource_set_descriptor_for_capture(*set)?
                .bindings
                .iter()
            ...
            | CommandOp::Present { .. }
            | CommandOp::Barrier(_)
            | CommandOp::EndPass => false,
        };
        if writes {
            return Err(invalid(
                "equipment geometry written after retained accepted upload",
            ));
        }
    }
    Ok(())
...
fn captured_mesh_geometry(
    ...
    for index in indices.chunks_exact(width) {
        let value = if width == 2 {
            u16::from_le_bytes(index.try_into().unwrap()) as u32
        } else {
            u32::from_le_bytes(index.try_into().unwrap())
        };
        if value as usize >= count {
            return Err(invalid("equipment mesh source index out of range"));
        }
    }
    let hex = |bytes: &[u8]| bytes.iter().map(|v| format!("{v:02x}")).collect::<String>();
    Ok(
        json!({"encoding":"packed-source-hex-v1","vertex_stride":WORLD_MESH_GPU_VERTEX_BYTES,
...
mod tests {
    ...
    fn wolf_geometry_capture_preserves_bytes_and_rejects_invalid_indices() {
        ...
        for (kind, width) in [(IndexType::U16, 2), (IndexType::U32, 4)] {
            ...
            assert!(value["vertex_hex"]
                .as_str()
                .unwrap()
                .starts_with("0000803e"));
            assert_eq!(value["vertex_hex"].as_str().unwrap().len(), 160);
            assert_eq!(value["index_hex"].as_str().unwrap(), "00".repeat(width * 6));
            assert!(
                captured_mesh_geometry(&vertices, &indices[..indices.len() - 1], kind).is_err()
            );
    ...
    fn wolf_full_body_capture_has_its_own_bounded_vertex_scope() {
        ...
        assert!(
            captured_mesh_uvs_bounded(&vec![0; 513 * WORLD_MESH_GPU_VERTEX_BYTES], 512).is_err()
        );
    }
    #[test]
