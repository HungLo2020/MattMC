...
pub(super) fn append_batches(
    ...
    for &ordinal in &cache.order {
        ...
        let key = mesh_key_for_section(instance, section, section_index, section.cull_policy,
            asset_generation_for_key(instance.mesh_key, asset)?, asset.vertex_bytes.len() / super::WORLD_MESH_GPU_VERTEX_BYTES, color_format, raster_y_direction, g_buffer);
            ...
            && last.index_offset + u64::from(last.index_count) * index_stride(asset.index_type) == index_offset)
        {
            ...
            batches.push(MeshBatch { model_submission_order: instance.model_submission_order, key, index_offset, index_count: 6, indices: vec![instance_index] });
        }
    }
    Ok(())
}
