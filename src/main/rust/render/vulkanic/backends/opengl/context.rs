...
fn load_clip_control(
    address: *const c_void,
    library: Option<&Library>,
) -> GalResult<ClipControlFn> {
    if !address.is_null() {
        return Ok(unsafe { std::mem::transmute::<*const c_void, ClipControlFn>(address) });
    }
    library
        .and_then(|library| unsafe {
            library
                .get::<ClipControlFn>(b"glClipControl\0")
                .ok()
                .map(|symbol| *symbol)
        })
        .ok_or_else(|| {
            GalError::unsupported_feature(
                "OpenGL glClipControl is required for explicit raster direction",
            )
        })
}

fn load_provoking_vertex(
    address: *const c_void,
    library: Option<&Library>,
) -> GalResult<ProvokingVertexFn> {
    if !address.is_null() {
    ...
    library
        .and_then(|library| unsafe {
            library
                .get::<ProvokingVertexFn>(b"glProvokingVertex\0")
                .ok()
                .map(|symbol| *symbol)
        })
        .ok_or_else(|| {
            GalError::unsupported_feature(
                "OpenGL glProvokingVertex is required for explicit flat-shading state",
            )
        })
}
...
impl OpenGlContext {
    pub(super) fn new(label: &str) -> GalResult<Self> {
        ...
        let clip_control = load_clip_control(
            native.get_proc_address(c"glClipControl".as_ptr()).cast(),
            gl_library.as_ref(),
        )?;
        let provoking_vertex = load_provoking_vertex(
            native
                .get_proc_address(c"glProvokingVertex".as_ptr())
                .cast(),
            gl_library.as_ref(),
        )?;
        let gl = unsafe {
    ...
    pub(super) fn from_existing_context(desc: ExistingOpenGlContextDesc) -> GalResult<Self> {
        ...
        let provoking_vertex = load_provoking_vertex(
            get_proc_address
                .map(|get| unsafe { get(c"glProvokingVertex".as_ptr().cast()) })
                .unwrap_or(ptr::null())
                .cast(),
            Some(&gl_library),
        )?;
        let clip_control = load_clip_control(
            get_proc_address
                .map(|get| unsafe { get(c"glClipControl".as_ptr().cast()) })
                .unwrap_or(ptr::null())
                .cast(),
            Some(&gl_library),
        )?;
    ...
    pub(super) fn borrowed_state_guard_with_images(
        ...
        self.native.is_existing().then(|| {
            BorrowedOpenGlStateGuard::capture(
                self.gl.clone(),
...
fn borrowed_provoking_vertex_state_is_restored_in_both_directions() {
    ...
    for (original, changed) in [
        (glow::FIRST_VERTEX_CONVENTION, glow::LAST_VERTEX_CONVENTION),
        (glow::LAST_VERTEX_CONVENTION, glow::FIRST_VERTEX_CONVENTION),
    ] {
        unsafe {
            (context.provoking_vertex)(original);
        }
        {
            let _guard = BorrowedOpenGlStateGuard::capture(
                context.gl.clone(),
                context.provoking_vertex,
                context.clip_control,
                false,
            );
            unsafe {
                (context.provoking_vertex)(changed);
                assert_eq!(
                    context.gl.get_parameter_i32(glow::PROVOKING_VERTEX) as u32,
                    changed
                );
            }
        }
...
fn borrowed_raster_y_direction_and_depth_clip_mode_are_restored() {
    ...
    context.make_current().unwrap();
    for (origin, depth) in [
        (glow::LOWER_LEFT, glow::NEGATIVE_ONE_TO_ONE),
        (glow::UPPER_LEFT, glow::ZERO_TO_ONE),
    ] {
        unsafe {
            (context.clip_control)(origin, depth);
        }
        {
            let _guard = BorrowedOpenGlStateGuard::capture(
                context.gl.clone(),
            ...
            unsafe {
                (context.clip_control)(
                    if origin == glow::LOWER_LEFT {
                        glow::UPPER_LEFT
                    } else {
                        glow::LOWER_LEFT
                    },
                    if depth == glow::ZERO_TO_ONE {
        ...
        unsafe {
            assert_eq!(
                context.gl.get_parameter_i32(glow::CLIP_ORIGIN) as u32,
                origin
            );
            assert_eq!(
                context.gl.get_parameter_i32(glow::CLIP_DEPTH_MODE) as u32,
                depth
            );
        }
    }
...
fn borrowed_unpack_skip_images_is_restored_after_buffer_texture_copy_state() {
    let context = OpenGlContext::new("unpack-image-skip-restoration")
        .expect("OpenGL required for state restoration");
    context.make_current().unwrap();
    unsafe {
        context.gl.pixel_store_i32(glow::UNPACK_SKIP_IMAGES, 3);
    }
    {
        let _guard = BorrowedOpenGlStateGuard::capture(
            context.gl.clone(),
        ...
        unsafe {
            context.gl.pixel_store_i32(glow::UNPACK_SKIP_IMAGES, 0);
        }
    }
    unsafe {
        assert_eq!(context.gl.get_parameter_i32(glow::UNPACK_SKIP_IMAGES), 3);
    }
}
...
impl Drop for BorrowedOpenGlStateGuard {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            self.gl
                .pixel_store_i32(glow::UNPACK_SKIP_IMAGES, self.unpack_skip_images);
            self.gl
