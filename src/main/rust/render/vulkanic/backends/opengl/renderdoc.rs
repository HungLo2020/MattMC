...
impl RenderDocFrame {
    pub(in crate::render::vulkanic) fn start_if_requested() -> Option<Self> {
        ...
        if !std::env::var("MATTMC_RENDERDOC_BACKEND")
            .map(|value| value.is_empty() || value.eq_ignore_ascii_case("opengl"))
            .unwrap_or(true)
        {
            return None;
        }
        ...
        eprintln!("RenderDoc capture requested for Rust OpenGL test");
        unsafe {
            ...
            eprintln!("RenderDoc API initialized for Rust OpenGL test capture");
