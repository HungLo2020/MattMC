...
fn graphics_storage_observes_every_vertex_and_fragment_with_explicit_reset_copy_and_retirement() {
    for opengl in [false, true] {
        ...
        let color = own!(gal.create_texture(TextureDesc {
            label: "graphics-storage.color".into(),
            dimension: TextureDimension::D2,
            format: TextureFormat::Rgba8Unorm,
            extent,
            mip_levels: 1,
        ...
        let view = own!(gal.create_texture_view(TextureViewDesc {
            label: "graphics-storage.view".into(),
            texture: color,
            format: TextureFormat::Rgba8Unorm,
            base_mip: 0,
            mip_count: 1,
        ...
        let set = own!(gal.create_resource_set(ResourceSetDesc {
            label: "graphics-storage.set".into(),
            ...
            bindings: vec![ResourceBinding {
                ...
                dynamic_offsets: vec![],
                buffer_range: Some(BYTES),
            }],
        }));
        let layout = own!(gal.create_pipeline_layout(PipelineLayoutDesc {
            label: "graphics-storage.pipeline-layout".into(),
        ...
        let declaration =
            format!("layout({qualifier}binding=0,std430) buffer Storage0 {{ uint words[]; }};\n");
...
"#
        ...
        let vertex = own!(gal.create_shader_module(ShaderModuleDesc {
            label: "graphics-storage.vertex".into(),
            stage: ShaderStage::Vertex,
            code_format: ShaderCodeFormat::Glsl,
            code: vertex_source.into_bytes(),
            entry_point: "main".into(),
        }));
        let fragment = own!(gal.create_shader_module(ShaderModuleDesc {
            label: "graphics-storage.fragment".into(),
            stage: ShaderStage::Fragment,
            code_format: ShaderCodeFormat::Glsl,
            code: fragment_source.into_bytes(),
            entry_point: "main".into(),
        }));
        let pipeline = own!(gal.create_graphics_pipeline(GraphicsPipelineDesc {
            label: "graphics-storage.pipeline".into(),
            layout,
        ...
        for epoch in [41u32, 42u32] {
            ...
            operations.extend([
                CommandOp::HostWriteBuffer {
                    buffer: seed,
                    offset: 0,
                    data: initial.iter().flat_map(|v| v.to_le_bytes()).collect(),
                },
                barrier(
                    seed,
                    TextureUsageState::TransferDst,
            ...
            let list = gal
                .create_command_list(CommandListDesc {
                    label: "graphics-storage.commands".into(),
                    operations,
                })
                .unwrap();
            let token = gal
                .submit(SubmissionBatch {
                    label: "graphics-storage.submit".into(),
            ...
            assert_eq!(
                reads
                    .iter()
                    .rev()
                    .find(|r| r.buffer == texture_readback)
                    .unwrap()
                    .bytes,
                read.bytes,
                "GPU buffer-to-texture copy must not use an obsolete upload shadow"
            );
            let partial = &reads
                .iter()
                .rev()
                .find(|r| r.buffer == partial_readback)
                .unwrap()
                .bytes;
            ...
            assert!(
                (3..=LIMIT).contains(&count),
                "bounded vertex observations: {count}, OpenGL={opengl}, epoch={epoch}, header={:?}",
                &words[..3]
            );
            assert_eq!(
                *first_count.get_or_insert(count),
                count,
                "reset must not accumulate previous observations"
            );
            ...
            for sample in words[3..3 + count * 8].chunks_exact(8) {
                ...
                assert_eq!(
                    &sample[5..],
                    &[17, 23, epoch],
                    "must observe this submission's initial data"
                );
            }
            ...
            assert!(
                ...
                "unused slots must stay untouched"
            );
            assert_eq!(
                reads
                    .iter()
                    .rev()
                    .find(|r| r.buffer == color_readback)
                    .unwrap()
                    .bytes,
                [255, 0, 255, 255]
            );
