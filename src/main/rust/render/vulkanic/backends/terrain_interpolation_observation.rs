...
fn acquired_frame_load_preserves_owned_world_copy_and_explicit_clear() {
    ...
    let surface = FrameSurfaceDesc {
        label: "explicit-frame-load".into(),
        extent,
        color_format: TextureFormat::Bgra8Unorm,
        present_mode: PresentMode::Fifo,
        max_frames_in_flight: 2,
    };
    let backend = super::vulkan::VulkanBackend::new_windowed_for_test(
        "explicit frame load",
        &window,
        surface.clone(),
    )
    ...
    let pass = gal
        .create_render_pass(RenderPassDesc {
            label: "explicit-frame.pass".into(),
            target: frame,
            color_formats: vec![acquired.color_format],
            depth_format: None,
        })
        .unwrap();
    let readback = gal
        .create_buffer(BufferDesc {
            label: "explicit-frame.readback".into(),
            size: 4,
    ...
    let snapshot = gal
        .create_texture(TextureDesc {
            label: "explicit-frame.snapshot".into(),
            dimension: TextureDimension::D2,
            format: acquired.color_format,
            extent: acquired.extent,
            mip_levels: 1,
            array_layers: 1,
        ...
        .unwrap();
    let barrier = |resource, before, after, texture| {
        CommandOp::Barrier(ResourceBarrier {
            resource,
            before,
            after,
            subresources: if texture {
                Some(TextureSubresourceRange {
                    base_mip: 0,
                    mip_count: 1,
                    base_layer: 0,
                    layer_count: 1,
    ...
    .enumerate()
    {
            ...
            ops.extend([
                ...
                CommandOp::BeginPass {
                    ...
                    colors: vec![PassAttachment {
                        view: owner.color_view,
                        load_op: AttachmentLoadOp::Clear,
                        store_op: AttachmentStoreOp::Store,
                        clear_color: Some(red),
                    }],
                    depth_stencil: Some(PassAttachment {
                        view: owner.depth_view,
                        load_op: AttachmentLoadOp::Clear,
                        store_op: AttachmentStoreOp::Store,
                        clear_color: None,
                    }),
        ...
        ops.extend([
            CommandOp::BeginPass {
                ...
                colors: vec![PassAttachment {
                    ...
                    load_op: load,
                    store_op: AttachmentStoreOp::Store,
                    clear_color: Some(if epoch == 2 { red } else { green }),
                }],
                depth_stencil: None,
            },
            CommandOp::EndPass,
            ...
            CommandOp::CopyTextureToBuffer(BufferImageCopyRegion {
                ...
                texture_mip: 0,
                texture_layer: 0,
                texture_origin: TextureOrigin3d { x: 64, y: 64, z: 0 },
                extent: Extent3d {
                    width: 1,
                    height: 1,
                    depth: 1,
        ...
        let list = gal
            .create_command_list(CommandListDesc {
                label: format!("explicit-load.{epoch}"),
                operations: ops,
            })
            .unwrap();
        let token = gal
        ...
        observed.push(
            gal.completed_host_reads()
                .iter()
                .find(|r| r.buffer == readback && r.submission == token.submission)
                .unwrap()
                .bytes
                .clone(),
        );
    ...
    gal.present_frame(PresentFrameDesc {
        ...
        wait_for: gal.latest_submission_id(),
    })
    .unwrap();
    ...
    let next = gal
        ...
        .unwrap();
    let copy_only_frame = gal
        .create_frame_target(FrameTargetDesc {
            label: "copy-only-frame".into(),
            frame_id: next.frame.0,
            render_target: next.render_target,
            extent: next.extent,
            color_format: next.color_format,
        })
        .unwrap();
    let mut ops = vec![
        barrier(
    ...
    let list = gal
        .create_command_list(CommandListDesc {
            label: "copy-only.commands".into(),
            operations: ops,
        })
        .unwrap();
    let token = gal
        .submit(SubmissionBatch {
            label: "copy-only.submit".into(),
            command_lists: vec![list],
        })
        .unwrap();
    gal.retire_through(token.submission).unwrap();
    ...
    gal.present_frame(PresentFrameDesc {
        ...
        wait_for: token.submission,
    })
    .unwrap();
    gal.destroy(copy_only_frame).unwrap();
    for handle in [snapshot, readback, pass] {
        ...
        for mirrored in [false, true] {
            for rotation in 0..3 {
                for expression in ["v_uv.x", "v_uv.y", "gl_FragCoord.w", "gl_FragCoord.z"] {
                    ...
                    assert!(
                        (0.0..1.0).contains(&f32::from_bits(bits)),
                        "must read covered triangle, not clear/background"
                    );
                }
            }
        }
    ...
    for opengl in [false, true] {
        ...
        for mirrored in [false, true] {
            ...
            assert_ne!(
                back == 0,
                front == 0,
                "exactly one cull face must retain the triangle"
            );
        ...
        assert_eq!(
            modes[0].1, modes[1].1,
            "Y direction must preserve logical front-face meaning"
        );
        visible.push(modes);
    }
    assert_eq!(
        visible[0][1].0, visible[1][0].0,
        "explicit Vulkan Down must match measured Frozen/OpenGL orientation on this fixture"
    );
}
...
fn explicit_raster_world_color_and_depth_compose_before_canonical_overlay() {
    let bytes = compose_world_attachments(
    ...
    let wrong_color = compose_world_attachments(
        TextureRowOrder::Preserve,
        TextureRowOrder::Reverse,
        RasterYDirection::Up,
    );
    let wrong_depth = compose_world_attachments(
        TextureRowOrder::Reverse,
        TextureRowOrder::Preserve,
        RasterYDirection::Up,
    );
    let wrong_gui = compose_world_attachments(
        TextureRowOrder::Reverse,
        TextureRowOrder::Reverse,
        RasterYDirection::Down,
    );
    let pixel = |data: &[u8], x: usize, y: usize| {
        <[u8; 4]>::try_from(&data[(y * 8 + x) * 4..(y * 8 + x + 1) * 4]).unwrap()
    };
    assert_eq!(
        pixel(&wrong_color, 6, 1),
        [0, 0, 255, 255],
        "unreversed color must expose the blue lower world half"
    );
    assert_eq!(
        pixel(&wrong_depth, 6, 1),
        [0, 255, 0, 255],
        "unreversed depth must incorrectly admit the green overlay"
    );
    assert_eq!(
        pixel(&wrong_gui, 1, 6),
        [255, 255, 0, 255],
        "Down GUI must visibly land in the wrong half"
    );
...
fn compose_world_attachments(
    color_rows: TextureRowOrder,
    ...
    for (label, direction) in [
        ("world", RasterYDirection::Down),
        ("canonical", RasterYDirection::Up),
    ] {
        let target = OrientedWorldTarget::create(
            &mut gal,
            label,
            WorldTargetDesc {
                extent,
        ...
        attachments.push((
            vec![
                (target.color_texture, target.color_view),
                (target.depth_texture, target.depth_view),
            ],
            target.target,
            target.pass,
        ));
...
"#
    ...
    let readback = own!(gal.create_buffer(BufferDesc {
        label: "composition.readback".into(),
        size: 256,
        memory: MemoryDomain::Readback,
        usages: vec![BufferUsage::TransferDst, BufferUsage::HostRead],
    }));
    ...
    let barrier = |resource, before, after, subresources| {
        CommandOp::Barrier(ResourceBarrier {
            resource,
            before,
            after,
    ...
    operations.extend([
        begin(0, AttachmentLoadOp::Clear),
    ...
    for operation in &mut transfer {
        if let CommandOp::CopyTexture(copy) = operation {
            ...
            let requested = if copy.src_texture == targets[0].color_texture {
                color_rows
            } else {
                depth_rows
            };
            if requested != TextureRowOrder::Reverse {
                copy.row_order = requested;
            }
        }
    }
    ...
    operations.extend([
        ...
        CommandOp::BindGraphicsPipeline(pipelines[0]),
        CommandOp::BindGraphicsPipeline(pipelines[1]),
        CommandOp::Draw {
            vertices: 6,
            instances: 1,
        },
        CommandOp::BindGraphicsPipeline(pipelines[2]),
        CommandOp::Draw {
            vertices: 6,
            instances: 1,
        },
    ...
    let list = gal
        .create_command_list(CommandListDesc {
            label: "composition.commands".into(),
            operations,
        })
        .unwrap();
    let token = gal
        .submit(SubmissionBatch {
            label: "composition.submit".into(),
            command_lists: vec![list],
        })
        .unwrap();
    gal.retire_through(token.submission).unwrap();
...
fn observe(
    ...
    let color = own!(gal.create_texture(TextureDesc {
        label: "interpolation.color".into(),
        dimension: TextureDimension::D2,
    ...
    let target = own!(gal.create_render_target(RenderTargetDesc {
        label: "interpolation.target".into(),
        color_views: vec![view],
        depth_stencil_view: None,
        extent
    }));
    let pass = own!(gal.create_render_pass(RenderPassDesc {
        label: "interpolation.pass".into(),
        target,
        color_formats: vec![TextureFormat::Rgba8Unorm],
        depth_format: None
    }));
    let layout = own!(gal.create_pipeline_layout(PipelineLayoutDesc {
        label: "interpolation.layout".into(),
        resource_layouts: vec![]
    }));
    let id = format!(
        "({}+{rotation})%3",
...
"#,
    ...
    let vertex_shader = own!(gal.create_shader_module(ShaderModuleDesc {
        label: "interpolation.vertex".into(),
        stage: ShaderStage::Vertex,
        code_format: ShaderCodeFormat::Glsl,
        code: vertex.into_bytes(),
        entry_point: "main".into()
    }));
    let fragment = format!(
        r#"#version 450
...
"#
    ...
    let fragment_shader = own!(gal.create_shader_module(ShaderModuleDesc {
        label: "interpolation.fragment".into(),
        stage: ShaderStage::Fragment,
        code_format: ShaderCodeFormat::Glsl,
        code: fragment.into_bytes(),
        entry_point: "main".into()
    }));
    let pipeline = own!(gal.create_graphics_pipeline(GraphicsPipelineDesc {
        label: "interpolation.pipeline".into(),
        layout,
        vertex_shader,
        fragment_shader,
        ...
        raster_y_direction: if mirrored {
            RasterYDirection::Down
        } else {
            RasterYDirection::Up
        },
    ...
    let readback = own!(gal.create_buffer(BufferDesc {
        label: "interpolation.readback".into(),
        size: 4,
        memory: MemoryDomain::Readback,
        usages: vec![BufferUsage::TransferDst, BufferUsage::HostRead]
    }));
    ...
    let canonical_color = if mirrored && !opengl {
        let canonical = own!(gal.create_texture(TextureDesc {
            label: "interpolation.canonical-color".into(),
            dimension: TextureDimension::D2,
            format: TextureFormat::Rgba8Unorm,
            extent,
            mip_levels: 1,
    ...
    operations.extend([
        CommandOp::CopyTextureToBuffer(BufferImageCopyRegion {
            ...
            bytes_per_row: 4,
            rows_per_image: 1,
            texture: canonical_color,
            texture_mip: 0,
            texture_layer: 0,
            texture_origin: TextureOrigin3d {
                x: 764,
                y: if mirrored && opengl { 399 } else { 320 },
                z: 0,
            },
            extent: Extent3d {
                width: 1,
    ...
    let list = gal
        .create_command_list(CommandListDesc {
            label: "interpolation.commands".into(),
            operations,
        })
        .unwrap();
    let token = gal
        .submit(SubmissionBatch {
            label: "interpolation.submit".into(),
            command_lists: vec![list],
        })
        .unwrap();
    gal.retire_through(token.submission).unwrap();
