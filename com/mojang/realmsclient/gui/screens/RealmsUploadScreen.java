...
public class RealmsUploadScreen extends RealmsScreen implements RealmsWorldUploadStatusTracker {
	...
	private void drawProgressBar(GuiGraphics guiGraphics) {
		...
		guiGraphics.drawCenteredString(this.font, Component.translatable("mco.upload.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
	}
	...
	private Component createProgressNarrationMessage() {
		...
		if (this.progress != null) {
			list.add(Component.translatable("mco.upload.percent", new Object[]{this.progress}));
		}
			...
			realmsWorldUpload.packAndUpload().handleAsync((object, throwable) -> {
				if (throwable != null) {
					...
					if (throwable instanceof RealmsUploadException realmsUploadException) {
						...
						this.status = Component.translatable("mco.upload.failed", new Object[]{throwable.getMessage()});
					}
				} else {
	...
	public void setUploading() {
		this.status = Component.translatable("mco.upload.uploading", new Object[]{this.selectedLevel.getLevelName()});
	}
}
