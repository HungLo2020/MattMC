...
class RealmsSubscriptionTab extends GridLayoutTab implements RealmsConfigurationTab {
	...
	RealmsSubscriptionTab(RealmsConfigureWorldScreen realmsConfigureWorldScreen, Minecraft minecraft, RealmsServer realmsServer) {
		...
		rowHelper.addChild(new StringWidget(200, 9, SUBSCRIPTION_START_LABEL, font));
		this.startDateWidget = rowHelper.addChild(new StringWidget(200, 9, this.startDate, font));
		rowHelper.addChild(SpacerElement.height(2));
		this.daysLeftLabelWidget = rowHelper.addChild(new StringWidget(200, 9, TIME_LEFT_LABEL, font));
		this.daysLeftWidget = rowHelper.addChild(new StringWidget(200, 9, this.daysLeft, font));
		rowHelper.addChild(SpacerElement.height(2));
		rowHelper.addChild(
			Button.builder(
		...
		rowHelper.addChild(SpacerElement.height(2));
		this.subscriptionInfo = rowHelper.addChild(new FocusableTextWidget(200, Component.empty(), font), LayoutSettings.defaults().alignHorizontallyCenter());
	...
	private Component daysLeftPresentation(int i) {
		if (i < 0 && this.serverData.expired) {
			...
			boolean bl2 = k > 0;
			if (bl && bl2) {
				return Component.translatable("mco.configure.world.subscription.remaining.months.days", new Object[]{j, k}).withStyle(ChatFormatting.GRAY);
			} else if (bl) {
				return Component.translatable("mco.configure.world.subscription.remaining.months", new Object[]{j}).withStyle(ChatFormatting.GRAY);
			} else {
				return bl2 ? Component.translatable("mco.configure.world.subscription.remaining.days", new Object[]{k}).withStyle(ChatFormatting.GRAY) : Component.empty();
			}
		}
	}
	...
	public void updateData(RealmsServer realmsServer) {
		...
		if (bl) {
			this.subscriptionInfo.setMessage(Component.translatable("mco.snapshot.subscription.info", new Object[]{realmsServer.parentWorldName}));
		} else {
	...
	public Component getTabExtraNarration() {
		return CommonComponents.joinLines(new Component[]{TITLE, SUBSCRIPTION_START_LABEL, this.startDate, TIME_LEFT_LABEL, this.daysLeft});
	}
}
