...
public class InputConstants {
	...
	public static InputConstants.Key getKey(KeyEvent keyEvent) {
		return keyEvent.key() == -1 ? InputConstants.Type.SCANCODE.getOrCreate(keyEvent.scancode()) : InputConstants.Type.KEYSYM.getOrCreate(keyEvent.key());
	}
	...
	public static boolean isKeyDown(Window window, int i) {
		return GLFW.glfwGetKey(window.handle(), i) == 1;
	}
	...
	public static void setupKeyboardCallbacks(Window window, GLFWKeyCallbackI gLFWKeyCallbackI, GLFWCharModsCallbackI gLFWCharModsCallbackI) {
		GLFW.glfwSetKeyCallback(window.handle(), gLFWKeyCallbackI);
		GLFW.glfwSetCharModsCallback(window.handle(), gLFWCharModsCallbackI);
	}
	...
	public static void setupMouseCallbacks(
		...
		GLFW.glfwSetCursorPosCallback(window.handle(), gLFWCursorPosCallbackI);
		GLFW.glfwSetMouseButtonCallback(window.handle(), gLFWMouseButtonCallbackI);
		GLFW.glfwSetScrollCallback(window.handle(), gLFWScrollCallbackI);
		GLFW.glfwSetDropCallback(window.handle(), gLFWDropCallbackI);
	}

	public static void grabOrReleaseMouse(Window window, int i, double d, double e) {
		GLFW.glfwSetCursorPos(window.handle(), d, e);
	...
	public static void updateRawMouseInput(Window window, boolean bl) {
		if (isRawMouseInputSupported()) {
			GLFW.glfwSetInputMode(window.handle(), GLFW_RAW_MOUSE_MOTION, bl ? 1 : 0);
		}
	}
	...
	public static enum Type {
		...
		MOUSE(
			"key.mouse",
			(integer, string) -> Language.getInstance().has(string) ? Component.translatable(string) : Component.translatable("key.mouse", new Object[]{integer + 1})
		);
