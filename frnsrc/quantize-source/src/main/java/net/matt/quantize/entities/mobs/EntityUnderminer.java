...
import net.matt.quantize.entities.ai.controllers.EtherealMoveController;
import net.matt.quantize.entities.ai.goals.MonsterAIWalkThroughHallsOfStructure;
...
public class EntityUnderminer extends PathfinderMob {
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.UNDERMINER_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.UNDERMINER_HURT.get();
    }
    ...
    protected void populateDefaultEquipmentSlots(RandomSource p_218949_, DifficultyInstance p_218950_) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(QItems.GHOSTLY_PICKAXE.get()));
    }
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            ...
            if(lastPosition != null && lastPosition.distSqr(this.blockPosition()) > 2.5F && Math.abs(distToFloor) < 0.5){
                this.playSound(QSounds.UNDERMINER_STEP.get(), 1F, 0.75F + random.nextFloat() * 0.25F);
            ...
            Player player = this.level().getNearestPlayer(this.getX(), this.getY(), this.getZ(), 8, true);
    ...
    public boolean wantsToPickUp(ItemStack stack) {
        return stack.is(QTags.UNDERMINER_ORES);
    }

    protected void pickUpItem(ItemEntity itemEntity) {
        ItemStack itemstack = itemEntity.getItem();
        if (itemstack.is(QTags.UNDERMINER_ORES)) {
    ...
    public boolean isExtraSpooky(){
        ...
        return Quantize.isAprilFools() || Quantize.isHalloween() || s != null && s.toLowerCase().contains("herobrine");
    }
    ...
    private boolean hasPick(){
        return this.getItemInHand(InteractionHand.MAIN_HAND).is(QItems.GHOSTLY_PICKAXE.get());
    }
