...
import net.matt.quantize.entities.ai.goals.BottomFeederAIWander;
...
public class EntityAlligatorSnappingTurtle extends Animal implements ISemiAquatic, Shearable, net.minecraftforge.common.IForgeShearable {
    ...
    public static EntityAlligatorSnappingTurtle create(EntityType<EntityAlligatorSnappingTurtle> type, Level world) {
        return new EntityAlligatorSnappingTurtle(type, world);
    }

    protected SoundEvent getAmbientSound() {
        return QSounds.ALLIGATOR_SNAPPING_TURTLE_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.ALLIGATOR_SNAPPING_TURTLE_HURT.get();
    }
    ...
    public static boolean canTurtleSpawn(EntityType type, LevelAccessor worldIn, MobSpawnType reason, BlockPos pos, RandomSource randomIn) {
        boolean spawnBlock = worldIn.getBlockState(pos.below()).is(QTags.ALLIGATOR_SNAPPING_TURTLE_SPAWNS);
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.ALLIGATOR_SNAPPING_TURTLE_BREEDABLES);
    }
    ...
    public void shear(SoundSource category) {
        ...
        if (!this.level().isClientSide()) {
            if (random.nextFloat() < this.getMoss() * 0.05F) {
                this.spawnAtLocation(QItems.SPIKED_SCUTE.get());
            } else {
    ...
    public java.util.List<ItemStack> onSheared(@Nullable Player player, @javax.annotation.Nonnull ItemStack item, Level world, BlockPos pos, int fortune) {
        ...
        if (!world.isClientSide()) {
            if (random.nextFloat() < this.getMoss() * 0.05F) {
                this.setMoss(0);
                return Collections.singletonList(new ItemStack(QItems.SPIKED_SCUTE.get()));
            } else {
                this.setMoss(0);
    ...
    public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
        return QEntities.ALLIGATOR_SNAPPING_TURTLE.get().create(p_241840_1_);
    }
}
