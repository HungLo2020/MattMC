...
import net.matt.quantize.block.block.PewenBranchBlock;
...
public class AtlatitanEntity extends SauropodBaseEntity implements KeybindUsingMount {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.1D, Ingredient.of(QBlocks.TREE_STAR.get()), false));
    ...
    protected void onStep() {
        if(!this.isBaby()){
            if (screenShakeAmount <= 1.0F) {
                this.playSound(QSounds.ATLATITAN_STEP.get(), 2, 1);
            }
            if (screenShakeAmount <= 1.0F) {
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            ...
            if(this.getRideableFor() > 0 && level().random.nextInt(2) == 0 && !this.isDancing()){
                ...
                this.level().addParticle(QParticles.HAPPINESS.get(), particlePos.x, particlePos.y, particlePos.z, ((double) this.random.nextFloat() - 0.5D) * 0.1D, ((double) this.random.nextFloat() - 0.5D) * 0.1D, ((double) this.random.nextFloat() - 0.5D) * 0.1D);
            }
            ...
            if(this.getAnimation() == ANIMATION_STOMP && this.getAnimationTick() == 30){
                this.playSound(QSounds.ATLATITAN_STOMP.get(), 3.0F, 1.0F);
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(QItems.SERENE_SALAD.get())) {
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QBlocks.TREE_STAR.get().asItem());
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!prev.consumesAction() && itemstack.is(QItems.SERENE_SALAD.get()) && !this.isBaby()) {
    ...
    public BlockPos getStandAtTreePos(BlockPos target) {
        ...
        if (state.is(QBlocks.PEWEN_BRANCH.get())) {
    ...
    public BlockState createEggBlockState() {
        return QBlocks.ATLATITAN_EGG.get().defaultBlockState();
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return QEntities.ATLATITAN.get().create(level);
    }
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.ATLATITAN_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.ATLATITAN_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.ATLATITAN_DEATH.get();
    }
}
