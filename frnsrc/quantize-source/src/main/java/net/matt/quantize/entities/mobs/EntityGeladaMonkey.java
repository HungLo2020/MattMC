...
import net.matt.quantize.entities.ai.goals.AnimalAIHerdPanic;
import net.matt.quantize.entities.ai.goals.GeladaAIGroom;
...
import net.matt.quantize.modules.entities.IHerdPanic;
...
public class EntityGeladaMonkey extends Animal implements IAnimatedEntity, IHerdPanic {
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.GELADA_MONKEY_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.GELADA_MONKEY_HURT.get();
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.0D, Ingredient.fromValues(Stream.of(new Ingredient.TagValue(QTags.GELADA_MONKEY_BREEDABLES), new Ingredient.TagValue(QTags.GELADA_MONKEY_LAND_CLEARING_FOODS))), false));
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.GELADA_MONKEY_BREEDABLES);
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if(itemstack.is(QTags.GELADA_MONKEY_LAND_CLEARING_FOODS) && this.getClearGrassTime() == 0){
    ...
    public AgeableMob getBreedOffspring(ServerLevel lvl, AgeableMob mob) {
        EntityGeladaMonkey baby = QEntities.GELADA_MONKEY.get().create(lvl);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, MobSpawnType reason, @javax.annotation.Nullable SpawnGroupData spawnDataIn, @javax.annotation.Nullable CompoundTag dataTag) {
        if (spawnDataIn instanceof AgeableMobGroupData) {
            AgeableMobGroupData pack = (AgeableMobGroupData) spawnDataIn;
    ...
    private class AIClearGrass extends Goal {
        ...
        public boolean canContinueToUse() {
            return target != null && EntityGeladaMonkey.this.level().getBlockState(target).is(QTags.GELADA_MONKEY_GRASS);
        }
        ...
        public BlockPos generateTarget() {
            ...
            for (int i = 0; i < 15; i++) {
                ...
                if (EntityGeladaMonkey.this.level().getBlockState(blockpos1).is(QTags.GELADA_MONKEY_GRASS)) {
                    blockpos = blockpos1;
                }
            }
            return blockpos;
