...
import net.matt.quantize.entities.ai.goals.AnimalBreedEggsGoal;
import net.matt.quantize.entities.ai.goals.AnimalLayEggGoal;
import net.matt.quantize.entities.ai.goals.GrottoceratopsEatPlantsGoal;
import net.matt.quantize.entities.ai.goals.GrottoceratopsMeleeGoal;
...
import net.matt.quantize.modules.entities.LegSolverQuadruped;
...
public class GrottoceratopsEntity extends DinosaurEntity implements IAnimatedEntity {
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if (!this.isBaby()) {
            this.playSound(QSounds.GROTTOCERATOPS_STEP.get(), 0.7F, 0.85F);
        }
    }
    ...
    public void tick() {
        ...
        if (this.getAnimation() == ANIMATION_CHEW && this.getAnimationTick() == 2 || this.getAnimation() == ANIMATION_CHEW_FROM_GROUND && this.getAnimationTick() == 10) {
            this.playSound(QSounds.GROTTOCERATOPS_GRAZE.get());
        }
    ...
    public BlockState createEggBlockState() {
        return QBlocks.GROTTOCERATOPS_EGG.get().defaultBlockState();
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return QEntities.GROTTOCERATOPS.get().create(level);
    }
    ...
    public void actuallyPlayAmbientSound() {
        ...
        if (this.getAnimation() == ANIMATION_SPEAK_2) {
            soundevent = QSounds.GROTTOCERATOPS_CALL.get();
            volume += 1.0F;
        }
        if (soundevent != null) {
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.GROTTOCERATOPS_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.GROTTOCERATOPS_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.GROTTOCERATOPS_DEATH.get();
    }
