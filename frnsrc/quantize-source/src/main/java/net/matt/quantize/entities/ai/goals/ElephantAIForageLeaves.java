...
public class ElephantAIForageLeaves extends MoveToBlockGoal {
    ...
    private void breakLeaves() {
        if (net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(elephant.level(), elephant)) {
            ...
            if (blockstate.is(QTags.ELEPHANT_FOODBLOCKS)) {
                ...
                if (blockstate.is(QTags.DROPS_ACACIA_BLOSSOMS) && rand.nextInt(30) == 0) {
                    ItemStack banana = new ItemStack(QItems.ACACIA_BLOSSOM.get());
    ...
    protected boolean isValidTarget(LevelReader worldIn, BlockPos pos) {
        return !elephant.aiItemFlag && worldIn.getBlockState(pos).is(QTags.ELEPHANT_FOODBLOCKS) && canSeeBlock(pos);
    }
