    ...
    public static final RegistryObject<MenuType<ElectricFurnaceMenu>> ELECTRIC_FURNACE_MENU =
            MENUS.register("electric_furnace_menu",
                    () -> IForgeMenuType.create((id, inv, data) -> {
                        ...
                        ElectricFurnaceBlockEntity blockEntity = (ElectricFurnaceBlockEntity) inv.player.getCommandSenderWorld().getBlockEntity(pos);
                        return new ElectricFurnaceMenu(id, inv, blockEntity);
                    }));
    private final ElectricFurnaceBlockEntity blockEntity;

    public ElectricFurnaceMenu(int id, Inventory playerInventory, ElectricFurnaceBlockEntity blockEntity) {
        super(ELECTRIC_FURNACE_MENU.get(), id);
