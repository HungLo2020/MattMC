...
import net.matt.quantize.entities.ai.goals.AnimalAIFollowParentRanged;
import net.matt.quantize.entities.ai.goals.AnimalAIHurtByTargetNotBaby;
import net.matt.quantize.entities.ai.goals.AnimalAIRandomSwimming;
...
public class EntityCachalotWhale extends Animal {
    ...
    public static EntityCachalotWhale create(EntityType<EntityCachalotWhale> type, Level world) {
        return new EntityCachalotWhale(type, world);
    }
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.CACHALOT_WHALE_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.CACHALOT_WHALE_HURT.get();
    }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(2, new EntityAINearestTarget3D(this, LivingEntity.class, 30, false, true, QEntities.buildPredicateFromTag(QTags.CACHALOT_WHALE_TARGETS)) {
            public boolean canUse() {
                return !EntityCachalotWhale.this.isSleeping() && !EntityCachalotWhale.this.isBeached() && super.canUse();
            }
        });
    ...
    public void breakBlock() {
        ...
        if (!this.level().isClientSide && this.blockBreakCounter == 0 && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(level(), this)) {
            final TagKey<Block> breakables = this.isCharging() && this.getTarget() != null && true ? QTags.CACHALOT_WHALE_BREAKABLES : QTags.ORCA_BREAKABLES;
    ...
    private void spawnSpoutParticles() {
        if (this.isAlive()) {
            ...
            for (int j = 0; j < 5 + random.nextInt(4); ++j) {
                ...
                this.level().addParticle(QParticles.WHALE_SPLASH.get(), this.headPart.getX() + extraX, this.headPart.getY() + this.headPart.getBbHeight(), this.headPart.getZ() + extraZ, motX * 0.1F + this.getDeltaMovement().x, 2F, motZ * 0.1F + this.getDeltaMovement().z);
            }
        }
    }
    ...
    protected float getSoundVolume() {
        return this.isSilent() ? 0 : (float) 3;
    }
        ...
        if (rewardPlayer != null && !hasRewardedPlayer && this.isInWaterOrBubble()) {
            ...
            if (this.distanceTo(rewardPlayer) < 10F) {
                if (!this.level().isClientSide) {
                    ...
                    final ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(QItems.AMBERGRIS.get(), 2 + random.nextInt(2)));
            ...
            if (target == null || !target.isAlive()) {
                ...
                if (isGrabbing() && this.getTarget().isAlive()) {
                    ...
                    if (waitForEchoFlag && !receivedEcho) {
                        ...
                        if (echoTimer % 10 == 0) {
                            if (echoTimer % 40 == 0) {
                                this.playSound(QSounds.CACHALOT_WHALE_CLICK.get(), this.getSoundVolume(), this.getVoicePitch());
                            ...
                            if (this.isCharging()) {
                                if (this.distanceTo(target) < this.getBbWidth() && chargeProgress > 4) {
                                    ...
                                    if (random.nextInt(10) == 0) {
                                        ...
                                        ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(QItems.CACHALOT_WHALE_TOOTH.get()));
                                        itementity.setDefaultPickUpDelay();
                                        level().addFreshEntity(itementity);
                                    }
                                }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        EntityCachalotWhale whale = QEntities.CACHALOT_WHALE.get().create(serverWorld);
