...
import net.matt.quantize.modules.entities.raycoms.pathjobs.AbstractPathJob;
import net.matt.quantize.modules.entities.raycoms.pathjobs.PathJobMoveAwayFromLocation;
import net.matt.quantize.modules.entities.raycoms.pathjobs.PathJobMoveToLocation;
import net.matt.quantize.modules.entities.raycoms.pathjobs.PathJobRandomPos;
import net.matt.quantize.modules.entities.raycoms.WorldChunkUtil;
...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    public PathResult setPathJob(
        ...
        if (speedFactor > MAX_SPEED_ALLOWED || speedFactor < MIN_SPEED_ALLOWED) {
            Quantize.LOGGER.error("Tried to set a bad speed:" + speedFactor + " for entity:" + ourEntity, new Exception());
            return null;
        }
        ...
        if (pathResult != null) {
            if (!pathResult.isFinished()) {
                ...
                try {
                    ...
                    Quantize.LOGGER.error("Exception occurred", e);
                }
            }
        }
    ...
    public void setSpeedModifier(final double speedFactor) {
        if (speedFactor > MAX_SPEED_ALLOWED || speedFactor < MIN_SPEED_ALLOWED) {
            Quantize.LOGGER.debug("Tried to set a bad speed:" + speedFactor + " for entity:" + ourEntity);
            return;
        }
