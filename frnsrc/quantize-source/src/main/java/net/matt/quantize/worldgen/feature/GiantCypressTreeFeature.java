...
public class GiantCypressTreeFeature extends Feature<RuTreeConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<RuTreeConfiguration> context) {
        ...
        while (placeCheck <= height_main) {
            if (isReplaceableDirt(level, placePos) || level.getBlockState(placePos).canBeReplaced()||level.getBlockState(placePos).is(BlockTags.LEAVES)||level.getBlockState(placePos).is(BlockTags.LEAVES)) {
