...
public class RelicheirusEntity extends DinosaurEntity implements IAnimatedEntity {
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if (!this.isBaby()) {
            this.playSound(QSounds.RELICHEIRUS_STEP.get(), 1.0F, 1.0F);
        }
    }
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(QItems.PRIMORDIAL_SOUP.get())) {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!prev.consumesAction() && itemstack.is(QItems.PRIMORDIAL_SOUP.get())) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return QEntities.RELICHEIRUS.get().create(level);
    }
    ...
    public BlockState createEggBlockState() {
        return QBlocks.RELICHEIRUS_EGG.get().defaultBlockState();
    }
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.RELICHEIRUS_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.RELICHEIRUS_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.RELICHEIRUS_DEATH.get();
    }
}
