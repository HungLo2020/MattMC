...
public class DClientProxy {
    ...
    public DClientProxy(FMLJavaModLoadingContext context) {
    ...
    private void registerShaderForLoading(RegisterShadersEvent event, ResourceLocation shaderResourceLocation, VertexFormat format) {
        try {
            Quantize.LOGGER.debug("Registering shader {} for loading", shaderResourceLocation);
            ...
            event.registerShader(new DarkShaderInstance(event.getResourceProvider(), shaderResourceLocation, format), (ShaderInstance shaderInstance) -> {
                Quantize.LOGGER.debug("Shader {} has loaded, resolving promise", shaderResourceLocation);
                ON_SHADERS_LOADED.get(shaderResourceLocation).setSuccess(shaderInstance);
            });
        } catch (IOException e) {
            Quantize.LOGGER.trace("Failed to register shader", e);
        }
    }
    ...
    public static boolean isElementNameBlacklisted(String elementName) {
        return BLACKLISTED_ELEMENTS.computeIfAbsent(elementName, (String name) -> {
            Quantize.LOGGER.debug("Considering {} for element blacklist", name);
            RenderedClassesTracker.add(name);
            return blacklistContains(MODDED_BLACKLIST, name) || blacklistContains(QClientConfig.CLIENT.METHOD_SHADER_BLACKLIST.get(), name);
        });
    }
    ...
    private Tooltip getShaderSwitchButtonTooltip() {
        MutableComponent tooltipComponent = (SELECTED_SHADER_VALUE == null ? Component.translatable("gui." + Quantize.MOD_ID + ".light_mode") : SELECTED_SHADER_VALUE.displayName).plainCopy();
        tooltipComponent.append(Component.literal("\n"));
        tooltipComponent.append(Component.translatable("gui.tooltip." + Quantize.MOD_ID + ".shader_switch_tooltip").withStyle(ChatFormatting.GRAY));
