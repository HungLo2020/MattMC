...
import net.matt.quantize.block.block.SpanishMossBlock;
import net.matt.quantize.worldgen.feature.RuTreeConfiguration;
...
public class CypressTreeFeature extends Feature<RuTreeConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<RuTreeConfiguration> context) {
        ...
        //System.out.println("[CypressTreeFeature] place called at; HasVines: " + hasVines);
    ...
    private static void placeVine(LevelAccessor level, BlockPos pos) {
        ...
        //System.out.println("[CypressTreeFeature] placeVine called at anchor " + pos);
        ...
        for(int i = 0; i<=size; i++){
            if(level.getBlockState(place).isAir()){
                ...
                //System.out.println("[CypressTreeFeature] Air at " + place + " placing " + (i == size ? "HEAD" : "PLANT"));

                if(i==size){
                    level.setBlock(place, QBlocks.SPANISH_MOSS.get().defaultBlockState().setValue(SpanishMossBlock.AGE, 25), 2);
                }
                else {
                    level.setBlock(place, QBlocks.SPANISH_MOSS_PLANT.get().defaultBlockState(), 2);
                }
                place = place.below();
            }
            else{

                //System.out.println("[CypressTreeFeature] Blocked at " + place + " with " + level.getBlockState(place) + " i=" + i);

                if(i!=0){

                    BlockPos headPos = place.above();
                    //System.out.println("[CypressTreeFeature] Placing HEAD at " + headPos + " instead");

                    level.setBlock(place.above(), QBlocks.SPANISH_MOSS.get().defaultBlockState().setValue(SpanishMossBlock.AGE, 25), 2);
                }
                break;
            }
        }
    ...
    public void placeNorthBranch(LevelAccessor level, BlockPos pos, RandomSource randomSource, RuTreeConfiguration treeConfiguration) {
        if(level.getBlockState(pos.north()).canBeReplaced()&&!level.isOutsideBuildHeight(pos.north())){
            ...
            level.setBlock(pos.west(), treeConfiguration.branchProvider.getState(randomSource, pos), 2);
        }
    ...
    public static boolean isReplaceableDirtBlock(BlockState state) {
        return state.is(QTags.TREE_GRASS_REPLACEABLES);
    }
    ...
    public static boolean isReplaceableBlock(BlockState state) {
        return state.is(QTags.REPLACEABLE_BLOCKS);
    }
