...
import net.matt.quantize.entities.models.ModelBaldEagle;
import net.matt.quantize.entities.mobs.EntityBaldEagle;
...
public class RenderBaldEagle extends MobRenderer<EntityBaldEagle, ModelBaldEagle> {
    private static final ResourceLocation TEXTURE = new ResourceIdentifier("textures/model/entity/bald_eagle/bald_eagle.png");
    private static final ResourceLocation TEXTURE_CAP = new ResourceIdentifier("textures/model/entity/bald_eagle/bald_eagle_hood.png");
    ...
    protected void scale(EntityBaldEagle eagle, PoseStack matrixStackIn, float partialTickTime) {
        if(eagle.isPassenger() && eagle.getVehicle() != null) {
            if (eagle.getVehicle() instanceof Player) {
                ...
                if(mount.getItemInHand(InteractionHand.MAIN_HAND).getItem() == QItems.FALCONRY_GLOVE.get()){
                    leftHand = mount.getMainArm() == HumanoidArm.LEFT;
                }else if(mount.getItemInHand(InteractionHand.OFF_HAND).getItem() == QItems.FALCONRY_GLOVE.get()){
                    leftHand = mount.getMainArm() != HumanoidArm.LEFT;
                }
