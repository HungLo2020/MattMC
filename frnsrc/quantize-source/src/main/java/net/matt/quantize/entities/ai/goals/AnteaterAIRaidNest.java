...
import net.matt.quantize.block.block.BlockLeafcutterAnthill;
...
public class AnteaterAIRaidNest extends MoveToBlockGoal {
    ...
    public static final ResourceLocation ANTEATER_REWARD = new ResourceIdentifier("gameplay/anteater_reward");
    ...
    private void breakHiveEffect(){
        if (net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(anteater.level(), anteater)) {
            ...
            if (blockstate.is(QBlocks.LEAFCUTTER_ANTHILL.get())) {
            ...
            }else if(blockstate.is(QBlocks.LEAFCUTTER_ANT_CHAMBER.get())){
                anteater.level().destroyBlock(blockPos, false);
                anteater.level().setBlockAndUpdate(blockPos, blockstate);
            }
        }
            ...
            if (blockstate.is(QBlocks.LEAFCUTTER_ANTHILL.get())) {
                if (anteater.level().getBlockEntity(this.blockPos) instanceof TileEntityLeafcutterAnthill) {
                    ...
                    if(!anthill.hasNoAnts()){
                        ...
                        if(state != null && state.is(QBlocks.LEAFCUTTER_ANT_CHAMBER.get()) && state.getValue(BlockLeafcutterAntChamber.FUNGUS) >= 5){
                            ItemStack stack = new ItemStack(QItems.GONGYLIDIA.get());
    ...
    protected boolean isValidTarget(LevelReader worldIn, BlockPos pos) {
        return worldIn.getBlockState(pos).is(QBlocks.LEAFCUTTER_ANT_CHAMBER.get()) || worldIn.getBlockState(pos).is(QBlocks.LEAFCUTTER_ANTHILL.get()) && worldIn.getBlockEntity(pos) instanceof TileEntityLeafcutterAnthill && this.isValidAnthill(pos, (TileEntityLeafcutterAnthill)worldIn.getBlockEntity(pos));
    }
