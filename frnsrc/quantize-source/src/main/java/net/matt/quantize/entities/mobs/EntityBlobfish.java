...
public class EntityBlobfish extends WaterAnimal implements FlyingAnimal, Bucketable {
    ...
    public static EntityBlobfish create(EntityType<EntityBlobfish> type, Level level) {
        return new EntityBlobfish(type, level);
    }
    ...
    public ItemStack getBucketItemStack() {
        ItemStack stack = new ItemStack(QItems.BLOBFISH_BUCKET.get());
    ...
    public static boolean canBlobfishSpawn(EntityType<EntityBlobfish> entityType, ServerLevelAccessor iServerWorld, MobSpawnType reason, BlockPos pos, RandomSource random) {
        return reason == MobSpawnType.SPAWNER || pos.getY() <= 25 && iServerWorld.getFluidState(pos).is(FluidTags.WATER) && iServerWorld.getFluidState(pos.above()).is(FluidTags.WATER);
    }
