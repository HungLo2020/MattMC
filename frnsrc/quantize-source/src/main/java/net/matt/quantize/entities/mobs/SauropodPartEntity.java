...
import net.matt.quantize.network.packet.MultipartEntityMessage;
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        ...
        if (parent == null) {
            ...
            if (player.level().isClientSide) {
                NetworkHandler.sendMSGToServer(new MultipartEntityMessage(parent.getId(), player.getId(), 0, 0));
            }
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        if (!this.isInvulnerableTo(source) && parent != null) {
            ...
            if (player != null && !parent.isAlliedTo(player) && player.level().isClientSide) {
                NetworkHandler.sendMSGToServer(new MultipartEntityMessage(parent.getId(), player.getId(), 1, amount));
            }
        }
        return false;
    }
