...
public class GiantAncientTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int height = 0; height <= maxHeight; height++) {
            ...
            if (height == maxHeight || height > 3 && (height - lastLeavesY > 4 + randomsource.nextInt(2) || randomsource.nextInt(5) == 0)) {
                lastLeavesY = height;
                drawLeafOrb(level, trunkRoot.offset((int) ((randomsource.nextInt(2) - 1) * radShrink), 0, (int) ((randomsource.nextInt(2) - 1) * radShrink)), randomsource, QBlocks.ANCIENT_LEAVES.get().defaultBlockState(), (int) Math.ceil(4 * leavesShrink) + randomsource.nextInt(2), 1 + randomsource.nextInt(2), (int) Math.ceil(4 * leavesShrink) + randomsource.nextInt(2));
            }
        }
    ...
    private static boolean canReplace(BlockState state) {
        return (state.isAir() || state.canBeReplaced() || state.is(QBlocks.ANCIENT_LEAVES.get()) || state.is(QBlocks.TREE_STAR.get())) && state.getFluidState().isEmpty() && !state.is(QTags.UNMOVEABLE);
    }
