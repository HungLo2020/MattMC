    ...
    public static class CitadelThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(final @NotNull Runnable runnable) throws RuntimeException {
            ...
            if (thread.getContextClassLoader() != classLoader) {
                Quantize.LOGGER.info("Corrected CCL of new Citadel Pathfinding Thread, was: " + thread.getContextClassLoader().toString());
                thread.setContextClassLoader(classLoader);
            }
            thread.setUncaughtExceptionHandler((thread1, throwable) -> Quantize.LOGGER.error("Citadel Pathfinding Thread errored! ", throwable));
            return thread;
        }
    }
}
