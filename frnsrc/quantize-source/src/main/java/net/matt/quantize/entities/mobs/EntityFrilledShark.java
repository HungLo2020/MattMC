...
public class EntityFrilledShark extends WaterAnimal implements IAnimatedEntity, Bucketable {
    ...
    public static EntityFrilledShark create(EntityType<EntityFrilledShark> type, Level world) {
        return new EntityFrilledShark(type, world);
    }
    ...
    public ItemStack getBucketItemStack() {
        ItemStack stack = new ItemStack(QItems.FRILLED_SHARK_BUCKET.get());
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide && this.getTarget() != null && this.getAnimation() == ANIMATION_ATTACK && this.getAnimationTick() == 12) {
            ...
            if (this.getTarget().hurt(this.damageSources().mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue())){
                this.getTarget().addEffect(new MobEffectInstance(QEffects.EXSANGUINATION.get(), 60, 2));
                if(random.nextInt(15) == 0 && this.getTarget() instanceof Squid){
                    this.spawnAtLocation(QItems.SERRATED_SHARK_TOOTH.get());
                }
            }
    ...
    public void handleEntityEvent(byte id) {
        if (id == 68) {
            ...
            level().addParticle(QParticles.TEETH_GLINT.get(), x, y, z, this.getDeltaMovement().x, this.getDeltaMovement().y, this.getDeltaMovement().z);
        } else {
