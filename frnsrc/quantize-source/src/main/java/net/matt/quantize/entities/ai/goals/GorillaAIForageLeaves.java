...
public class GorillaAIForageLeaves extends MoveToBlockGoal {
    ...
    private void breakLeaves() {
        if (net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(gorilla.level(), gorilla)) {
            ...
            if (blockstate.is(QTags.GORILLA_BREAKABLES)) {
                ...
                /*if(blockstate.is(QTags.DROPS_BANANAS) && rand.nextInt(30) == 0){
                    ItemStack banana = new ItemStack(QItems.BANANA.get());
    ...
    protected boolean isValidTarget(LevelReader worldIn, BlockPos pos) {
        return worldIn.getBlockState(pos).is(QTags.GORILLA_BREAKABLES);
    }
}
