...
import net.matt.quantize.block.block.MultipleDinosaurEggsBlock;
...
import net.matt.quantize.modules.entities.FlyingMount;
...
import net.matt.quantize.modules.entities.PackAnimal;
...
import net.matt.quantize.modules.entities.raycoms.AdvancedPathNavigate;
...
public class SubterranodonEntity extends DinosaurEntity implements PackAnimal, FlyingAnimal, KeybindUsingMount, FlyingMount {
    ...
    public static boolean checkSubterranodonSpawnRules(EntityType<? extends Animal> type, LevelAccessor levelAccessor, MobSpawnType mobType, BlockPos pos, RandomSource randomSource) {
        ...
        return (below.is(QTags.DINOSAURS_SPAWNABLE_ON) || below.is(QBlocks.PEWEN_BRANCH.get()) || below.is(BlockTags.LEAVES)) && levelAccessor.getFluidState(pos).isEmpty() && levelAccessor.getFluidState(pos.below()).isEmpty();
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.1D, Ingredient.of(Items.COD, Items.COOKED_COD, QItems.COOKED_TRILOCARIS_TAIL.get(), QItems.TRILOCARIS_TAIL.get()), false));
    ...
    private PathNavigation createMultithreadedPathFinder(boolean flying){
        ...
        return new AdvancedPathNavigateNoTeleport(this, level(), flying ? AdvancedPathNavigate.MovementType.FLYING : AdvancedPathNavigate.MovementType.WALKING, climbing, wide);
    }
    ...
    public void tick() {
        ...
        if (isFlying()) {
            if(timeFlying % 10 == 0 && (flapAmount > 0 || controlUpTicks > 0)){
                this.playSound(QSounds.SUBTERRANODON_FLAP.get());
            }
            timeFlying++;
        ...
        if (!level().isClientSide) {
            ...
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (Quantize.PROXY.isKeyDown(0) && !Quantize.PROXY.isKeyDown(1) && controlUpTicks < 2 && getMeterAmount() > 0.1F) {
                    if (getMeterAmount() > 0.1F) {
                        NetworkHandler.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 0));
                        controlUpTicks = 5;
                    }
                }
                if (Quantize.PROXY.isKeyDown(1) && !Quantize.PROXY.isKeyDown(0) && controlDownTicks < 2) {
                    NetworkHandler.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 1));
                    controlDownTicks = 5;
                }
            }
        }
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            ...
            if (attackProgress == 5F && target != null && this.distanceTo(target) < 3D + target.getBbWidth() && this.hasLineOfSight(target)) {
                ...
                this.playSound(QSounds.SUBTERRANODON_ATTACK.get());
            }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return QEntities.SUBTERRANODON.get().create(serverLevel);
    }
    ...
    public BlockState createEggBlockState() {
        return QBlocks.SUBTERRANODON_EGG.get().defaultBlockState().setValue(MultipleDinosaurEggsBlock.EGGS, 1 + random.nextInt(3));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (prev != InteractionResult.SUCCESS) {
            ...
            if (!this.isTame() && (itemStack.is(QItems.TRILOCARIS_TAIL.get()) || itemStack.is(QItems.COOKED_TRILOCARIS_TAIL.get()))) {
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.SUBTERRANODON_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.SUBTERRANODON_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.SUBTERRANODON_DEATH.get();
    }
