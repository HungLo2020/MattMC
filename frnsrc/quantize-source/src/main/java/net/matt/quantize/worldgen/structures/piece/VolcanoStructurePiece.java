package net.matt.quantize.worldgen.structures.piece;

import net.matt.quantize.block.QBlocks;
import net.matt.quantize.block.block.PrimalMagmaBlock;
                ...
                if(dist < 1.0F){
                    ...
                    while (carve.getY() < generateHeight) {
                        ...
                        if (beneathAmount == generateCoreHeight && carve.getX() == center.getX() && carve.getZ() == center.getZ()) {
                            checkedSetBlock(level, carve, QBlocks.VOLCANIC_CORE.get().defaultBlockState());
                            ...
                            checkedSetBlock(level, carve, QBlocks.PRIMAL_MAGMA.get().defaultBlockState().setValue(PrimalMagmaBlock.ACTIVE, true).setValue(PrimalMagmaBlock.PERMANENT, true));
                        } else {
                            ...
                            if (volcanoCurve < 0.035F) {
                                if (random.nextFloat() * 0.13F < volcanoCurve) {
                                    ...
                                    checkedSetBlock(level, carve, QBlocks.LIMESTONE.get().defaultBlockState());
                                }
                            } else if (magmaVeinNoise < 0.075F && magmaVeinNoise > -0.075F) {
                                checkedSetBlock(level, carve, QBlocks.PRIMAL_MAGMA.get().defaultBlockState());
                            } else {
                                checkedSetBlock(level, carve, QBlocks.FLOOD_BASALT.get().defaultBlockState());
                            }
                        }
                        carve.move(0, 1, 0);
                    }
