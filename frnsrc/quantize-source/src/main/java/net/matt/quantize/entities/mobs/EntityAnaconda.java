...
public class EntityAnaconda extends Animal implements ISemiAquatic {
    ...
    public static EntityAnaconda create(EntityType<EntityAnaconda> type, Level world) {
        return new EntityAnaconda(type, world);
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.ANACONDA_HURT.get();
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if (!isBaby()) {
            this.playSound(QSounds.ANACONDA_SLITHER.get(), 1.0F, 1.0F);
        } else {
    ...
    public static boolean canAnacondaSpawn(EntityType type, LevelAccessor worldIn, MobSpawnType reason, BlockPos pos, RandomSource randomIn) {
        final boolean spawnBlock = worldIn.getBlockState(pos.below()).is(QTags.ANACONDA_SPAWNS);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.25D, Ingredient.of(QTags.ANACONDA_FOODSTUFFS), false));
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal(this, LivingEntity.class, 200, false, false, QEntities.buildPredicateFromTag(QTags.ANACONDA_TARGETS)));
    ...
    public void tick() {
        super.tick();
        //System.out.println("EntityAnaconda Position: " + this.blockPosition());
        ...
        if (!this.level().isClientSide) {
            ...
            if (child == null) {
                ...
                for (int i = 0; i < segments; i++) {
                    ...
                    EntityAnacondaPart part = new EntityAnacondaPart(QEntities.ANACONDA_PART.get(), this);
...
//                float headRot = Mth.wrapDegrees(this.getYRot());
        ...
        if (this.getSheddingTime() > 0) {
            ...
            if (this.getSheddingTime() == 0) {
                this.spawnItemAtOffset(new ItemStack(QItems.SHED_SNAKE_SKIN.get()), 1 + random.nextFloat(), 0.2F);
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.ANACONDA_FOODSTUFFS);
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob mob) {
        EntityAnaconda anaconda = QEntities.ANACONDA.get().create(serverWorld);
        ...
        public void tick() {
            ...
            if (target != null && target.isAlive()) {
                if (jumpAttemptCooldown == 0 && snake.distanceTo(target) < 1 + target.getBbWidth() && !snake.isStrangling()) {
                    ...
                    snake.playSound(QSounds.ANACONDA_ATTACK.get(), snake.getSoundVolume(), snake.getVoicePitch());
