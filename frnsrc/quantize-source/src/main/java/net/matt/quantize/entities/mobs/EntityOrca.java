...
import net.matt.quantize.entities.ai.goals.OrcaAIJump;
import net.matt.quantize.entities.ai.goals.OrcaAIMeleeJump;
import net.matt.quantize.entities.ai.navigators.SwimmerJumpPathNavigator;
import net.matt.quantize.entities.ai.goals.OrcaAIMelee;
...
public class EntityOrca extends TamableAnimal implements IAnimatedEntity {
    ...
    public static EntityOrca create(EntityType<EntityOrca> type, Level world) {
        return new EntityOrca(type, world);
    }
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, MobSpawnType spawnReasonIn) {
        return QEntities.rollSpawn(6, this.getRandom(), spawnReasonIn);
    }
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.ORCA_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.ORCA_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.ORCA_DIE.get();
    }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(3, new EntityAINearestTarget3D(this, LivingEntity.class, 200, false, true, QEntities.buildPredicateFromTag(QTags.ORCA_TARGETS)));
    }
            ...
            for (int a = (int) Math.round(this.getBoundingBox().minX); a <= (int) Math.round(this.getBoundingBox().maxX); a++) {
                for (int b = (int) Math.round(this.getBoundingBox().minY) - 1; (b <= (int) Math.round(this.getBoundingBox().maxY) + 1) && (b <= 127); b++) {
                    for (int c = (int) Math.round(this.getBoundingBox().minZ); c <= (int) Math.round(this.getBoundingBox().maxZ); c++) {
                        ...
                        if (!state.isAir() && !state.getShape(level(), pos).isEmpty() && state.is(QTags.ORCA_BREAKABLES) && fluidState.isEmpty()) {
                            if (block != Blocks.AIR) {
    ...
    public void tick() {
        super.tick();
        //System.out.println("Mob: " + this.getName().getString() + " | Location: X=" + this.getX() + ", Y=" + this.getY() + ", Z=" + this.getZ());
        if (jumpCooldown > 0) {
            jumpCooldown--;
        ...
        if (attackTarget != null && attackTarget instanceof Player && attackTarget.hasEffect(QEffects.ORCAS_MIGHT.get())) {
            attackTarget.removeEffect(QEffects.ORCAS_MIGHT.get());
        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob p_241840_2_) {
        return QEntities.ORCA.get().create(serverWorld);
    }
    ...
    static class SwimWithPlayerGoal extends Goal {
        ...
        public void tick() {
            ...
            if (this.targetPlayer.isSwimming() && this.targetPlayer.level().random.nextInt(6) == 0) {
                this.targetPlayer.addEffect(new MobEffectInstance(QEffects.ORCAS_MIGHT.get(), 1000));
            }
        }
    }
