...
public class EntityEnderiophage extends Animal implements Enemy, FlyingAnimal {
    ...
    private static final Predicate<LivingEntity> ENDERGRADE_OR_INFECTED = (entity) -> {
        return entity instanceof EntityEndergrade || entity.hasEffect(QEffects.ENDER_FLU.get());
    };
    ...
    public static EntityEnderiophage create(EntityType<EntityEnderiophage> type, Level world) {
        return new EntityEnderiophage(type, world);
    }
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, MobSpawnType spawnReasonIn) {
        return QEntities.rollSpawn(2, this.getRandom(), spawnReasonIn);
    }
                ...
                if (mount instanceof LivingEntity) {
                    ...
                    if (!this.level().isClientSide && attachTime > 15) {
                            ...
                            if (mount instanceof EnderMan) {
                                ...
                                if (random.nextInt(3) == 0) {
                                    if (!this.isMissingEye()) {
                                        if (target.getEffect(QEffects.ENDER_FLU.get()) == null) {
                                            target.addEffect(new MobEffectInstance(QEffects.ENDER_FLU.get(), 12000));
                                        } else {
                                            MobEffectInstance inst = target.getEffect(QEffects.ENDER_FLU.get());
                                            int duration = 12000;
                                            int level = 0;
                                            if (inst != null) {
                                            ...
                                            target.removeEffect(QEffects.ENDER_FLU.get());
                                            target.addEffect(new MobEffectInstance(QEffects.ENDER_FLU.get(), duration, Math.min(level + 1, 4)));
                                        }
                                        this.heal(5);
                        ...
                        if (((LivingEntity) mount).getHealth() <= 0 || this.fleeAfterStealTime > 0 || this.isMissingEye() && !(mount instanceof EnderMan) || !this.isMissingEye() && mount instanceof EnderMan) {
                            ...
                            this.setTarget(null);
                            dismountCooldown = 100;
                            NetworkHandler.sendMSGToAll(new MessageMosquitoDismount(this.getId(), mount.getId()));
                            this.setFlying(true);
                        }
                    }
                }
    ...
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.ENDERIOPHAGE_HURT.get();
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        this.playSound(QSounds.ENDERIOPHAGE_WALK.get(), 0.4F, 1.0F);
    }
    ...
    public void tick() {
        ...
        if (this.isAlive() && this.isFlying() && randomMotionSpeed > 0.75F && this.getDeltaMovement().lengthSqr() > 0.02D) {
            if (this.level().isClientSide) {
                ...
                this.level().addParticle(QParticles.DNA.get(), this.getX() + extraX, this.getY() + extraY, this.getZ() + extraZ, motX, motY, motZ);
            }
        }
        ...
        if (this.phageRotation < Mth.PI) {
            ...
            if ((double) f > 0.75D) {
                if (squishCooldown == 0 && this.isFlying()) {
                    squishCooldown = 20;
                    this.playSound(QSounds.ENDERIOPHAGE_SQUISH.get(), 3F, this.getVoicePitch());
                }
            ...
            if (parentEntity.getTarget() != null) {
                ...
                if (parentEntity.dismountCooldown == 0 && parentEntity.getBoundingBox().inflate(0.3, 0.3, 0.3).intersects(parentEntity.getTarget().getBoundingBox()) && !isBittenByPhage(parentEntity.getTarget())) {
                    ...
                    if (!parentEntity.level().isClientSide) {
                        NetworkHandler.sendMSGToAll(new MessageMosquitoMountPlayer(parentEntity.getId(), parentEntity.getTarget().getId()));
                    }
                }
            }
        }
