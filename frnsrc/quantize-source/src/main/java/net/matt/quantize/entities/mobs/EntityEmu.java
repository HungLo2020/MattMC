...
public class EntityEmu extends Animal implements IAnimatedEntity, IHerdPanic {
    ...
    public EntityEmu(EntityType type, Level world) {
        super(type, world);
    }
    ...
    public static <T extends Mob> boolean canEmuSpawn(EntityType<? extends Animal> animal, LevelAccessor worldIn, MobSpawnType reason, BlockPos pos, RandomSource random) {
        boolean spawnBlock = worldIn.getBlockState(pos.below()).is(QTags.EMU_SPAWNS);
    ...
    public static EntityEmu create(EntityType<EntityEmu> type, Level world) {
        return new EntityEmu(type, world);
    }

    protected SoundEvent getAmbientSound() {
        return QSounds.EMU_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.EMU_HURT.get();
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.1D, Ingredient.of(QTags.EMU_BREEDABLES), false));
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.EMU_BREEDABLES);
    }
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide && this.isAlive() && !this.isBaby() && --this.timeUntilNextEgg <= 0) {
            ...
            this.spawnAtLocation(QItems.EMU_EGG.get());
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        EntityEmu emu = QEntities.EMU.get().create(serverWorld);
