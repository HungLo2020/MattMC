...
import net.matt.quantize.modules.entities.TephraExplosion;
...
public class TephraEntity extends Projectile {
    ...
    public TephraEntity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(QEntities.TEPHRA.get(), level);
    }
    ...
    public void tick() {
        ...
        if(!playedSpawnSound){
            ...
            this.playSound(QSounds.TEPHRA_WHISTLE.get(), 8.0F, Mth.clamp(2.0F - this.getMaxScale() * 0.5F, 0.5F, 2.0F));
        }
        if (!level().isClientSide) {
            ...
            for (int j = 0; j < 1 + random.nextInt(2); ++j) {
                ...
                this.level().addAlwaysVisibleParticle(QParticles.TEPHRA.get(), true, this.getRandomX(this.getScale()) + this.getDeltaMovement().x, this.getRandomY() + this.getDeltaMovement().y, this.getRandomZ(this.getScale()) + this.getDeltaMovement().z, delta.x, delta.y, delta.z);
            }
        }
