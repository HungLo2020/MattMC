...
import net.matt.quantize.block.block.AmbersolBlock;
...
public class AmbersolFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (blockpos.getY() >= worldgenlevel.getMaxBuildHeight() - 3) {
            ...
            for (int i = 0; i < 3; i++) {
                drawOrb(worldgenlevel, blockpos.offset(randomsource.nextInt(4) - 2, randomsource.nextInt(4) - 2, randomsource.nextInt(4) - 2), randomsource, QBlocks.AMBER.get().defaultBlockState(), 2 + randomsource.nextInt(2), 2 + randomsource.nextInt(2), 2 + randomsource.nextInt(2));
            }
            drawOrb(worldgenlevel, blockpos, randomsource, QBlocks.AMBER.get().defaultBlockState(), 2, 2, 2);
            worldgenlevel.setBlock(blockpos, QBlocks.AMBERSOL.get().defaultBlockState(), 3);
