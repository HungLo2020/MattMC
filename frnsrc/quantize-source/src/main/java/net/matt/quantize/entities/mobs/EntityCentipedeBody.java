...
import net.matt.quantize.network.packet.MessageHurtMultipart;
...
import net.matt.quantize.modules.entities.IHurtableMultipart;
...
public class EntityCentipedeBody extends Mob implements IHurtableMultipart {
            ...
            if (parent != null && !this.level().isClientSide) {
                if (parent instanceof final LivingEntity parentEntity) {
                    if ((parentEntity.hurtTime > 0 || parentEntity.deathTime > 0)) {
                        NetworkHandler.sendMSGToAll(new MessageHurtMultipart(this.getId(), parent.getId(), 0));
    ...
    public static EntityCentipedeBody create(EntityType<EntityCentipedeBody> type, Level level) {
        return new EntityCentipedeBody(type, level);
    }
    ...
    public boolean hurt(DamageSource source, float damage) {
        ...
        if (prev && !this.level().isClientSide) {
            NetworkHandler.sendMSGToAll(new MessageHurtMultipart(this.getId(), parent.getId(), damage * this.damageMultiplier));
        }
        return prev;
    }
