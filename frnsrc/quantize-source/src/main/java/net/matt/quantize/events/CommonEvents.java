...
public class CommonEvents {
    ...
    public void livingAttack(LivingAttackEvent event) {
        ...
        if (event.getSource() != null && event.getSource().getDirectEntity() instanceof LivingEntity directSource && directSource.hasEffect(QEffects.STUNNED.get())) {
            event.setCanceled(true);
        }
    }
    ...
    public void onEntityJoinWorld(MobSpawnEvent.FinalizeSpawn event) {
        try {
            ...
            Quantize.LOGGER.warn("Tried to add unique behaviors to vanilla mobs and encountered an error");
        }
    }*/
    ...
    public void onServerAboutToStart(ServerAboutToStartEvent event) {
        Quantize.LOGGER.info("HUNGTESTonServerAboutToStart called!");
        ModBiomeRarity.init();
        //moved from citadel
        ...
        for (ResourceKey<LevelStem> levelStemResourceKey : levelStems.registryKeySet()) {
            ...
            if (holderOptional.isPresent() && holderOptional.get().value().generator().getBiomeSource() instanceof BiomeSourceAccessor expandedBiomeSource) {
                ...
                if (levelStemResourceKey.equals(LevelStem.OVERWORLD)) {
                    ...
                    for (ResourceKey<Biome> biomeResourceKey : QBiomes.MOD_CAVE_BIOMES) {
                        allBiomes.getHolder(biomeResourceKey).ifPresent(biomeHolders::add);
                    }
                    ImmutableSet<Holder<Biome>> biomesToAdd = biomeHolders.build();
                    Quantize.LOGGER.info("Biomes being added to expandBiomesWith: {}", biomesToAdd.stream()
                            .map(holder -> holder.unwrapKey()
                                    .map(key -> key.location().toString())
                                    .orElse("Unknown"))
                            .toList());
                    expandedBiomeSource.expandBiomesWith(biomesToAdd);
    ...
    private static boolean itemTagContainsAC(CompoundTag tag, String tagID, boolean allowUndergroundCabin) {
        if (tag.contains(tagID)) {
            ...
            return resourceLocation.contains("quantize:") && (!allowUndergroundCabin || !resourceLocation.contains("underground_cabin"));
        }
        return false;
    }
}