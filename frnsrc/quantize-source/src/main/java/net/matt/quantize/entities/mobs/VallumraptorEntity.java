...
import net.matt.quantize.entities.ai.navigators.GroundPathNavigatorNoSpin;
import net.matt.quantize.modules.entities.ChestThief;
import net.matt.quantize.modules.entities.PackAnimal;
import net.matt.quantize.modules.entities.TargetsDroppedItems;
...
public class VallumraptorEntity extends DinosaurEntity implements IAnimatedEntity, ICustomCollisions, PackAnimal, ChestThief, TargetsDroppedItems {
    ...
    private static final Predicate<LivingEntity> VALLUMRAPTOR_TARGETS = living -> living.getType().is(QTags.VALLUMRAPTOR_TARGETS);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(7, new TemptGoal(this, 1.1D, Ingredient.of(QItems.DINOSAUR_NUGGET.get()), false));
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return QEntities.VALLUMRAPTOR.get().create(level);
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (eatHeldItemIn > 0) {
                ...
                this.heal(5);
                if (stack.is(QItems.DINOSAUR_NUGGET.get()) && justLootedChest) {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficultyIn, MobSpawnType reason, @javax.annotation.Nullable SpawnGroupData spawnDataIn, @javax.annotation.Nullable CompoundTag dataTag) {
        if (spawnDataIn instanceof AgeableMobGroupData) {
            AgeableMobGroupData data = (AgeableMobGroupData) spawnDataIn;
    ...
    public void actuallyPlayAmbientSound() {
        ...
        if (this.getAnimation() == ANIMATION_CALL_2) {
            soundevent = QSounds.VALLUMRAPTOR_CALL.get();
            volume += 1.0F;
        }
        if (soundevent != null) {
    ...
    public void afterSteal(BlockPos stealPos) {
        ...
        if (this.getItemInHand(InteractionHand.MAIN_HAND).is(QItems.DINOSAUR_NUGGET.get())) {
    ...
    public boolean canTargetItem(ItemStack stack) {
        return (stack.is(QTags.VALLUMRAPTOR_STEALS) || stack.getItem().isEdible() && stack.getItem().getFoodProperties(stack, this).isMeat()) && !stack.is(QBlocks.VALLUMRAPTOR_EGG.get().asItem());
    }
    ...
    public BlockState createEggBlockState() {
        return QBlocks.VALLUMRAPTOR_EGG.get().defaultBlockState().setValue(MultipleDinosaurEggsBlock.EGGS, 1 + random.nextInt(3));
    }
    ...
    public boolean isFood(ItemStack stack) {
        return this.isTame() && stack.is(QItems.DINOSAUR_NUGGET.get());
    }
    ...
    protected SoundEvent getAmbientSound() {
        return this.getRelaxedFor() > 0 ? QSounds.VALLUMRAPTOR_SLEEP.get() : QSounds.VALLUMRAPTOR_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.VALLUMRAPTOR_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.VALLUMRAPTOR_DEATH.get();
    }


    @Override
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(QItems.SERENE_SALAD.get()) && this.getRelaxedFor() > 0 && !this.isTame()) {
            this.heal(5);
