...
public class EntityRoadrunner extends Animal {
    ...
    public EntityRoadrunner(EntityType type, Level worldIn) {
        super(type, worldIn);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.1D, Ingredient.of(QTags.ROADRUNNER_BREEDABLES), false));
    ...
    protected SoundEvent getAmbientSound() {
        return isMeep() || random.nextInt(2000) == 0 ? QSounds.ROADRUNNER_MEEP.get() : QSounds.ROADRUNNER_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.ROADRUNNER_HURT.get();
    }
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.ROADRUNNER_BREEDABLES);
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
        return QEntities.ROADRUNNER.get().create(p_241840_1_);
    }
    ...
    public static boolean canRoadrunnerSpawn(EntityType<? extends Animal> animal, LevelAccessor worldIn, MobSpawnType reason, BlockPos pos, RandomSource random) {
        boolean spawnBlock = worldIn.getBlockState(pos.below()).is(QTags.ROADRUNNER_SPAWNS);
        return spawnBlock && worldIn.getRawBrightness(pos, 0) > 8;
    }

    public boolean isMeep(){
        String s = ChatFormatting.stripFormatting(this.getName().getString());
        return (s != null && s.toLowerCase().contains("meep")) || Quantize.isAprilFools();
    }
}
