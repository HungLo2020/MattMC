...
import net.matt.quantize.entities.ai.navigators.*;
import net.matt.quantize.modules.entities.AquaticMoveController;
import net.matt.quantize.modules.entities.ITargetsDroppedItems;
import net.matt.quantize.modules.entities.IHerdPanic;
import net.matt.quantize.modules.entities.ISemiAquatic;
...
public class EntitySeal extends Animal implements ISemiAquatic, IHerdPanic, ITargetsDroppedItems {
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.SEAL_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return QSounds.SEAL_HURT.get();
    }
    ...
    public static boolean canSealSpawn(EntityType<? extends Animal> animal, LevelAccessor worldIn, MobSpawnType reason, BlockPos pos, RandomSource random) {
        ...
        if (!holder.is(Biomes.FROZEN_OCEAN) && !holder.is(Biomes.DEEP_FROZEN_OCEAN)) {
            boolean spawnBlock = worldIn.getBlockState(pos.below()).is(QTags.SEAL_SPAWNS);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(10, new TemptGoal(this, 1.1D, Ingredient.fromValues(Stream.of(new Ingredient.TagValue(QTags.SEAL_BREEDABLES), new Ingredient.TagValue(QTags.SEAL_OFFERINGS))), false));
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, EntityCatfish.class, 55, true, true, null));
    ...
    private boolean isBiomeArctic(LevelAccessor worldIn, BlockPos position) {
        return worldIn.getBiome(position).is(QTags.SPAWNS_WHITE_SEALS);
    }
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(QTags.SEAL_BREEDABLES);
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        EntitySeal seal = QEntities.SEAL.get().create(serverWorld);
    ...
    public boolean canTargetItem(ItemStack stack) {
        return stack.is(QTags.SEAL_OFFERINGS) || stack.is(QTags.SEAL_BREEDABLES);
    }

    @Override
    public void onGetItem(ItemEntity e) {
        if (e.getItem().is(QTags.SEAL_OFFERINGS)) {
            fishFeedings++;
