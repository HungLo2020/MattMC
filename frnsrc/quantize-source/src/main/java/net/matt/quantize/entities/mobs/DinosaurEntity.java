...
import net.matt.quantize.modules.entities.raycoms.IAdvancedPathingMob;
...
public abstract class DinosaurEntity extends TamableAnimal implements IDancesToJukebox, LaysEggs, IAdvancedPathingMob, RidingMeterMount {
    ...
    public static boolean checkPrehistoricSpawnRules(EntityType<? extends Animal> type, LevelAccessor levelAccessor, MobSpawnType mobType, BlockPos pos, RandomSource randomSource) {
        return levelAccessor.getBlockState(pos.below()).is(QTags.DINOSAURS_SPAWNABLE_ON) && levelAccessor.getFluidState(pos).isEmpty() && levelAccessor.getFluidState(pos.below()).isEmpty();
    }
    ...
    public void handleEntityEvent(byte b) {
        if (b == 77) {
            ...
            if(b == 82){
                options = QParticles.DINOSAUR_TRANSFORMATION_AMBER.get();
            }else {
                options = QParticles.DINOSAUR_TRANSFORMATION_TECTONIC.get();
            }
    ...
    public int getAltSkinForItem(ItemStack stack) {
        if (stack.is(QItems.AMBER_CURIOSITY.get())) {
            return 1;
        } else if (stack.is(QItems.TECTONIC_SHARD.get())) {
            return 2;
        } else {
            return 0;
        }
    ...
    public BlockState createEggBeddingBlockState() {
        return QBlocks.FERN_THATCH.get().defaultBlockState();
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!interactionresult.consumesAction() && !type.consumesAction()) {
            ...
            if (altSkinFromItem > 0) {
                ...
                this.playSound(altSkinFromItem == 2 ? QSounds.TECTONIC_SHARD_TRANSFORM.get() : QSounds.AMBER_MONOLITH_SUMMON.get());
                if (!level().isClientSide) {
                ...
                if (canOwnerCommand(player)) {
                    ...
                    if (this.getType() == QEntities.SUBTERRANODON.get() && this.canAddPassenger(player)) {
