...
public class AnimationMessage {
    ...
    public static class Handler {
        ...
        /*public static void handle(AnimationMessage message, Supplier<NetworkEvent.Context> context) {
            Quantize.PROXY.handleAnimationPacket(message.entityID, message.index);
            context.get().setPacketHandled(true);
        }*/

        public static void handle(AnimationMessage message, Supplier<NetworkEvent.Context> context) {
            if (FMLEnvironment.dist == Dist.CLIENT) {
                // Perform client-specific logic here
                AnimationHandler.INSTANCE.handleAnimationPacket(message.entityID, message.index);
            }
            context.get().setPacketHandled(true);
        }
    }
