...
import net.matt.quantize.entities.ai.goals.*;
import net.matt.quantize.entities.ai.navigators.AdvancedPathNavigateNoTeleport;
...
import net.matt.quantize.modules.entities.KeybindUsingMount;
import net.matt.quantize.modules.entities.ShakesScreen;
import net.matt.quantize.item.QItems;
import net.matt.quantize.network.packet.MountedEntityKeyMessage;
import net.matt.quantize.network.packet.UpdateEffectVisualityEntityMessage;
...
import net.matt.quantize.modules.entities.Animation;
import net.matt.quantize.modules.entities.AnimationHandler;
import net.matt.quantize.modules.entities.IAnimatedEntity;
import net.matt.quantize.modules.entities.LegSolver;
import net.matt.quantize.modules.entities.raycoms.ITallWalker;
...
public class TremorsaurusEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, ITallWalker {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new TemptGoal(this, 1.1D, Ingredient.of(QBlocks.COOKED_DINOSAUR_CHOP.get(), QBlocks.DINOSAUR_CHOP.get()), false));
    ...
    public void tick() {
        ...
        if (this.onGround() && !this.isInFluidType() && this.walkAnimation.speed() > 0.1F && !this.isBaby()) {
            ...
            if (Math.abs(f) < 0.2) {
                if (screenShakeAmount <= 0.3) {
                    this.playSound(QSounds.TREMORSAURUS_STOMP.get(), 2, 1.0F);
                    this.shakeWater();
                }
        ...
        if (!level().isClientSide) {
            ...
            if (!held && getHeldMobId() != -1) {
                this.setHeldMobId(-1);
                this.playSound(QSounds.TREMORSAURUS_THROW.get());
                riderHitEntity = null;
            }
        } else {
            Player player = Quantize.PROXY.getClientSidePlayer();
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (Quantize.PROXY.isKeyDown(2) && getMeterAmount() >= 1.0F) {
                    NetworkHandler.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 2));
                }
                if (Quantize.PROXY.isKeyDown(3) && (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null)) {
                    NetworkHandler.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 3));
                }
            }
        }
        if (this.isVehicle()) {
    ...
    private void playRoarSound() {
        if(this.isBaby()){
            this.playSound(QSounds.TREMORSAURUS_ROAR.get(), 1.0F, 1.5F);
        }else{
            this.playSound(QSounds.TREMORSAURUS_ROAR.get(), 4.0F, 1.0F);
        }
    }
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int z = -radius; z <= radius; z++) {
                    if (x * x + z * z <= radius * radius) {
                        ...
                        if (water > 0.0F) {
                            level().addParticle(QParticles.WATER_TREMOR.get(), mutableBlockPos.getX() + 0.5F, mutableBlockPos.getY() + water + 0.01, mutableBlockPos.getZ() + 0.5F, 0, 0, 0);
                        }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return QEntities.TREMORSAURUS.get().create(level);
    }

    @Override
    public boolean wantsToAttack(LivingEntity living, LivingEntity owner) {
        if(living instanceof TremorsaurusEntity tremorsaurus && (tremorsaurus.getTameAttempts() > 0 || tremorsaurus.hasEffect(QEffects.STUNNED.get()))){
            return false;
        }
    ...
    public BlockState createEggBlockState() {
        return QBlocks.TREMORSAURUS_EGG.get().defaultBlockState();
    }
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(QItems.SERENE_SALAD.get()) && this.hasEffect(QEffects.STUNNED.get())) {
            this.removeEffect(QEffects.STUNNED.get());
            NetworkHandler.sendMSGToAll(new UpdateEffectVisualityEntityMessage(this.getId(), this.getId(), 3, 0, true));
            if (!level().isClientSide) {
    ...
    public boolean isFood(ItemStack stack) {
        return this.isTame() && (stack.is(QBlocks.COOKED_DINOSAUR_CHOP.get().asItem()) || stack.is(QBlocks.DINOSAUR_CHOP.get().asItem()));
    }
    ...
    protected SoundEvent getAmbientSound() {
        return QSounds.TREMORSAURUS_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return QSounds.TREMORSAURUS_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return QSounds.TREMORSAURUS_DEATH.get();
    }
