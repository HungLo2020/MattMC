...
public class DhMinecraftLevelHook implements MinecraftLevelHooks {
    @Override
    public void onLevelUpdate(@Nullable ClientLevel previousLevel, @Nullable ClientLevel newLevel) {
        if (previousLevel != null && newLevel != previousLevel) {
            ClientApi.INSTANCE.clientLevelUnloadEvent(ClientLevelWrapper.getWrapper(previousLevel));
        }
