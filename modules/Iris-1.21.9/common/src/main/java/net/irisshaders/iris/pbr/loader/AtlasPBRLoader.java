...
import net.iris.mixin.texture.AnimationMetadataSectionAccessor;
import net.iris.mixin.texture.TextureAtlasAccessor;
...
import net.iris.pbr.texture.PBRAtlasTexture;
import net.iris.pbr.texture.PBRSpriteHolder;
import net.iris.pbr.texture.PBRType;
import net.iris.pbr.texture.SpriteContentsExtension;
import net.iris.pbr.util.ImageManipulationUtil;
...
public class AtlasPBRLoader implements PBRTextureLoader<TextureAtlas> {
	...
	protected static class PBRSpriteContents extends SpriteContents implements CustomMipmapGenerator.Provider {
		...
		public PBRSpriteContents(ResourceLocation name, FrameSize size, NativeImage image, ResourceMetadata metadata, PBRType pbrType) {
			super(name, size, image, metadata.getSection(AnimationMetadataSection.TYPE), List.of());
